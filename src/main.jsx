...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],clients=[],updateInvoice,csrfToken}){
  ...
  useEffect(()=>{const standardIds=['ref','reception','handling','storage','transport','waiting'];const standardSet=new Set(standardIds);billableCases.forEach(item=>{const existing=invoices.find(invoice=>invoice.expediente===item.id);const draft=draftInvoiceFromCase(item,warehouseEntries);const locked=existing&&['Enviado a Holded','Facturado','Cobrado'].includes(existing.estado);if(locked)return;if(!existing){updateInvoice(draft);return}const currentStandard=standardIds.map(id=>{const line=(existing.lines||[]).find(entry=>entry.id===id);return line?[id,line.item,line.detail,Number(line.price)||0,Number(line.units)||0,line.tax||'0%']:null}).filter(Boolean);const draftStandard=standardIds.map(id=>{const line=(draft.lines||[]).find(entry=>entry.id===id);return line?[id,line.item,line.detail,Number(line.price)||0,Number(line.units)||0,line.tax||'0%']:null}).filter(Boolean);const needsRefresh=String(existing.concepto||'')!==String(draft.concepto||'')||String(existing.buque||'')!==String(draft.buque||'')||String(existing.puerto||'')!==String(draft.puerto||'')||JSON.stringify(currentStandard)!==JSON.stringify(draftStandard);if(!needsRefresh)return;const customLines=(existing.lines||[]).filter(line=>!standardSet.has(line.id));updateInvoice({...draft,id:existing.id,cliente:existing.cliente||draft.cliente,estado:existing.estado||draft.estado,vencimiento:existing.vencimiento||draft.vencimiento,proforma:existing.proforma||draft.proforma,observaciones:existing.observaciones||draft.observaciones,payment:existing.payment||draft.payment,lines:[...draft.lines,...customLines]})})},[billableCases.map(item=>`${item.id}:${item.buque}:${item.puerto}:${item.eta}:${item.etb}:${item.etd}:${item.cliente}:${item.updatedAt||''}`).join('|'),invoices.map(item=>`${item.id}:${item.expediente}:${item.estado}:${item.concepto}:${item.buque}:${item.puerto}:${(item.lines||[]).map(line=>[line.id,line.item,line.detail,line.price,line.units,line.tax].join(':')).join('|')}`).join('||'),warehouseEntries.map(item=>`${item.ref}:${item.expediente}:${item.dias}:${item.estado}:${item.archivado}:${item.salida||''}:${item.updatedAt||''}`).join('|')]);
