...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const saveCalendarEvent=event=>{
    ...
    colored={...colored,scheduleStatus:colored.inicio?'confirmed':'missing_time',scheduleNote:colored.inicio?'':'Falta hora ETB; pendiente de confirmar horario'};
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {tab==='calendario'&&<>{!driverOnly&&<DriverLegend team={operationalTeam}/>} {!driverOnly&&(hasRole(effectiveRoles,'operations')||hasRole(effectiveRoles,'admin'))&&<section className="calendar-clean-bar"><div><b>¿Servicios duplicados o incorrectos?</b><small>Limpia la semana y reconstruye solo transportes con ETB. Las recepciones quedan en expediente/almacén.</small></div><button className="button warning" onClick={rebuildCalendarServices}><RefreshCw/> Recrear solo transportes</button></section>}<Calendario events={calendarEvents} team={operationalTeam} cases={cases} transports={transports} providers={providers} warehouseEntries={warehouseEntries} saveEvent={saveCalendarEvent} completeCaseStep={completeCaseStep} undoCaseStep={undoCaseStep} openCase={openCase} currentUser={visibleUser} csrfToken={auth.csrfToken} reloadOperational={loadOperational} notify={notify} rebuildCalendarServices={rebuildCalendarServices}/></>}
...
const calendarNeedsTime=event=>!event?.inicio||['provisional','missing_time'].includes(String(event?.scheduleStatus||''));
...
function CalendarEventContent({event,cases}){const related=cases.find(item=>item.id===event.expediente);const schedule=related?portCallSchedule(related):null;const missingTime=calendarNeedsTime(event);return <><time>{missingTime?'FALTA HORARIO':`${event.inicio}${event.fin?`–${event.fin}`:''}`}</time><b>{related?.buque||event.titulo||'Buque sin indicar'}</b><small className="calendar-service">{event.tipoServicio||'Transporte'}</small>{missingTime&&<small className="calendar-provisional">PENDIENTE ETB / HORA</small>}<small>{event.asignado||'Sin asignar'}</small><small>{related?.puerto||'Puerto sin indicar'}</small>{schedule&&<small className="calendar-port-call">LLEGADA · ETA {schedule.eta}</small>}</>}
