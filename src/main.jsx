...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],transports=[],calendarEvents=[],clients=[],updateInvoice,syncInvoices,csrfToken,currentUser}){
  ...
  const visibleInvoices=invoices.filter(item=>item.estado!=='Archivado');
  ...
  const sortedVisibleInvoices=[...visibleInvoices].sort((left,right)=>{
  ...
  return <>
    ...
    <section className="panel"><SectionHeader title="Documentos de facturación" subtitle={archivedInvoices.length?`Borradores internos editables con formato Holded · ${archivedInvoices.length} archivados`:'Borradores internos editables con formato Holded'} action={<label className="billing-sort-control"><span>Ordenar por</span><select value={billingSort} onChange={event=>setBillingSort(event.target.value)}><option value="exp_desc">Expediente mayor → menor</option><option value="exp_asc">Expediente menor → mayor</option><option value="doc_desc">Documento mayor → menor</option><option value="doc_asc">Documento menor → mayor</option></select></label>}/><div className="responsive-table billing-table"><div className="table-head"><span>Documento / expediente</span><span>Cliente</span><span>Concepto</span><span>Importe</span><span>Estado</span><span/></div>{sortedVisibleInvoices.length?sortedVisibleInvoices.map(item=><div className="table-row" key={item.id}><span className="primary-cell"><span className="invoice-icon"><ReceiptText/></span><span><b>{item.id}</b><button onClick={()=>openCase(item.expediente)}>{item.expediente}</button>{item.holdedStatus&&<small>Holded: {item.holdedStatus}{item.holdedNumber?`  -  ${item.holdedNumber}`:''}</small>}</span></span><span data-label="Cliente">{item.cliente}</span><span data-label="Concepto">{item.concepto}</span><strong data-label="Importe">{moneyExact(item.importe||invoiceTotal(item))}</strong><span data-label="Estado"><Badge>{item.estado}</Badge></span><span className="billing-row-actions"><button className="icon-button" aria-label={'Editar '+item.id} onClick={()=>setEditing(item)}><PencilLine/></button>{canAdmin&&<button className="icon-button danger" aria-label={'Archivar '+item.id} onClick={()=>archiveInvoice(item)} title="Archivar de facturación"><Archive/></button>}<button className="button secondary compact" disabled={sendingHolded===item.id||item.estado==='Enviado a Holded'} onClick={()=>sendHolded(item)}>{sendingHolded===item.id?'Enviando…':'Enviar a Holded'}</button></span></div>):<Empty text="Todavía no hay borradores visibles. Cuando un expediente esté listo para facturar se creará automáticamente."/>}</div></section>
