...
function NewCaseModal({clientOptions=[],vessels=[],team=[],close,submit}){
  ...
  const [form,setForm]=useState({buque:'',imo:'',mmsi:'',cliente:clientOptions[0]||'UME Shipping',puerto:'Barcelona',etaDate:'',etaTime:'',prioridad:'Media',bultos:'',createReception:false,receptionDate:'',receptionStart:'09:00',receptionEnd:'10:00',receptionLocation:warehouse,createTransport:false,transportDate:'',transportStart:'',transportEnd:'',transportPickup:warehouse,transportDelivery:'BUQUE POR CONFIRMAR  -  Barcelona',transportConductor:'Sin asignar'});
  const update=event=>{
    ...
    if(name==='etaDate'){
      setForm(current=>({...current,etaDate:value,receptionDate:current.receptionDate||value,transportDate:value||current.transportDate}));
      return;
    }
    if(name==='etaTime'){
      setForm(current=>({...current,etaTime:value,transportStart:value&&!current.transportStart?value:current.transportStart,transportEnd:value&&!current.transportEnd?plusHourClient(value):current.transportEnd}));
      return;
    }
