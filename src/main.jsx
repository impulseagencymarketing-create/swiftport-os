...
const cleanSupplierPdfText=text=>{
  const allowed=/[A-Za-z0-9ÁÃ‰ÍÃ“ÃšÃœÃ‘áéíóúüñ€$.,:;()\/+\-\s]/g;
  ...
  const lines=String(text||'').split(/\r?\n/).map(line=>{
    ...
    if(!/[A-Za-zÁÃ‰ÍÃ“ÃšÃœÃ‘áéíóúüñ]/.test(cleaned))return '';
...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const createCase=async form=>{
    ...
    const item=normalizeMerchandise({id,buque:vessel,imo:cleanImo(form.imo)||known.imo||'',mmsi:cleanMmsi(form.mmsi)||known.mmsi||'',cliente,puerto:form.puerto,eta:etaDate||'Por confirmar',portCall:{etaDate,etaTime,etbDate,etbTime,etdDate,etdTime,updatedAt:new Date().toISOString()},serviceType:form.serviceType||'vessel_delivery',estado:'Nuevo',prioridad:form.prioridad,conductor:'Sin asignar',servicios:[surveyService&&'Survey / muestras a bordo',!surveyService&&form.createReception&&'Recepción',form.createTransport&&(surveyService?'Survey':'Transporte')].filter(Boolean),bultos:surveyService?0:Number(form.bultos)||0,peso:surveyService?'No aplica':'Por registrar',progreso:0,siguiente:surveyService?'Revisar servicio y agendar surveyor':'Revisar expediente y servicios programados',aduana:surveyService?'No aplica':'Por revisar',autoTransportDisabled:false,manualVesselName:true,manualEditedAt:new Date().toISOString()});
    ...
    const route=[form.transportPickup,form.transportDelivery].filter(Boolean).join(' → ')||(surveyService?`SURVEYOR → BUQUE ${vessel}  -  ${form.puerto}`:`ALMACÃ‰N → ${form.puerto}`);
    ...
    <section className="modal operation-modal">
      ...
      <div className="operation-modal-body">
        ...
        {needsEvidence&&<div className="pod-scanner evidence-capture">
          ...
          {step.key==='delivery'&&<div className="evidence-requirements"><span className={deliveryPhotos.length?'done':''}><CheckCircle2/> {surveyService?'Foto / evidencia':storageOnly?'Evidencia de salida':'Foto de entrega'} {deliveryPhotos.length?`(${deliveryPhotos.length})`:'pendiente'}</span>{!storageOnly&&!surveyService&&<span className={podFiles.length||podException?'done':''}><CheckCircle2/> {podException?'POD no sellado justificado':'POD firmado'} {podFiles.length?`(${podFiles.length})`:podException?'con observación':'pendiente'}</span>}{surveyService&&<span className={podFiles.length?'done':''}><CheckCircle2/> Informe / documento {podFiles.length?`(${podFiles.length})`:'opcional'}</span>}</div>}
          {evidenceFiles.length>0&&<div className="evidence-file-list">{evidenceFiles.map((file,index)=><a className="pod-uploaded" href={file.url} target="_blank" rel="noreferrer" key={`${file.id}-${index}`}><CheckCircle2/><span><b>{file.evidenceType==='pod'?(surveyService?'Informe / documento':'POD escaneado'):file.evidenceType==='delivery-photo'?(surveyService?'Evidencia survey':'Foto de entrega'):'Foto de recepción'}</b><small>{file.name}</small></span><ExternalLink/></a>)}</div>}
          {error&&<p className="form-error"><CircleAlert/>{error}</p>}
        </div>}
...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],transports=[],calendarEvents=[],clients=[],updateInvoice,syncInvoices,csrfToken,currentUser}){
  ...
  const archiveInvoice=item=>{
    if(!canAdmin){notify('Solo un administrador puede archivar facturación.');return}
    updateInvoice({...item,estado:'Archivado',archivedAt:new Date().toISOString()});
    notify(`${item.expediente} archivado de facturación`);
  };
  const archiveReadyCase=item=>{
    ...
    notify(`${item.id} archivado de facturación`);
  };
