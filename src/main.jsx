...
function NewCaseModal({clientOptions=[],vessels=[],team=[],close,submit}){
  ...
  const [form,setForm]=useState({buque:'',imo:'',mmsi:'',cliente:clientOptions[0]||'UME Shipping',puerto:'Barcelona',eta:'',prioridad:'Media',bultos:'',createReception:false,receptionDate:'',receptionStart:'09:00',receptionEnd:'10:00',receptionLocation:warehouse,createTransport:false,transportDate:'',transportStart:'',transportEnd:'',transportPickup:warehouse,transportDelivery:'BUQUE POR CONFIRMAR  -  Barcelona',transportConductor:'Sin asignar'});
  const update=event=>{
    ...
    if(name==='eta'){
      ...
      setForm(current=>({...current,eta:value,receptionDate:current.receptionDate||date,transportDate:date||current.transportDate,transportStart:time&&!current.transportStart?time:current.transportStart,transportEnd:time&&!current.transportEnd?plusHourClient(time):current.transportEnd}));
      return;
    }
