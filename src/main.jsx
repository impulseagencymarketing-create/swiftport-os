...
const formatReceptionDate=value=>value?new Date(value).toLocaleString('es-ES',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'}):'Sin fecha';
...
function CargoManifest({item}){
  ...
  return <div className="driver-manifest wide"><div><Box/><span><b>CARGA PARA EL CONDUCTOR</b><small>{item.buque} · {item.puerto}</small></span></div>{(item.mercancias||[]).map(piece=><p key={piece.id}><b>{piece.cantidad} {piece.tipo}{piece.cantidad===1?'':'S'}</b><span>{piece.seguimiento?`Tracking: ${piece.seguimiento}`:'Sin seguimiento'}</span></p>)}{Boolean(photos.length||receptionDocuments.length)&&<div className="manifest-arrival-files"><Camera/><span><b>{photos.length} fotos · {receptionDocuments.length} documentos de llegada</b><small>Disponibles en el expediente</small></span></div>}<footer><span>Aduanas: {documentation.alcance==='global'?(documentation.aduaneroDisponible?`${documentation.tipoAduanero} DISPONIBLE`:'PENDIENTE'):'DOCUMENTOS INDIVIDUALES'}</span><span>POD: {documentation.podDisponible?'DISPONIBLE':'PENDIENTE'}</span></footer></div>;
}
...
function WarehouseEntryModal({cases,close,submit,csrfToken}){
  const [form,setForm]=useState({expediente:cases[0]?.id||'',fechaRecepcion:localDateTimeValue(),zona:'A-01',peso:'100',mercancias:[{tipo:'CAJA',cantidad:'1',seguimiento:''}]});
  ...
  const selectPhotos=event=>setPhotos([...event.target.files].map((file,index)=>({file,preview:URL.createObjectURL(file),tipo:index===0?'VISTA GENERAL':'ESTADO DE EMBALAJE',mercanciaIndex:'GENERAL',nota:''})));
  ...
  const save=async event=>{
    ...
    try{
      ...
      const uploadedPhotos=await Promise.all(photos.map(async photo=>{
        ...
        const merchandise=photo.mercanciaIndex==='GENERAL'||!line?'RECEPCIÓN COMPLETA':`${line.cantidad} ${line.tipo}${Number(line.cantidad)===1?'':'S'}${line.seguimiento?` · ${line.seguimiento.toUpperCase()}`:''}`;
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)close()}}><section className="modal warehouse-entry-modal" role="dialog" aria-modal="true" aria-labelledby="warehouse-entry-title"><div className="modal-head"><div><span className="overline">Almacén</span><h2 id="warehouse-entry-title">Registrar mercancía</h2><p>Documenta la mercancía exactamente como llega.</p></div><button className="icon-button" aria-label="Cerrar" disabled={busy} onClick={close}><X/></button></div><form onSubmit={save}><label className="field wide"><span>Expediente</span><select name="expediente" value={form.expediente} onChange={update} required>{cases.map(item=><option value={item.id} key={item.id}>{caseLabel(item)}</option>)}</select></label><label className="field"><span>Fecha y hora de llegada *</span><input name="fechaRecepcion" type="datetime-local" value={form.fechaRecepcion} onChange={update} required/></label><label className="field"><span>Ubicación *</span><input name="zona" value={form.zona} onChange={update} placeholder="Ej. A-01" required/></label><label className="field"><span>Peso total (kg) *</span><input name="peso" type="number" min="0.1" step="0.1" value={form.peso} onChange={update} required/></label><div className="cargo-lines wide"><div className="cargo-lines-title"><b>Mercancías</b><button type="button" className="button secondary" onClick={addLine}><Plus/> Añadir tipo</button></div>{form.mercancias.map((line,index)=><div className="cargo-line" key={index}><label className="field"><span>Tipo</span><select value={line.tipo} onChange={event=>updateLine(index,'tipo',event.target.value)}><option>CAJA</option><option>PALLET</option><option>SOBRE</option></select></label><label className="field"><span>Cantidad</span><input type="number" min="1" value={line.cantidad} onChange={event=>updateLine(index,'cantidad',event.target.value)} required/></label><label className="field tracking-field"><span>N.º seguimiento (opcional)</span><input value={line.seguimiento} onChange={event=>updateLine(index,'seguimiento',event.target.value)} placeholder="Tracking / AWB"/></label>{form.mercancias.length>1&&<button type="button" className="icon-button remove-cargo" onClick={()=>removeLine(index)}><X/></button>}</div>)}</div><div className="arrival-files wide"><label className="file-picker"><Camera/><span><b>Fotos de la mercancía *</b><small>Después identifica qué muestra cada imagen</small></span><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" multiple required onChange={selectPhotos}/></label><label className="file-picker"><FileText/><span><b>Escanear documentos</b><small>Packing list, CMR, delivery note o albarán</small></span><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf,.pdf" multiple onChange={event=>setDocuments([...event.target.files])}/></label>{Boolean(photos.length)&&<div className="photo-identification"><div className="photo-identification-title"><b>Identificación fotográfica</b><small>Asocia cada evidencia con su mercancía.</small></div>{photos.map((photo,index)=><article key={`${photo.file.name}-${photo.file.lastModified}`}><img src={photo.preview} alt={`Vista previa ${index+1}`}/><div><span className="photo-number">FOTO {String(index+1).padStart(2,'0')}</span><label className="field"><span>Qué muestra</span><select value={photo.tipo} onChange={event=>updatePhoto(index,{tipo:event.target.value})}>{PHOTO_TYPES.map(type=><option key={type}>{type}</option>)}</select></label><label className="field"><span>Mercancía asociada</span><select value={photo.mercanciaIndex} onChange={event=>updatePhoto(index,{mercanciaIndex:event.target.value})}><option value="GENERAL">RECEPCIÓN COMPLETA</option>{form.mercancias.map((line,lineIndex)=><option key={lineIndex} value={lineIndex}>{line.cantidad} {line.tipo}{Number(line.cantidad)===1?'':'S'}{line.seguimiento?` · ${line.seguimiento.toUpperCase()}`:''}</option>)}</select></label><label className="field photo-note"><span>Observación (opcional)</span><input value={photo.nota} onChange={event=>updatePhoto(index,{nota:event.target.value})} placeholder="Ej. esquina golpeada, precinto intacto…"/></label></div></article>)}</div>}{Boolean(documents.length)&&<div className="selected-files documents-selected">{documents.map(file=><span key={`doc-${file.name}`}><FileText/>{file.name}</span>)}</div>}</div>{error&&<div className="form-error wide"><CircleAlert/>{error}</div>}<div className="modal-actions wide"><button type="button" className="button tertiary" disabled={busy} onClick={close}>Cancelar</button><button className="button primary" disabled={busy}><UploadCloud/> {busy?'Subiendo archivos…':'Registrar entrada'}</button></div></form></section></div>;
}
