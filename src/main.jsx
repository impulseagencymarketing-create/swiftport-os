...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  useEffect(()=>{
    ...
    const normalized=transports.map(item=>item.conductor!=='Sin asignar'&&!names.has(item.conductor)?{...item,conductor:'Sin asignar',estado:'Sin asignar'}:item);
    if(normalized.some((item,index)=>item!==transports[index])){setTransports(normalized);saveOperational(cases,normalized,warehouseEntries)}
