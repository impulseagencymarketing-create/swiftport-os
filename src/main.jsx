...
const OPERATION_STEPS=[
  ...
  {key:'delivered',title:'Mercancía entregada',next:'Entregar la mercancía en destino'},
  {key:'pod',title:'POD recibido',next:'Obtener y registrar el POD'}
];
const operationFlow=item=>{
  if(item.operationalFlow)return {cargo:false,documents:false,delivered:false,pod:false,billingReady:false,...item.operationalFlow};
  const progress=Number(item.progreso)||0;
  const completed=item.estado==='Completado'||progress>=100;
  return {cargo:progress>=25,documents:progress>=50,delivered:progress>=75,pod:completed,billingReady:completed};
};
...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const completeCaseStep=(id,stepKey,note='')=>{
    ...
    const delivered=stepKey==='delivered';
    const ready=stepKey==='pod';
    if(ready)flow.billingReady=true;
    const nextStep=OPERATION_STEPS.find(step=>!flow[step.key]);
    const now=new Date();
    const timelineEntry={id:`FLOW-${id}-${stepKey}-${Date.now()}`,fecha:now.toLocaleDateString('es-ES'),hora:now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),titulo:expected.title,detalle:note||'Paso confirmado sin incidencias',actor:visibleUser.fullName,estado:'done'};
    const nextCases=cases.map(item=>item.id===id?normalizeMerchandise({...item,operationalFlow:flow,progreso:ready?100:OPERATION_STEPS.filter(step=>flow[step.key]).length*25,siguiente:ready?'Listo para facturar':nextStep?.next||'',estado:ready?'Completado':'En curso',documentacionMercancia:ready?{...item.documentacionMercancia,podDisponible:true}:item.documentacionMercancia,timelineCustom:[timelineEntry,...(item.timelineCustom||[])]}):item);
    const nextTransports=delivered||ready?transports.map(item=>item.expediente===id?{...item,estado:'Entregado'}:item):transports;
    const nextWarehouse=delivered||ready?warehouseEntries.map(item=>item.expediente===id?{...item,estado:'Expedido',archivado:true,salida:new Date().toISOString()}:item):warehouseEntries;
  ...
  const registerWarehouseEntry=form=>{
    ...
    const nextCases=cases.map(entry=>{
      ...
      return normalizeMerchandise({
        ...
        operationalFlow:{...operationFlow(entry),cargo:true},
        estado:'En curso',
        timelineCustom:[{
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {tab==='correos'&&<Correos csrfToken={auth.csrfToken} notify={notify} openCase={openCase} reloadOperational={loadOperational}/>}
...
function CalendarEventContent({event,cases}){const related=cases.find(item=>item.id===event.expediente);return <><time>{event.inicio}{event.fin?`–${event.fin}`:''}</time><b>{related?.buque||event.titulo||'Buque sin indicar'}</b><small className="calendar-service">{event.tipoServicio||(event.transporte?'Transporte':'Recepción')}</small><small>{event.asignado||'Sin asignar'}</small><small>{related?.puerto||'Puerto sin indicar'}</small></>}
function Calendario({events,team,cases,transports,providers,warehouseEntries,saveEvent,completeCaseStep,openCase,currentUser}){
  const [weekStart,setWeekStart]=useState(new Date('2026-06-29T12:00:00'));
  const [editing,setEditing]=useState(null);
  const [mineOnly,setMineOnly]=useState(false);
  if(currentUser.role==='driver')return <DriverCalendar events={events} cases={cases} transports={transports} warehouseEntries={warehouseEntries} currentUser={currentUser} completeCaseStep={completeCaseStep}/>;
...
function DriverCalendar({events,cases,transports,warehouseEntries,currentUser,completeCaseStep}){
  const [selected,setSelected]=useState(null);
  const assigned=events.filter(event=>event.asignado===currentUser.fullName).sort((a,b)=>(a.fecha+a.inicio).localeCompare(b.fecha+b.inicio));
  return <><section className="driver-day-hero"><div><span className="overline"><Truck/> Mi jornada</span><h2>Hola, {currentUser.fullName.split(' ')[0]}</h2><p>Abre cada servicio y completa únicamente el paso que aparece marcado.</p></div><strong>{assigned.filter(event=>cases.find(item=>item.id===event.expediente)?.estado!=='Completado').length}<small>trabajos pendientes</small></strong></section><section className="panel driver-jobs"><SectionHeader title="Mi calendario" subtitle="Servicios asignados por fecha y hora"/>{assigned.length?<div className="driver-job-list">{assigned.map(event=>{const related=cases.find(item=>item.id===event.expediente);const completed=related?.estado==='Completado';const next=related&&nextOperationStep(related);return <button key={event.id} className={completed?'completed':''} onClick={()=>setSelected(event)}><time><b>{event.inicio}</b><small>{new Date(event.fecha+'T12:00:00').toLocaleDateString('es-ES',{weekday:'short',day:'2-digit',month:'short'})}</small></time><span className="driver-job-main"><b>{related?.buque||event.titulo}</b><small>{event.tipoServicio} · {related?.puerto||'Puerto pendiente'}</small><em>{completed?'Trabajo terminado':next?.title||'Abrir trabajo'}</em></span><span className={'driver-job-status '+(completed?'done':'')}><CheckCircle2/><small>{completed?'Completo':`${operationProgress(related||{})}%`}</small></span><ChevronRight/></button>})}</div>:<Empty text="No tienes trabajos asignados en el calendario."/>}</section>{selected&&<DriverTaskModal event={selected} item={cases.find(entry=>entry.id===selected.expediente)} transport={transports.find(entry=>entry.id===selected.transporte)} warehouseEntries={warehouseEntries} close={()=>setSelected(null)} submit={(key,note)=>{completeCaseStep(selected.expediente,key,note);setSelected(null)}}/>}</>;
}

function DriverTaskModal({event,item,transport,warehouseEntries,close,submit}){
  ...
  const instructions={
    ...
    delivered:'Entrega toda la mercancía en destino y confirma quién la recibe.',
    pod:'Fotografía o escanea el POD firmado y confirma que ha quedado recibido.'
  };
  return <div className="modal-backdrop driver-task-backdrop" onMouseDown={mouse=>{if(mouse.target===mouse.currentTarget)close()}}><section className="modal driver-task-modal"><div className="modal-head"><div><span className="overline">{event.inicio}–{event.fin} · {event.tipoServicio}</span><h2>{item.buque}</h2><p>{caseLabel(item)}</p></div><button className="icon-button" onClick={close}><X/></button></div><div className="driver-task-body"><div className="driver-route"><MapPin/><span><small>PUERTO / RUTA</small><b>{transport?.ruta||item.puerto}</b></span></div><OperationChecklist item={item}/><CargoManifest item={item}/>{step?<><div className="driver-next-action"><span>{OPERATION_STEPS.findIndex(entry=>entry.key===step.key)+1}</span><div><small>AHORA TOCA</small><b>{step.title}</b><p>{instructions[step.key]}</p></div></div><label className="field"><span>Observación del trabajo (opcional)</span><input value={note} onChange={change=>setNote(change.target.value)} placeholder="Persona que recibe, incidencia, referencia…"/></label><button className="button primary full driver-confirm" onClick={()=>submit(step.key,note)}><CheckCircle2/> Confirmar: {step.title}</button></>:<div className="driver-finished"><CheckCircle2/><span><b>Trabajo terminado</b><small>POD recibido y expediente listo para facturación.</small></span></div>}<button className="button tertiary full" onClick={close}>{flow.billingReady?'Cerrar':'Volver al calendario'}</button></div></section></div>;
}
...
function Expedientes({cases,selected,select,search,setSearch,completeCaseStep,notify,showFinance,updateCase,clientOptions,warehouseEntries,transports}){
  ...
  return <div className={'case-layout '+(mobileDetail?'mobile-detail-open':'')}>
    ...
    {flowOpen&&<OperationStepModal item={selected} warehouseEntries={warehouseEntries} transports={transports} close={()=>setFlowOpen(false)} submit={(key,note)=>{completeCaseStep(selected.id,key,note);setFlowOpen(false)}}/>}
  </div>;
}
...
function OperationStepModal({item,warehouseEntries,transports,close,submit}){
  ...
  const guidance={
    ...
    delivered:'Confirma que toda la mercancía se entregó. La salida quedará archivada automáticamente en Almacén.',
    pod:'Confirma que el POD está recibido. El expediente pasará automáticamente a Facturación.'
  };
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal operation-modal"><div className="modal-head"><div><span className="overline">Paso operativo</span><h2>{step.title}</h2><p>{item.id} · {item.buque}</p></div><button className="icon-button" onClick={close}><X/></button></div><div className="operation-modal-body"><OperationChecklist item={item}/><div className="operation-guidance"><ClipboardCheck/><div><b>Qué debes comprobar</b><p>{guidance[step.key]}</p>{step.key==='delivered'&&transport&&<small>{transport.id} · {transport.ruta}</small>}</div></div><label className="field"><span>Observación (opcional)</span><input value={note} onChange={event=>setNote(event.target.value)} placeholder="Incidencias, persona que recibe, referencia…"/></label><div className="modal-actions"><button className="button tertiary" onClick={close}>Cancelar</button><button className="button primary" onClick={()=>submit(step.key,note)}><CheckCircle2/> Confirmar paso</button></div></div></section></div>;
}
...
function Correos({csrfToken,notify,openCase,reloadOperational}){
  ...
  return <><section className="mail-automation-hero"><div><Mail/><span><b>Entrada automática con IA</b><small>Los datos completos crean trabajos; los dudosos esperan revisión.</small></span></div><div><small>Última comprobación</small><b>{lastRun?.finished_at?formatReceptionDate(lastRun.finished_at):'Todavía no ejecutada'}</b></div><button className="button primary" disabled={processing} onClick={process}><RefreshCw className={processing?'spinning':''}/>{processing?'Leyendo buzones…':'Comprobar correos ahora'}</button></section>{error&&<div className="form-error"><CircleAlert/>{error}</div>}<section className="panel"><SectionHeader title="Bandeja de servicios" subtitle="info@swiftportlogistic.com y operations@swiftportlogistic.com"/><div className="mail-filters">{[['all','Todos',Object.values(counts).reduce((a,b)=>a+b,0)],['review','Revisar',counts.review],['processed','Creados',counts.processed],['ignored','Descartados',counts.ignored],['error','Errores',counts.error]].map(([value,label,total])=><button key={value} className={filter===value?'active':''} onClick={()=>setFilter(value)}>{label}<span>{total}</span></button>)}</div>{loading?<div className="users-loading">Cargando correos…</div>:items.length?<div className="mail-list">{items.map(item=><article key={item.id} className={'mail-item '+item.status}><header><div><b>{item.subject||'Sin asunto'}</b><small>{item.sender_name||item.sender_email} · {formatReceptionDate(item.received_at)}</small></div><Badge>{MAIL_STATUS[item.status]||item.status}</Badge></header>{item.extracted&&<div className="mail-extracted"><span><small>BUQUE</small><b>{item.extracted.vessel||'—'}</b></span><span><small>ETA</small><b>{item.extracted.eta||'—'}</b></span><span><small>PUERTO</small><b>{item.extracted.port||'—'}</b></span><span><small>SERVICIO</small><b>{[item.extracted.reception?.required&&'RECEPCIÓN',item.extracted.transport?.required&&'TRANSPORTE'].filter(Boolean).join(' + ')||'—'}</b></span></div>}{item.review_reason&&<p className="mail-reason"><CircleAlert/>{item.review_reason}</p>}{item.error_message&&<p className="mail-reason error"><CircleAlert/>{item.error_message}</p>}<details className="mail-original"><summary>Ver correo original</summary><pre>{item.body}</pre></details><footer>{item.case_ref&&<button className="button tertiary" onClick={()=>openCase(item.case_ref)}>Abrir {item.case_ref}</button>}{['ignored','error'].includes(item.status)&&<button className="button secondary" onClick={()=>reprocess(item)}><RefreshCw/> Reinterpretar</button>}{item.status==='review'&&<><button className="button tertiary" onClick={()=>ignore(item)}>Descartar</button><button className="button primary" onClick={()=>setEditing(item)}><PencilLine/> Revisar y crear</button></>}</footer></article>)}</div>:<Empty text="No hay correos en este estado."/>}</section>{editing&&<MailReviewModal item={editing} close={()=>setEditing(null)} submit={data=>approve(editing,data)}/>}</>;
}

function MailReviewModal({item,close,submit}){
  const base=item.extracted||{};
  const [form,setForm]=useState({
    client:base.client||'',vessel:base.vessel||'',eta:base.eta||'',port:base.port||'',priority:base.priority||'Media',cargo_summary:base.cargo_summary||'',
