...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const operationalTeam=useMemo(()=>team.filter(member=>member.role==='operations'),[team]);
  useEffect(()=>{
  ...
  const completeCaseStep=(id,stepKey,note='')=>{
    ...
    const timelineEntry={id:`FLOW-${id}-${stepKey}-${Date.now()}`,hora:new Date().toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),titulo:expected.title,detalle:note||'Registrado desde el flujo operativo',estado:'done'};
  ...
  const assignedAlerts=effectiveRole==='operations'
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {tab==='calendario'&&<><DriverLegend team={operationalTeam}/><Calendario events={calendarEvents} team={operationalTeam} cases={cases} transports={transports} providers={providers} saveEvent={saveCalendarEvent} openCase={openCase} currentUser={visibleUser}/></>}
...
function Calendario({events,team,cases,transports,providers,saveEvent,openCase,currentUser}){
...
function CalendarEventModal({item,team,cases,transports,providers,close,submit,openCase}){
  ...
  const validTeam=team.filter(member=>member.role==='operations');
