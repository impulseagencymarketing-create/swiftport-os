...
async function uploadPlanningCalendar(file,csrfToken){
  const data=new FormData();
  data.append('action','import_reference');
  data.append('calendar',file);
  const response=await fetch('/api/admin/planning.php',{method:'POST',credentials:'same-origin',headers:{'X-CSRF-Token':csrfToken},body:data});
  const body=await response.json().catch(()=>({}));
...
function Correos({csrfToken,notify,openCase,reloadOperational,canRebuild}){
  ...
  const [planning,setPlanning]=useState({referenceEvents:[],excludedEvents:[],draftTasks:[],status:'empty'});
  ...
  useEffect(()=>{if(canRebuild)api('/api/admin/planning.php').then(result=>setPlanning(result.data||planning)).catch(()=>{})},[canRebuild]);
  const importPlanning=async file=>{
    if(!file)return;
    setPlanningBusy(true);setError('');
    try{const result=await uploadPlanningCalendar(file,csrfToken);setPlanning(result.data);notify(`${result.summary.operational} tareas operativas importadas como referencia`)}
    catch(reason){setError(reason.message)}
