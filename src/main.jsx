...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const createCase=form=>{
    ...
    const item=normalizeMerchandise({id,buque:form.buque.toUpperCase(),imo:cleanImo(form.imo)||known.imo||'',mmsi:cleanMmsi(form.mmsi)||known.mmsi||'',cliente:form.cliente,puerto:form.puerto,eta:etaDate||'Por confirmar',portCall:{etaDate,etaTime,etbDate:'',etbTime:'',etdDate:'',etdTime:'',updatedAt:new Date().toISOString()},estado:'Nuevo',prioridad:form.prioridad,conductor:'Sin asignar',servicios:[form.createReception&&'Recepción',form.createTransport&&'Transporte'].filter(Boolean),bultos:Number(form.bultos)||0,peso:'Por registrar',progreso:0,siguiente:'Revisar expediente y servicios programados',aduana:'Por revisar'});
  ...
  const updateCase=updated=>{const {importe,...rawCase}=updated;const known=findKnownVessel(vessels,rawCase.buque)||{};const operationalCase=normalizeMerchandise({...rawCase,imo:cleanImo(rawCase.imo)||known.imo||'',mmsi:cleanMmsi(rawCase.mmsi)||known.mmsi||''});const next=cases.map(item=>item.id===operationalCase.id?operationalCase:item);const nextVessels=upsertVesselFromCase(vessels,operationalCase);const activeEntries=warehouseEntries.filter(entry=>entry.expediente===operationalCase.id&&!entry.archivado);const nextWarehouse=warehouseEntries.map(entry=>activeEntries.length===1&&entry.ref===activeEntries[0].ref?{...entry,buque:operationalCase.buque,mercancias:operationalCase.mercancias,bultos:merchandiseCount(operationalCase.mercancias),peso:merchandiseWeightLabel(operationalCase.mercancias)}:entry);setCases(next);setVessels(nextVessels);setWarehouseEntries(nextWarehouse);saveOperational(next,transports,nextWarehouse,customs,calendarEvents,providers,nextVessels);notify(activeEntries.length===1?'Expediente, buque y almacén actualizados':'Expediente y buque actualizados')};
  ...
  return <div className="shell">
    ...
    {newOpen&&<NewCaseModal clientOptions={clientOptions} vessels={vessels} close={()=>setNewOpen(false)} submit={createCase}/>}
...
function NewCaseModal({clientOptions=[],vessels=[],close,submit}){
