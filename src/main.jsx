...
const DEMO_TEAM=[
  {id:'local-admin',fullName:'Javier FernÃ¡ndez',email:'javier@swiftport.local',roles:['admin','operations','finance']},
  {id:'local-driver',fullName:'MoisÃ©s Rodriguez',email:'moises@swiftport.local',roles:['driver','operations']}
];
...
const normalizeClientProfile=client=>{
  ...
  return {
    ...
    direccion:client?.direccion||(isLimani?'Calle Juan de la Cierva 11, Villalbilla (28810), Madrid, EspaÃ±a':''),
    condicionesPago:client?.condicionesPago||client?.terms||'30 dÃ­as',
    moneda:client?.moneda||'EUR',
    tarifaActiva:client?.tarifaActiva||(isLimani?'LIMANI Barcelona 2026':isAls?'ALS Barcelona 2026':'Sin tarifa automÃ¡tica'),
    notas:client?.notas||'',
    expedientes:Number(client?.expedientes??client?.activeCases??0),
    recepcion:client?.recepcion||(isLimani?'0-35 kg 15â‚¬  -  35-250 kg 60â‚¬  -  251-500 kg 130â‚¬  -  501-2500 kg 245â‚¬':isAls?'LOAD / UNLOAD: 0,12 â‚¬/kg por separado':'Pendiente de tarifa'),
    storage:client?.storage||(isLimani?'0-35 kg gratis  -  36-100 kg 5â‚¬/dÃ­a  -  101-500 kg 10â‚¬/dÃ­a  -  500+ kg 15â‚¬/dÃ­a':isAls?'3 dÃ­as gratis Â· 36-100 kg 2,50â‚¬/dÃ­a Â· 101-500 kg 3,50â‚¬/dÃ­a Â· 500+ kg 7,50â‚¬/dÃ­a':'Pendiente de tarifa'),
    transporte:client?.transporte||(isLimani?'Warehouseâ†’Vessel: 45â‚¬ / 95â‚¬ / 250â‚¬ / 350â‚¬ por peso':isAls?'AÃ±adir manual segÃºn servicio: aeropuerto, recogidas o transporte especial':'Pendiente de tarifa'),
...
const parseWarehouseMoment=value=>{
  ...
  const match=normalized.toLowerCase().match(/(\d{1,2})\s+([a-zÃ¡Ã©Ã­Ã³Ãº]{3,})\s*(\d{1,2}:\d{2})?/i);
...
const defaultClientCostEstimate=(item,warehouseEntries=[])=>{
  ...
  if(isAlsCase(item)){
    ...
    return {
      ...
      note:'Tarifa ALS Barcelona 2026. Storage con 3 dÃ­as gratis.',
      lines:[
  ...
  return {
    id:`COST-${item.id}`,
    title:`PREVISIÃ“N COSTES ${item.buque||item.id}`.toUpperCase(),
    note:'Sin tarifa automÃ¡tica para este cliente. AÃ±ade conceptos manuales.',
...
function DriverWeekView({events,cases,select,saveEvent,notify}){
  ...
  const isEventCompleted=event=>operationFlow(cases.find(item=>item.id===event.expediente)||{}).billingReady;
