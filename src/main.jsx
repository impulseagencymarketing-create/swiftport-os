...
const draftInvoiceFromCase=(item,warehouseEntries=[])=>{
  ...
  return {...invoice,importe:invoiceTotal(invoice),_autoSignature:JSON.stringify({concepto:invoice.concepto,cliente:invoice.cliente,buque:item.buque,puerto:item.puerto,eta:item.eta,etb:item.etb,etd:item.etd,cargo,storageDays,lines:lines.map(line=>[line.id,line.item,line.detail,line.price,line.units,line.tax])})};
};
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {tab==='facturacion'&&showFinance&&<Facturacion openCase={openCase} notify={notify} invoices={finance.invoices} cases={casesWithFinance} warehouseEntries={warehouseEntries} clients={finance.clients} updateInvoice={updateInvoice} csrfToken={auth.csrfToken}/>}
...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],clients=[],updateInvoice,csrfToken}){
  const [editing,setEditing]=useState(null);
  ...
  useEffect(()=>{billableCases.forEach(item=>{const existing=invoices.find(invoice=>invoice.expediente===item.id);const draft=draftInvoiceFromCase(item,warehouseEntries);const locked=existing&&['Enviado a Holded','Facturado','Cobrado'].includes(existing.estado);if(locked)return;const lineCount=(existing?.lines||[]).length;const existingIds=(existing?.lines||[]).map(line=>line.id||String(line.item||'').toLowerCase());const expectedIds=draft.lines.map(line=>line.id);const missingExpectedLine=expectedIds.some(id=>!existingIds.includes(id));const staleSignature=existing&&existing._autoSignature!==draft._autoSignature;const oldHeader=existing&&(/^SPL/i.test(String(existing.lines?.[0]?.item||''))||String(existing.concepto||'')!==draft.concepto||String(existing.lines?.[0]?.item||'')!==draft.lines[0]?.item);const needsRefresh=!existing||lineCount<4||missingExpectedLine||staleSignature||oldHeader;if(!needsRefresh)return;const customLines=(existing?.lines||[]).filter(line=>!['ref','reception','handling','storage','transport','waiting'].includes(line.id));updateInvoice({...draft,id:existing?.id||draft.id,cliente:existing?.cliente||draft.cliente,estado:existing?.estado||draft.estado,vencimiento:existing?.vencimiento||draft.vencimiento,proforma:existing?.proforma||draft.proforma,observaciones:existing?.observaciones||draft.observaciones,payment:existing?.payment||draft.payment,lines:[...draft.lines,...customLines]})})},[billableCases.map(item=>`${item.id}:${item.buque}:${item.puerto}:${item.eta}:${item.etb}:${item.etd}:${item.cliente}:${item.updatedAt||''}`).join('|'),invoices.map(item=>`${item.id}:${item.expediente}:${item.estado}:${item._autoSignature||''}:${item.concepto}:${(item.lines||[]).length}:${item.lines?.map(line=>line.id||line.item).join(',')||''}`).join('|'),warehouseEntries.map(item=>`${item.ref}:${item.expediente}:${item.dias}:${item.estado}:${item.archivado}:${item.salida||''}:${item.updatedAt||''}`).join('|')]);
  ...
  return <>
    ...
    {editing&&<InvoiceEditModal item={editing} cases={cases} warehouseEntries={warehouseEntries} clients={clients} close={()=>setEditing(null)} submit={item=>{updateInvoice(item);setEditing(null)}} simulateHolded={item=>{sendHolded(item);setEditing(null)}}/>}
  </>;
}
