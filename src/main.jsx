...
const draftInvoiceFromCase=(item,warehouseEntries=[])=>{
  ...
  const lines=[
    ...
    {id:'transport',item:'TRANSPORT FROM WAREHOUSE TO VESSEL',detail:cargo,price:suggestedTransportPrice(item,warehouseEntries),units:1,tax:'0%'}
  ];
...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const createCase=async form=>{
    ...
    const item=normalizeMerchandise({id,buque:vessel,imo:cleanImo(form.imo)||known.imo||'',mmsi:cleanMmsi(form.mmsi)||known.mmsi||'',cliente,puerto:form.puerto,eta:etaDate||'Por confirmar',portCall:{etaDate,etaTime,etbDate,etbTime,etdDate,etdTime,updatedAt:new Date().toISOString()},serviceType:form.serviceType||'vessel_delivery',estado:'Nuevo',prioridad:form.prioridad,conductor:'Sin asignar',servicios:[form.createReception&&'Recepción',form.createTransport&&'Transporte'].filter(Boolean),bultos:Number(form.bultos)||0,peso:'Por registrar',progreso:0,siguiente:'Revisar expediente y servicios programados',aduana:'Por revisar',autoTransportDisabled:false,manualVesselName:true,manualEditedAt:new Date().toISOString()});
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {tab==='facturacion'&&showFinance&&<Facturacion openCase={openCase} notify={notify} invoices={finance.invoices} cases={casesWithFinance} warehouseEntries={warehouseEntries} clients={finance.clients} updateInvoice={updateInvoice}/>}
...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],clients=[],updateInvoice}){
  const [editing,setEditing]=useState(null);
  const generatedDrafts=useRef(new Set());
  ...
  useEffect(()=>{billableCases.forEach(item=>{const existing=invoices.find(invoice=>invoice.expediente===item.id);const draft=draftInvoiceFromCase(item,warehouseEntries);const lineCount=(existing?.lines||[]).length;const existingIds=(existing?.lines||[]).map(line=>line.id||String(line.item||'').toLowerCase());const expectedIds=draft.lines.map(line=>line.id);const missingExpectedLine=expectedIds.some(id=>!existingIds.includes(id));const needsRefresh=!existing||lineCount<4||missingExpectedLine||/^SPL/i.test(String(existing.lines?.[0]?.item||''));if(!needsRefresh||generatedDrafts.current.has(item.id))return;generatedDrafts.current.add(item.id);updateInvoice({...draft,id:existing?.id||draft.id,cliente:existing?.cliente||draft.cliente,estado:existing?.estado||draft.estado,vencimiento:existing?.vencimiento||draft.vencimiento})})},[billableCases.map(item=>item.id).join('|'),invoices.map(item=>`${item.id}:${(item.lines||[]).length}:${item.lines?.map(line=>line.id||line.item).join(',')||''}`).join('|'),warehouseEntries.map(item=>`${item.ref}:${item.expediente}:${item.dias}:${item.estado}:${item.archivado}:${item.salida||''}`).join('|')]);
  const draftFromCase=item=>draftInvoiceFromCase(item,warehouseEntries);
  const createDraft=item=>setEditing(draftFromCase(item));
  const groupedStatus=['Borrador','Revisar','Listo para enviar','Enviado a Holded (demo)','Facturado','Cobrado'];
  const simulateHoldedSend=item=>{
    updateInvoice({...item,importe:item.importe||invoiceTotal(item),estado:'Enviado a Holded (demo)',holdedStatus:'Simulado',holdedAt:new Date().toISOString()});
    notify('Simulación Holded creada: cuando conectemos API, este botón creará la factura real.');
  };
  return <>
    ...
    <section className="billing-hero"><div><span>Importe pendiente de gestión</span><strong>{money(total)}</strong><small>{activeInvoices.length} borradores/documentos activos</small></div><div><span className="holded-mark">H</span><div><b>Holded en modo prueba</b><small>Revisa el borrador y simula el envío antes de conectar la API real</small></div></div><button className="button primary" onClick={()=>notify('Modo prueba: todavía no se crea nada real en Holded.')}><Download/> Preparado para API</button></section>
    <section className="panel"><SectionHeader title="Documentos de facturación" subtitle="Borradores internos editables con formato Holded"/><div className="responsive-table billing-table"><div className="table-head"><span>Documento / expediente</span><span>Cliente</span><span>Concepto</span><span>Importe</span><span>Estado</span><span/></div>{invoices.length?invoices.map(item=><div className="table-row" key={item.id}><span className="primary-cell"><span className="invoice-icon"><ReceiptText/></span><span><b>{item.id}</b><button onClick={()=>openCase(item.expediente)}>{item.expediente}</button>{item.holdedStatus&&<small>Holded: {item.holdedStatus}</small>}</span></span><span data-label="Cliente">{item.cliente}</span><span data-label="Concepto">{item.concepto}</span><strong data-label="Importe">{moneyExact(item.importe||invoiceTotal(item))}</strong><span data-label="Estado"><Badge>{item.estado}</Badge></span><span className="billing-row-actions"><button className="icon-button" aria-label={'Editar '+item.id} onClick={()=>setEditing(item)}><PencilLine/></button><button className="button secondary compact" onClick={()=>simulateHoldedSend(item)}>Probar Holded</button></span></div>):<Empty text="Todavía no hay borradores. Cuando un expediente esté listo para facturar se creará automáticamente."/>}</div></section>
    {editing&&<InvoiceEditModal item={editing} cases={cases} warehouseEntries={warehouseEntries} clients={clients} close={()=>setEditing(null)} submit={item=>{updateInvoice(item);setEditing(null)}} simulateHolded={item=>{simulateHoldedSend(item);setEditing(null)}}/>}
  </>;
}
...
function InvoiceEditModal({item,cases=[],warehouseEntries=[],clients=[],close,submit,simulateHolded}){
  ...
  const template=relatedCase?draftInvoiceFromCase(relatedCase,warehouseEntries):null;
  ...
  const shouldUseTemplate=Boolean(template)&&(storedLines.length<4||missingTemplateLine||/^SPL/i.test(String(storedLines[0]?.item||'')));
  const initialLines=(shouldUseTemplate?template.lines:storedLines).map((line,index)=>currentCargo?{...line,item:index===0?currentHeader:line.item,detail:currentCargo}:line);
  ...
  const applyClient=value=>{
    ...
    if(relatedCase){
      const repriced=draftInvoiceFromCase({...relatedCase,cliente:clientName},warehouseEntries);
...
function NewCaseModal({clientOptions=[],vessels=[],team=[],close,submit}){
  ...
  const update=event=>{
    ...
    if(name==='serviceType'){
      setForm(current=>({...current,serviceType:value,createTransport:value==='storage_other'?false:current.createTransport,transportDelivery:value==='storage_other'?'ENTREGA / RECOGIDA POR CONFIRMAR':current.transportDelivery}));
      return;
    }
    if(name==='etaDate'){
