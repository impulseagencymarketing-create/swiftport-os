...
const withLimaniPodSuffix=(value,item)=>{
  const text=String(value||'').trim();
  if(!isLimaniCase(item)||!text)return text;
  return text.replace(/\s+POD\s*$/i,'')+' POD';
};
const ensureLimaniPodReferenceLines=(lines,item)=>{
  ...
  return invoiceLinesOf(lines).map(line=>{
    ...
    if(!isLimaniCase(item)||!isReference)return line;
    referenceUpdated=true;
    return {...line,item:withLimaniPodSuffix(line.item,item)};
  });
};
const invoiceHeaderTitle=(item,transports=[],calendarEvents=[])=>withLimaniPodSuffix([item.id,item.buque,invoiceScheduleLabel(item,transports,calendarEvents),item.puerto].filter(Boolean).join(' ').toUpperCase(),item);
  ...
  const updateTransport=updated=>{
    ...
    const nextCases=cases.map(item=>{
      ...
      if(cancelled){
        ...
        return normalizeMerchandise({...item,autoTransportDisabled:!otherActive,conductor:otherActive?item.conductor:'Sin asignar',siguiente:'Revisar cancelación, gastos y facturación',gastos:expenses,billingAdjustments:adjustments,timelineCustom:timeline});
      }
...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],transports=[],calendarEvents=[],clients=[],updateInvoice,syncInvoices,csrfToken,currentUser}){
  ...
  const sendHolded=item=>{
    ...
    const preparedLines=ensureLimaniPodReferenceLines(itemLines.length?itemLines:templateLines,billingContext);
    const prepared={...item,concepto:withLimaniPodSuffix(item.concepto,billingContext),clientProfile:holdedClientProfile(item.cliente),importe:item.importe||invoiceTotal(item),coste:related?caseExpenseTotal(related):Number(item.coste)||0,margen:related?invoiceRevenue(item)-caseExpenseTotal(related):Number(item.margen)||0,lines:preparedLines};
