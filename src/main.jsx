...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const createCase=form=>{
    ...
    const item=normalizeMerchandise({id,buque:form.buque.toUpperCase(),cliente:form.cliente,puerto:form.puerto,eta:etaDate||'Por confirmar',portCall:{etaDate,etaTime,etbDate:'',etbTime:'',etdDate:'',etdTime:'',updatedAt:new Date().toISOString()},estado:'Nuevo',prioridad:form.prioridad,conductor:'Sin asignar',servicios:[form.createReception&&'Recepción',form.createTransport&&'Transporte'].filter(Boolean),bultos:Number(form.bultos)||0,peso:'Por registrar',progreso:0,siguiente:'Revisar expediente y servicios programados',aduana:'Por revisar'});
    ...
    const transport=form.createTransport?{id:transportId,expediente:id,origen:form.transportPickup,destino:form.transportDelivery,ruta:route,fecha:form.transportDate,inicio:form.transportStart,fin:form.transportEnd,hora:formatSchedule(form.transportDate,form.transportStart,form.transportEnd),conductor:'Sin asignar',proveedorId:'',vehiculo:'Por asignar',estado:'Sin asignar'}:null;
    const transportEvent=transport?{id:`EV-${stamp}-T`,titulo:route,origen:transport.origen,destino:transport.destino,tipoServicio:'Transporte',fecha:transport.fecha,inicio:transport.inicio,fin:transport.fin,asignado:'Sin asignar',expediente:id,transporte:transportId,proveedorId:'',color:'gray'}:null;
  ...
  const saveCalendarEvent=event=>{
    ...
    if(colored.tipoServicio==='Transporte'){
      ...
      if(!colored.transporte){
        ...
        nextTransports=[{id:transportId,expediente:colored.expediente,...parts,ruta:route,fecha:colored.fecha,inicio:colored.inicio,fin:colored.fin,hora:formatSchedule(colored.fecha,colored.inicio,colored.fin),conductor:colored.asignado,proveedorId:colored.proveedorId||'',vehiculo:'Por asignar',estado:colored.asignado==='Sin asignar'?'Sin asignar':'Asignado'},...transports];
      }else{
        nextTransports=transports.map(item=>item.id===colored.transporte?{...item,...parts,expediente:colored.expediente||item.expediente,ruta:route,conductor:colored.asignado,proveedorId:colored.proveedorId||item.proveedorId||'',fecha:colored.fecha,inicio:colored.inicio,fin:colored.fin,hora:formatSchedule(colored.fecha,colored.inicio,colored.fin),estado:colored.asignado==='Sin asignar'?'Sin asignar':item.estado==='Sin asignar'?'Asignado':item.estado}:item);
      }
    }
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {tab==='calendario'&&<>{!driverOnly&&<DriverLegend team={operationalTeam}/>} {!driverOnly&&(hasRole(effectiveRoles,'operations')||hasRole(effectiveRoles,'admin'))&&<section className="calendar-clean-bar"><div><b>¿Servicios duplicados o incorrectos?</b><small>Limpia el calendario y vuelve a crearlo desde las fechas reales de los expedientes.</small></div><button className="button warning" onClick={rebuildCalendarServices}><RefreshCw/> Recrear calendario</button></section>}<Calendario events={calendarEvents} team={operationalTeam} cases={cases} transports={transports} providers={providers} warehouseEntries={warehouseEntries} saveEvent={saveCalendarEvent} completeCaseStep={completeCaseStep} undoCaseStep={undoCaseStep} openCase={openCase} currentUser={visibleUser} csrfToken={auth.csrfToken} reloadOperational={loadOperational} notify={notify} rebuildCalendarServices={rebuildCalendarServices}/></>}
...
function Calendario({events,team,cases,transports,providers,warehouseEntries,saveEvent,completeCaseStep,undoCaseStep,openCase,currentUser,csrfToken,reloadOperational,notify,rebuildCalendarServices}){
...
function Expedientes({cases,selected,select,search,setSearch,completeCaseStep,notify,showFinance,updateCase,deleteCase,clientOptions,warehouseEntries,transports,calendarEvents,team,providers,saveEvent,csrfToken,reloadOperational,currentUser}){
...
function CaseServicesPanel({item,events,cases,transports,team,providers,saveEvent}){
  const [editing,setEditing]=useState(null);
  const scheduled=(events||[]).filter(event=>event.expediente===item.id).sort((a,b)=>(String(a.fecha)+String(a.inicio)).localeCompare(String(b.fecha)+String(b.inicio)));
  const addService=type=>{
    ...
    const route=type==='Recepción'?'Recepción en almacén':`${origen} → ${destino}`;
  ...
  return <><SectionHeader title="Trabajos programados" subtitle={`${scheduled.length} servicios sincronizados con Calendario y Transportes`}/><div className="case-service-actions"><button className="button secondary" onClick={()=>addService('Recepción')}><Plus/> Añadir recepción</button><button className="button primary" onClick={()=>addService('Transporte')}><Truck/> Añadir transporte</button></div>{scheduled.length?<div className="case-service-list">{scheduled.map(event=><button key={event.id} onClick={()=>setEditing(event)}><span className={'case-service-icon '+(event.tipoServicio==='Transporte'?'transport':'reception')}>{event.tipoServicio==='Transporte'?<Truck/>:<WarehouseIcon/>}</span><span><b>{event.tipoServicio} · {event.inicio}–{event.fin}</b><small>{event.fecha} · {event.titulo||item.puerto}</small></span><span><b>{event.asignado||'Sin asignar'}</b><small>{event.transporte?'TRANSPORTE Y CALENDARIO':'CALENDARIO'}</small></span><PencilLine/></button>)}</div>:<div className="case-services-empty"><CalendarDays/><span><b>Este expediente todavía no tiene trabajos</b><small>Añade la recepción de mercancía y el transporte para que aparezcan a los conductores.</small></span></div>}{editing&&<CalendarEventModal item={editing} team={team} cases={cases} transports={transports} providers={providers} close={()=>setEditing(null)} submit={event=>{saveEvent(event);setEditing(null)}} openCase={()=>setEditing(null)}/>}</>;
}
...
function NewCaseModal({clientOptions=[],close,submit}){
  ...
  const update=event=>{
    ...
    if(name==='buque'){setForm(current=>({...current,buque:value,transportDelivery:`BUQUE ${value.toUpperCase()} · ${current.puerto}`}));return}
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal new-case-modal" role="dialog" aria-modal="true" aria-labelledby="new-case-title"><div className="modal-head"><div><span className="overline">Nuevo registro</span><h2 id="new-case-title">Crear expediente y trabajos</h2><p>La recepción y el transporte quedarán visibles inmediatamente en el calendario.</p></div><button className="icon-button" aria-label="Cerrar" onClick={close}><X/></button></div><form onSubmit={event=>{event.preventDefault();submit(form)}}><label className="field wide"><span>Buque *</span><input name="buque" value={form.buque} onChange={update} placeholder="Ej. Baltic Horizon" required autoFocus/></label><label className="field"><span>Cliente</span><select name="cliente" value={form.cliente} onChange={update}>{(clientOptions.length?clientOptions:clientNames).map(name=><option key={name}>{name}</option>)}</select></label><label className="field"><span>Puerto</span><select name="puerto" value={form.puerto} onChange={update}>{['Barcelona','Algeciras','Tarragona','Valencia','Bilbao'].map(value=><option key={value}>{value}</option>)}</select></label><label className="field"><span>ETA del buque</span><input name="eta" type="datetime-local" value={form.eta} onChange={update}/></label><label className="field"><span>Prioridad</span><select name="prioridad" value={form.prioridad} onChange={update}>{['Baja','Media','Alta','Urgente'].map(value=><option key={value}>{value}</option>)}</select></label><label className="field"><span>N.º de bultos</span><input name="bultos" type="number" min="0" value={form.bultos} onChange={update}/></label><fieldset className="case-service-fieldset wide"><label className="service-check"><input name="createReception" type="checkbox" checked={form.createReception} onChange={update}/><WarehouseIcon/><span><b>PROGRAMAR RECEPCIÓN</b><small>Entrada o recogida de mercancía</small></span></label>{form.createReception&&<div className="case-service-fields"><label className="field"><span>Fecha *</span><input name="receptionDate" type="date" value={form.receptionDate} onChange={update} required/></label><label className="field"><span>Inicio *</span><input name="receptionStart" type="time" value={form.receptionStart} onChange={update} required/></label><label className="field"><span>Fin *</span><input name="receptionEnd" type="time" value={form.receptionEnd} onChange={update} required/></label><label className="field wide"><span>Lugar de recepción / recogida</span><input name="receptionLocation" value={form.receptionLocation} onChange={update} required/></label></div>}</fieldset><fieldset className="case-service-fieldset wide"><label className="service-check"><input name="createTransport" type="checkbox" checked={form.createTransport} onChange={update}/><Truck/><span><b>PROGRAMAR TRANSPORTE</b><small>Recogida, entrega, fecha y horas</small></span></label>{form.createTransport&&<div className="case-service-fields"><label className="field"><span>Fecha *</span><input name="transportDate" type="date" value={form.transportDate} onChange={update} required/></label><label className="field"><span>Inicio *</span><input name="transportStart" type="time" value={form.transportStart} onChange={update} required/></label><label className="field"><span>Fin *</span><input name="transportEnd" type="time" value={form.transportEnd} onChange={update} required/></label><label className="field"><span>Recogida</span><input name="transportPickup" value={form.transportPickup} onChange={update} required/></label><label className="field"><span>Entrega</span><input name="transportDelivery" value={form.transportDelivery} onChange={update} required/></label></div>}</fieldset><div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Crear expediente y calendario</button></div></form></section></div>;
}
