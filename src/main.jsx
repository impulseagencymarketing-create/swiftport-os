...
function Correos({csrfToken,notify,openCase,reloadOperational,canRebuild}){
  ...
  const rebuild=async()=>{
    if(!window.confirm('Se eliminarán todos los expedientes abiertos y sus tareas. Los expedientes completados se conservarán. ¿Iniciar la reconstrucción?'))return;
    setRebuilding(true);setError('');setRebuildProgress('Preparando reconstrucción…');
    try{
      const reset=await api('/api/admin/rebuild.php',{method:'POST',headers:{'X-CSRF-Token':csrfToken},body:JSON.stringify({action:'reset'})});
      let remaining=Number(reset.pendingEmails||0),interpreted=0,created=0,ignored=0;
      while(remaining>0){
        setRebuildProgress(`Interpretando histórico guardado · ${remaining} pendientes`);
        const batch=await api('/api/admin/rebuild.php',{method:'POST',headers:{'X-CSRF-Token':csrfToken},body:JSON.stringify({action:'process_batch'})});
      ...
      for(let round=0;round<60;round++){
        setRebuildProgress(`Buscando correos antiguos · lote ${round+1}`);
        const result=await api('/api/mail/process.php',{method:'POST',headers:{'X-CSRF-Token':csrfToken},body:'{}'});
        created+=Number(result.summary?.processed||0);ignored+=Number(result.summary?.ignored||0);
        if(Number(result.summary?.scanned||0)===0)break;
      }
      setRebuildProgress('');
      notify(`${reset.removedOpenCases} expedientes abiertos retirados · ${created} trabajos reconstruidos · ${ignored} correos no operativos`);
