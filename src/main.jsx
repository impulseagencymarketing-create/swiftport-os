...
const normalizeClientProfile=client=>{
  ...
  const storage=client?.storage&&!/sin tarifa|pendiente/i.test(String(client.storage))?client.storage:(isLimani?'0-35 kg gratis - 36-100 kg 5€/día - 101-500 kg 10€/día - 500+ kg 15€/día':isAls?'3 días gratis · 36-100 kg 2,50€/día · 101-500 kg 3,50€/día · 500+ kg 7,50€/día':isUme?'Warehousing 0,715€/kg/día · min. 9,90€ · storage min. 99€':'Pendiente de tarifa');
...
function Calendario({events,team,cases,transports,providers,warehouseEntries,saveEvent,deleteEvent,completeCaseStep,undoCaseStep,openCase,currentUser,csrfToken,reloadOperational,notify}){
  ...
  return <>
    <section className="calendar-toolbar">
      <div className="calendar-nav"><button className="button tertiary" aria-label="Periodo anterior" onClick={()=>movePeriod(-1)}><ChevronLeft size={18}/></button><button className="button tertiary" onClick={goToday}>Hoy</button><button className="button tertiary" aria-label="Periodo siguiente" onClick={()=>movePeriod(1)}><ChevronRight size={18}/></button><h2>{calendarTitle}</h2></div>
