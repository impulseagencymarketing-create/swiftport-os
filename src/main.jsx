...
const normalizeClientProfile=client=>{
  ...
  return {
    ...
    direccion:client?.direccion||(isLimani?'Calle Juan de la Cierva 11, Villalbilla (28810), Madrid, España':''),
    condicionesPago:client?.condicionesPago||client?.terms||'30 días',
    moneda:client?.moneda||'EUR',
    tarifaActiva:client?.tarifaActiva||(isLimani?'LIMANI Barcelona 2026':'Sin tarifa automática'),
    notas:client?.notas||'',
    expedientes:Number(client?.expedientes??client?.activeCases??0),
    recepcion:client?.recepcion||(isLimani?'0-35 kg 15€  -  35-250 kg 60€  -  251-500 kg 130€  -  501-2500 kg 245€':'Pendiente de tarifa'),
    storage:client?.storage||(isLimani?'0-35 kg gratis  -  36-100 kg 5€/día  -  101-500 kg 10€/día  -  500+ kg 15€/día':'Pendiente de tarifa'),
    transporte:client?.transporte||(isLimani?'Warehouse→Vessel: 45€ / 95€ / 250€ / 350€ por peso':'Pendiente de tarifa'),
    recargo:client?.recargo||(isLimani?'+30% overtime / holidays':'Pendiente'),
...
const draftInvoiceFromCase=(item,warehouseEntries=[],transports=[],calendarEvents=[])=>{
  ...
  if(isSurveyService(item)){
    ...
    return {...invoice,importe:invoiceTotal(invoice)};
  }
  ...
  const lines=[
    ...
    {id:'transport',item:'TRANSPORT FROM WAREHOUSE TO VESSEL',detail:transportDetail,price:suggestedTransportPrice(item,warehouseEntries),units:transportUnits,tax:'0%'}
  ];
...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const target=cases.find(item=>item.id===id);
    ...
    if(stepKey==='delivery'&&!deliveryPhotos.length&&!surveyService){notify('Añade al menos una foto de la mercancía entregada');return}
    if(stepKey==='delivery'&&surveyService&&!deliveryPhotos.length&&!podFiles.length){notify('Añade una foto, documento o informe del survey realizado');return}
    if(stepKey==='delivery'&&!storageOnly&&!surveyService&&!podFiles.length){notify('Escanea o adjunta el POD firmado');return}
    ...
    notify(ready?(surveyService?'Survey confirmado: expediente listo para facturar':storageOnly?'Salida confirmada: expediente listo para facturar':'POD registrado: expediente listo para facturar'):expected.title+' registrado');
  };
...
const PORT_TONE_MAP={BARCELONA:'blue',TARRAGONA:'orange',SAGUNTO:'teal',VALENCIA:'purple',ALGECIRAS:'red',BILBAO:'green',VINAROS:'pink',VINARÒS:'pink',VINARÓS:'pink',CASTELLON:'teal',CASTELLÓN:'teal',MARIN:'purple',MARÍN:'purple','A CORUÑA':'green','A CORUNA':'green',HUELVA:'red',SEVILLA:'pink',CARTAGENA:'orange',ALICANTE:'blue'};
...
function DraggableCalendarEvent({event,cases,team,saveEvent,setEditing,openCase,canDeleteEvent,deleteEvent,startPointerDrag,draggingId,suppressClick}){
  ...
  return <article onPointerDown={pointer=>startPointerDrag(pointer,clean)} className={`calendar-event ${event.color} ${event._columns>1?'is-overlap':''} ${draggingId===event.id?'dragging':''}`} style={calendarEventStyle(event)} title={`${event.inicio}–${event.fin}  -  ${event.titulo||event.id}`}>
    <button className="calendar-event-open" title={clean.expediente?'Abrir expediente':'Editar servicio'} onClick={click=>{if(suppressClick?.()){click.preventDefault();return}clean.expediente?openCase(clean.expediente):setEditing(clean)}}><CalendarEventContent event={event} cases={cases}/></button>
...
function PodDocuments({item,notify,onDelete}){
  ...
  return <div className="document-box"><h3>POD de entrega <small>{pods.length?`${pods.length} PDF${pods.length===1?'':'S'}`:'PENDIENTE'}</small></h3>{pods.length?pods.map((file,index)=><div className="attachment-row compact" key={file.id||file.url||index}><a className="document-link" href={file.url} target="_blank" rel="noreferrer"><FileText/><span><b>POD {index+1}</b><small>{file.name}</small></span><ExternalLink/></a>{onDelete&&<button type="button" className="icon-button danger attachment-delete" aria-label="Eliminar POD" onClick={()=>onDelete(file,'pod')}><Trash2/></button>}</div>):<button onClick={()=>notify('POD todavía pendiente')}><Camera/><span><b>POD / fotografías</b><small>Pendiente de entrega</small></span><ChevronRight/></button>}{deliveryPhotos.length>0&&<div className="delivery-photo-list"><b>Fotos de entrega</b>{deliveryPhotos.map((file,index)=><div className="attachment-row compact" key={file.id||file.url||index}><a className="document-link" href={file.url} target="_blank" rel="noreferrer"><Camera/><span><b>Foto entrega {index+1}</b><small>{file.name}</small></span><ExternalLink/></a>{onDelete&&<button type="button" className="icon-button danger attachment-delete" aria-label="Eliminar foto de entrega" onClick={()=>onDelete(file,'delivery-photo')}><Trash2/></button>}</div>)}</div>}</div>;
}
function DriverTaskModal({event,item,transport,warehouseEntries,currentUser,csrfToken,reloadOperational,notify,close,claim,submit,undo}){
  ...
  const evidenceReady=step?.key==='cargo'?cargoPhotos.length>0||surveyService:step?.key==='delivery'?(surveyService?(deliveryPhotos.length>0||podFiles.length>0):warehouseReviewed&&deliveryPhotos.length>0&&(storageOnly||podFiles.length>0)):true;
    ...
    <section className="modal driver-task-modal">
      ...
      <div className="driver-task-body">
        ...
        {step?<>
          ...
          <button className="button primary full driver-confirm" disabled={uploading||!evidenceReady} onClick={()=>submit(step.key,note,evidenceFiles)}><CheckCircle2/> {!evidenceReady?(step.key==='cargo'?'Añade una foto para confirmar':surveyService?'Añade evidencia o informe del survey':storageOnly?'Revisa almacén y añade evidencia de salida':'Revisa almacén, foto de entrega y POD'):`Confirmar: ${step.title}`}</button>
...
function OperationStepModal({item,warehouseEntries,transports,csrfToken,currentUser,close,submit}){
  ...
  const needsEvidence=['cargo','delivery'].includes(step.key);
  const evidenceReady=step.key==='cargo'?cargoPhotos.length>0||selectedWarehouseEntries.length>0||surveyService:step.key==='delivery'?(surveyService?(deliveryPhotos.length>0||podFiles.length>0):warehouseReviewed&&deliveryPhotos.length>0&&(storageOnly||podFiles.length>0)):true;
...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],transports=[],calendarEvents=[],clients=[],updateInvoice,syncInvoices,csrfToken}){
  const [editing,setEditing]=useState(null);
  ...
  const total=activeInvoices.reduce((sum,item)=>sum+Number(item.importe||invoiceTotal(item)||0),0);
  ...
  const sendHolded=item=>{
    ...
    const prepared={...item,clientProfile:holdedClientProfile(item.cliente),importe:item.importe||invoiceTotal(item),lines:(item.lines&&item.lines.length?item.lines:(related?draftInvoiceFromCase(related,warehouseEntries,transports,calendarEvents).lines:[]))};
  ...
  return <>
    ...
    {readyCases.length>0&&<section className="billing-ready-panel panel"><SectionHeader title="Listos para facturar" subtitle="Se crearán automáticamente con líneas habituales de operativa"/><div className="billing-ready-list">{readyCases.map(item=><article key={item.id} className="billing-ready-card"><span className="invoice-icon"><CheckCircle2/></span><div><b>{caseLabel(item)}</b><small>{item.cliente}  -  {item.puerto}  -  {isStorageOnly(item)?'salida/recogida verificada':'POD verificado'}</small><em>{invoiceCargoSummary(item,warehouseEntries)}  -  sugerido {moneyExact(suggestedTransportPrice(item,warehouseEntries))}</em></div><div className="billing-ready-actions"><button className="button tertiary" onClick={()=>openCase(item.id)}>Ver expediente</button><button className="button primary" onClick={()=>createDraft(item)}>Revisar borrador</button></div></article>)}</div></section>}
    <section className="billing-hero"><div><span>Importe pendiente de gestión</span><strong>{money(total)}</strong><small>{activeInvoices.length} borradores/documentos activos</small></div><div><span className="holded-mark">H</span><div><b>Holded conectado</b><small>El botón crea una proforma real. La factura final se hará cuando confirmemos el flujo.</small></div></div><button className="button primary" onClick={()=>notify('Holded listo: revisa un borrador y pulsa Enviar a Holded.')}><Download/> Proformas reales</button></section>
    <section className="panel"><SectionHeader title="Documentos de facturación" subtitle="Borradores internos editables con formato Holded"/><div className="responsive-table billing-table"><div className="table-head"><span>Documento / expediente</span><span>Cliente</span><span>Concepto</span><span>Importe</span><span>Estado</span><span/></div>{invoices.length?invoices.map(item=><div className="table-row" key={item.id}><span className="primary-cell"><span className="invoice-icon"><ReceiptText/></span><span><b>{item.id}</b><button onClick={()=>openCase(item.expediente)}>{item.expediente}</button>{item.holdedStatus&&<small>Holded: {item.holdedStatus}{item.holdedNumber?`  -  ${item.holdedNumber}`:''}</small>}</span></span><span data-label="Cliente">{item.cliente}</span><span data-label="Concepto">{item.concepto}</span><strong data-label="Importe">{moneyExact(item.importe||invoiceTotal(item))}</strong><span data-label="Estado"><Badge>{item.estado}</Badge></span><span className="billing-row-actions"><button className="icon-button" aria-label={'Editar '+item.id} onClick={()=>setEditing(item)}><PencilLine/></button><button className="button secondary compact" disabled={sendingHolded===item.id||item.estado==='Enviado a Holded'} onClick={()=>sendHolded(item)}>{sendingHolded===item.id?'Enviando…':'Enviar a Holded'}</button></span></div>):<Empty text="Todavía no hay borradores. Cuando un expediente esté listo para facturar se creará automáticamente."/>}</div></section>
