...
function ReceptionRecords({records}){
  ...
  return <section className="reception-records"><div className="reception-title"><Camera/><div><h3>Recepciones de mercancía</h3><p>Fotos y documentos escaneados a la llegada.</p></div></div>{records.map(record=><article className="reception-record" key={record.ref}><header><div><b>{formatReceptionDate(record.fecha)}</b><small>{record.ref} · ZONA {record.zona}</small></div><Badge>{(record.fotos||[]).length} FOTOS · {(record.documentos||[]).length} DOCS</Badge></header>{Boolean((record.fotos||[]).length)&&<div className="reception-photos">{record.fotos.map(file=><a href={file.url} target="_blank" rel="noreferrer" key={file.id} title={file.name}><img src={file.url} alt={`Mercancía recibida · ${file.name}`}/></a>)}</div>}{Boolean((record.documentos||[]).length)&&<div className="reception-documents">{record.documentos.map(file=><a href={file.url} target="_blank" rel="noreferrer" key={file.id}><FileText/><span><b>{documentLabel(file.name)}</b><small>{file.name}</small></span><ExternalLink/></a>)}</div>}</article>)}</section>;
}
...
function WarehouseEntryModal({cases,close,submit,csrfToken}){
  ...
  const save=async event=>{
    ...
    try{
      const uploadedPhotos=await Promise.all(photos.map(file=>uploadAttachment(file,'photo',csrfToken)));
