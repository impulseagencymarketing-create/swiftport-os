...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  useEffect(()=>{
    ...
    const normalizedCalendar=calendarEvents.map(event=>({...event,color:driverTone(event.asignado,team)}));
...
function CalendarEventModal({item,team,cases,transports,close,submit,openCase}){
  const [form,setForm]=useState({...item});const update=event=>{if(event.target.name==='transporte'){const linked=transports.find(entry=>entry.id===event.target.value);setForm({...form,transporte:event.target.value,expediente:linked?.expediente||form.expediente,titulo:form.titulo||linked?.ruta||'',asignado:linked?.conductor||form.asignado,fecha:linked?.fecha||form.fecha,inicio:linked?.inicio||form.inicio,fin:linked?.fin||form.fin});return}setForm({...form,[event.target.name]:event.target.value})};
