...
function Expedientes({cases,selected,select,search,setSearch,completeCaseStep,notify,showFinance,updateCase,deleteCase,deleteAttachment,reopenCaseStep,clientOptions,warehouseEntries,transports,calendarEvents,team,providers,vessels,saveEvent,csrfToken,reloadOperational,currentUser}){
  ...
  const attachCaseFiles=async(scope,files)=>{
    ...
    try{
      ...
      const timelineEntry={id:`ATTACH-${selected.id}-${scope}-${Date.now()}`,fecha:now.toLocaleDateString('es-ES'),hora:now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),titulo:title,detalle:`${uploaded.length} archivo(s) agregado(s) sin reabrir el expediente.`,actor:currentUser?.fullName||'Swiftport',archivos:uploaded,estado:'done'};
