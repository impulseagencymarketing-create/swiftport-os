...
const DEMO_USER={id:'local-demo',fullName:'Javier FernÃ¡ndez',email:'demo@swiftport.local',roles:['admin','operations','finance','driver']};
const DEMO_TEAM=[
...
const clientProfileKey=client=>{
  ...
  if(name.includes('marine logistics'))return 'marine logistics espaÃ±a';
...
const demoFinance=()=>({
  ...
  invoices:[{
    ...
    concepto:`Servicio logÃ­stico ${expedientesIniciales[0]?.buque||'BUQUE DEMO'}  -  ${expedientesIniciales[0]?.puerto||'PUERTO'}`,
    importe:420,
    estado:'Borrador',
    vencimiento:'2026-08-15',
    notas:'Borrador local para revisar el flujo de facturaciÃ³n.'
  }]
});
const NAV = [
  ...
  ['almacen','AlmacÃ©n',WarehouseIcon],
  ['buques','Buques',Ship],
  ['clientes','Clientes / Tarifas',UsersRound],
  ['facturacion','FacturaciÃ³n',ReceiptText],
  ['auditoria','AuditorÃ­a',ClipboardList],
  ['usuarios','Usuarios',ShieldCheck]
];
const TITLES = {
  dashboard:['Dashboard','Vista general de la operativa'],
  calendario:['Calendario','PlanificaciÃ³n semanal del equipo'],
  expedientes:['Expedientes','Seguimiento completo por buque'],
  almacen:['AlmacÃ©n','Entradas, ubicaciÃ³n y dÃ­as de storage'],
  buques:['Buques','Fichas, IMO/MMSI y seguimiento AIS'],
  transportes:['Transportes','PlanificaciÃ³n y asignaciÃ³n de conductores'],
  aduanas:['Aduanas','DocumentaciÃ³n y control de despachos'],
  correos:['Correos automÃ¡ticos','Servicios recibidos por info@ y operations@'],
  clientes:['Clientes y tarifas','Condiciones comerciales por cliente'],
  facturacion:['FacturaciÃ³n','Servicios listos para revisar y exportar'],
  auditoria:['AuditorÃ­a','Registro de movimientos por usuario'],
  usuarios:['Usuarios y permisos','Control de acceso al equipo']
};
const ROLE_LABELS={driver:'Transportista',operations:'Operaciones',finance:'Finanzas',admin:'AdministraciÃ³n'};
const SERVICE_TYPES=[
  {value:'vessel_delivery',label:'Entrega a buque',short:'Entrega a buque',hint:'Operativa normal: mercancÃ­a en almacÃ©n y transporte final al buque.'},
  {value:'storage_other',label:'Solo almacenaje / otra entrega',short:'Almacenaje / otra entrega',hint:'La mercancÃ­a queda en almacÃ©n o se entrega a otra empresa/direcciÃ³n.'},
  {value:'survey_samples',label:'Survey / muestras a bordo',short:'Survey / muestras',hint:'Servicio tÃ©cnico a bordo: ballast water samples, inspecciones o muestreos sin mercancÃ­a de almacÃ©n.'}
];
...
const cleanVesselDisplayName=value=>String(value||'').toUpperCase().replace(/^[#:\-\s]*(ENTREGA|DELIVERY|SERVICIO|SERVICE|RECOGIDA|PICK\s*UP|COLLECT(?:ION)?|SOLICITUD|REQUEST)\s+/,'').replace(/^(MV|M\/V|MT|M\/T|MY|M\/Y|MS|M\/S|SS|VSL|VESSEL|SHIP|BUQUE|BARCO)\s+/,'').split(/\s*(?:\/\/|\||;)\s*/)[0].replace(/\s+\b(EN|AT|IN)\s+(EL\s+)?(PUERTO(\s+DE)?|PORT(\s+OF)?|ALGECIRAS|SAGUNTO|TARRAGONA|BARCELONA|VINAR[OÃ“]S|VINAROS|MAR[IÃ]N|A\s+CORU[Ã‘N]A|VALENCIA|CASTELL[OÃ“]N|MARSEILLE|BILBAO|ALICANTE|M[ÃA]LAGA|ALMER[ÃI]A|HUELVA|C[ÃA]DIZ)\b.*$/u,'').replace(/\s+\b(A\s+LA\s+MAYOR\s+BREVEDAD|ASAP|URGENTE|URGENT|PROSPECTS?\s+UPDATE|UPDATE|ACTUALIZACI[Ã“O]N|PREVISI[Ã“O]N|PREVISIONES|ETA|ETB|ETD)\b.*$/u,'').replace(/\s+/g,' ').replace(/^[ .,_-]+|[ .,_-]+$/g,'');
...
const formatEtaDate=value=>{
  ...
  const match=text.match(/(\d{1,2})\s+([a-zÃ¡Ã©Ã­Ã³Ãº]{3})/i);
...
const disabledChangedDriverSchedules=(previous,current)=>{
  ...
  return Object.entries(current).flatMap(([id,next])=>{
    ...
    const oldTask=[before.date,before.start,before.end&&`â€“${before.end}`].filter(Boolean).join(' ')||'sin programar';
    const newTask=[next.date,next.start,next.end&&`â€“${next.end}`].filter(Boolean).join(' ')||'sin programar';
...
const PHOTO_TYPES=['VISTA GENERAL','ETIQUETA / TRACKING','ESTADO DE EMBALAJE','DAÃ‘OS / INCIDENCIA','PRECINTO'];
const SWIFTPORT_WAREHOUSE='ALMACÃ‰N SWIFTPORT  -  Bluespace El Prat';
const routeParts=transport=>{
  const legacy=String(transport?.ruta||'').split(/\s*(?:â†’|->|\bTO\b)\s*/i).map(value=>value.trim()).filter(Boolean);
  return {
...
const syncLinkedTransportWithCase=(transport,previousCase,nextCase)=>{
  ...
  return {...transport,origen,destino,ruta:`${origen} â†’ ${destino}`};
};
const OPERATION_STEPS=[
  {key:'review',title:'Expediente revisado',next:'Comprobar los datos del servicio y la mercancÃ­a',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
  {key:'cargo',title:'MercancÃ­a recibida o recogida',next:'Recibir en almacÃ©n o recoger en el punto indicado',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
  {key:'documents',title:'DocumentaciÃ³n del envÃ­o revisada',next:'Revisar y adjuntar la documentaciÃ³n antes de la entrega',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
  ...
  {key:'delivery',title:'Entrega confirmada con POD',next:'Entregar la mercancÃ­a y registrar fotos y POD firmado',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']}
];
const STORAGE_OPERATION_STEPS=[
  {key:'review',title:'Expediente revisado',next:'Comprobar cliente, referencia y mercancÃ­a esperada',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
  {key:'cargo',title:'MercancÃ­a recibida en almacÃ©n',next:'Registrar entrada, fotos, bultos, peso y ubicaciÃ³n',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
  {key:'documents',title:'DocumentaciÃ³n / instrucciones revisadas',next:'Adjuntar documentos o instrucciones de retirada/entrega',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
  {key:'assignment',title:'Salida o recogida coordinada',next:'Confirmar quiÃ©n recoge o dÃ³nde se entregarÃ¡ la mercancÃ­a',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
  {key:'delivery',title:'Salida de almacÃ©n confirmada',next:'Confirmar retirada/entrega con evidencia',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']}
];
const SURVEY_OPERATION_STEPS=[
  ...
  {key:'assignment',title:'Responsable asignado',next:'Asignar o confirmar quiÃ©n realizarÃ¡ el servicio',responsibility:'LIBRE PARA TODOS',roles:['admin','operations','driver','finance']},
...
const LIMANI_BARCELONA_RATES={
  ...
  storage:[[35,0],[100,5],[500,10],[Infinity,15]],
...
const umeAlgecirasLines=(item,warehouseEntries=[],transports=[],calendarEvents=[],options={})=>{
  ...
  if(hasRouteKeyword(routeRecords,/\bAGP\b|MALAGA|MÃLAGA|AIRPORT|AEROPUERTO/i))lines.push({id:'transport-agp',item:'TRANSPORT FROM AGP TO WAREHOUSE',detail:cargo,price:UME_ALGECIRAS_RATES.transportAgpXrjNextDay,units:1,tax:'0%'});
...
const supplierLineAmount=rawLine=>{
  const matches=[...String(rawLine||'').matchAll(/(\d{1,6}(?:[.,]\d{1,2}))\s*(?:EUR|â‚¬)?/gi)];
...
const supplierConceptLine=(rawLine,item,warehouseEntries=[],transports=[],calendarEvents=[],index=0,contextCargo='')=>{
  ...
  const withoutAmount=clean.replace(/\s+[-+]?\d+[.,]\d{1,2}\s*(EUR|â‚¬)?$/i,'').trim();
  ...
  if(/AGP|MALAGA|MÃLAGA|AIRPORT|AEROPUERTO/.test(label)&&/TRANSPORT|TRUCK|RECOGIDA|COLLECTION/.test(label))return make('transport-agp','TRANSPORT FROM AGP TO WAREHOUSE',cargo,UME_ALGECIRAS_RATES.transportAgpXrjNextDay);
...
const cleanSupplierPdfText=text=>{
  const allowed=/[A-Za-z0-9ÃÃ‰ÃÃ“ÃšÃœÃ‘Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±â‚¬$.,:;()\/+\-\s]/g;
  ...
  const lines=String(text||'').split(/\r?\n/).map(line=>{
    ...
    if(!/[A-Za-zÃÃ‰ÃÃ“ÃšÃœÃ‘Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±]/.test(cleaned))return '';
...
const cleanSupplierPdfTextForInvoices=text=>{
  ...
  const lines=String(text||'').split(/\r?\n/).map(line=>{
    ...
    const cleaned=original.replace(/[^\p{L}\p{N}â‚¬$.,:;()\/+\-\s]/gu,' ').replace(/\s+/g,' ').trim();
    if(cleaned.length<3||cleaned.length>220)return '';
...
const extractSupplierPdfText=async file=>{
  try{
    ...
    // Si pdf.js no puede leer el PDF, usamos el extractor bÃ¡sico como respaldo.
  }
  try{
    const buffer=await file.arrayBuffer();
...
const invoiceTransportServices=(item,transports=[],calendarEvents=[])=>{
  ...
  if(!caseId)return[];
  const events=(calendarEvents||[]).filter(event=>event.expediente===caseId&&(String(event?.tipoServicio||'').toLowerCase().startsWith('transporte')||Boolean(event?.transporte)));
...
async function api(path,options={}){
  ...
  if(!response.ok) throw Object.assign(new Error(body.error||'No se pudo completar la operaciÃ³n.'),{status:response.status,body});
  return body;
}
...
const imageFromFile=file=>new Promise((resolve,reject)=>{
  ...
  image.onerror=()=>{URL.revokeObjectURL(url);reject(new Error('No se pudo leer la fotografÃ­a del POD.'))};
  image.src=url;
});
...
const documentLabel=name=>{
  ...
  return 'DOCUMENTO DE RECEPCIÃ“N';
};
...
function AuthRoot(){
  ...
  const authenticated=result=>{if(!result?.user) throw new Error('Respuesta de acceso invÃ¡lida.');setSession(result);setSetupRequired(false);setError('')};
  ...
  if(loading) return <AuthShell><div className="auth-loading"><span className="auth-spinner"/><b>Preparando Swiftport OSâ€¦</b></div></AuthShell>;
...
function LoginForm({onSuccess,globalError,localDesign=false,onDesignMode}){
  ...
  return <section className="auth-card"><span className="auth-icon"><LockKeyhole/></span><h1>Iniciar sesiÃ³n</h1><p>Accede con tu cuenta de Swiftport.</p>{(error||globalError)&&<div className="form-error"><CircleAlert/>{error||globalError}</div>}<form onSubmit={submit}><label className="field"><span>Email</span><input type="email" autoComplete="username" value={form.email} onChange={event=>setForm({...form,email:event.target.value})} required autoFocus/></label><label className="field"><span>ContraseÃ±a</span><input type="password" autoComplete="current-password" value={form.password} onChange={event=>setForm({...form,password:event.target.value})} required/></label><button className="button primary full" disabled={busy}>{busy?'Comprobandoâ€¦':'Entrar'}</button></form></section>;
}
function SetupForm({onSuccess,globalError}){
  ...
  return <section className="auth-card setup-card"><span className="auth-icon"><ShieldCheck/></span><h1>Crear administrador</h1><p>Solo aparece una vez. Crea la primera cuenta con control total.</p>{(error||globalError)&&<div className="form-error"><CircleAlert/>{error||globalError}</div>}<form onSubmit={submit}><label className="field"><span>Nombre completo</span><input name="fullName" value={form.fullName} onChange={update} required autoFocus/></label><label className="field"><span>Email</span><input name="email" type="email" autoComplete="username" value={form.email} onChange={update} required/></label><label className="field"><span>ContraseÃ±a (mÃ­nimo 4 caracteres)</span><input name="password" type="password" autoComplete="new-password" minLength="4" value={form.password} onChange={update} required/></label><label className="field"><span>CÃ³digo inicial</span><input name="setupToken" type="password" autoComplete="off" value={form.setupToken} onChange={update} required/></label><button className="button primary full" disabled={busy}>{busy?'Creando cuentaâ€¦':'Crear administrador'}</button></form></section>;
}
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  useEffect(()=>{
    ...
    fresh.forEach(alert=>{sent[alert.key]=now.toISOString()});
    try{localStorage.setItem(storageKey,JSON.stringify(sent))}catch{}
    const entries=fresh.map(alert=>({id:`${alert.key}-${now.getTime()}`,alertKey:alert.key,type:'billing',title:wasAlreadySent[alert.key]?'Recordatorio facturaciÃ³n':'Listo para facturar',message:alert.message,createdAt:now.toISOString(),caseId:alert.case?.id||'',vessel:alert.case?.buque||'',rule:alert.rule?.label||''}));
    setNotificationLog(previous=>{const next=[...entries,...previous].slice(0,100);try{localStorage.setItem(`swiftport-notification-log-${user.id}`,JSON.stringify(next))}catch{}return next});
    setDeliveryPopup(fresh[0]);
    if(localStorage.getItem('swiftport-alert-sound')!=='0')playAlertSound().catch(()=>{});
    notify(fresh.length===1?fresh[0].message:`${fresh.length} expedientes listos para facturar.`);
    if(localStorage.getItem('swiftport-device-alerts')==='1'){
      fresh.forEach(alert=>showDeviceNotification('Swiftport facturaciÃ³n',alert.message,alert.key).catch(()=>{}));
    }
  ...
  useEffect(()=>{
    ...
    notify(`${missing.length} ${missing.length===1?'recepciÃ³n sincronizada':'recepciones sincronizadas'} con AlmacÃ©n`);
  },[operationalLoaded]);
  useEffect(()=>{
  ...
  const createCase=async form=>{
    ...
    const item=normalizeMerchandise({id,buque:vessel,imo:cleanImo(form.imo)||known.imo||'',mmsi:cleanMmsi(form.mmsi)||known.mmsi||'',cliente,puerto:form.puerto,eta:etaDate||'Por confirmar',portCall:{etaDate,etaTime,etbDate,etbTime,etdDate,etdTime,updatedAt:new Date().toISOString()},serviceType:form.serviceType||'vessel_delivery',estado:'Nuevo',prioridad:form.prioridad,conductor:'Sin asignar',servicios:[surveyService&&'Survey / muestras a bordo',!surveyService&&form.createReception&&'RecepciÃ³n',form.createTransport&&(surveyService?'Survey':'Transporte')].filter(Boolean),bultos:surveyService?0:Number(form.bultos)||0,peso:surveyService?'No aplica':'Por registrar',progreso:0,siguiente:surveyService?'Revisar servicio y agendar surveyor':'Revisar expediente y servicios programados',aduana:surveyService?'No aplica':'Por revisar',autoTransportDisabled:false,manualVesselName:true,manualEditedAt:new Date().toISOString()});
    const stamp=Date.now();
    const receptionEvent=form.createReception&&form.receptionDate?{id:`EV-${stamp}-R`,titulo:form.receptionLocation||'RecepciÃ³n en almacÃ©n',tipoServicio:'RecepciÃ³n',fecha:form.receptionDate,inicio:form.receptionStart,fin:form.receptionEnd,asignado:'Sin asignar',expediente:id,transporte:'',color:'gray'}:null;
    ...
    const route=[form.transportPickup,form.transportDelivery].filter(Boolean).join(' â†’ ')||(surveyService?`SURVEYOR â†’ BUQUE ${vessel}  -  ${form.puerto}`:`ALMACÃ‰N â†’ ${form.puerto}`);
  ...
  const updateCase=updated=>{
    ...
    notify(slot.date?'Expediente, buque, almacÃ©n y calendario actualizados':(activeEntries.length===1?'Expediente, buque y almacÃ©n actualizados':'Expediente y buque actualizados'));
  };
  const deleteCaseAttachment=(caseId,scope,file,receptionRef='')=>{
    ...
    if(!window.confirm(`Â¿Eliminar ${label} del expediente?`))return;
  ...
  const updateWarehouseEntry=updated=>{
    ...
    const normalized={...updated,buque:relatedCase?.buque||updated.buque||'MercancÃ­a sin identificar',expediente:updated.expediente||'',sinExpediente:!updated.expediente};
    ...
    if(affectedCaseIds.length){
      ...
      nextCases=cases.map(item=>affectedCaseIds.includes(item.id)?{
        ...
        timelineCustom:normalized.expediente===item.id&&previous?.expediente!==updated.expediente?[{
          ...
          titulo:'MercancÃ­a vinculada desde almacÃ©n',
    ...
    notify(relatedCase&&previous?.expediente!==updated.expediente?'MercancÃ­a vinculada al expediente':'Entrada de almacÃ©n actualizada');
  };
  const deleteWarehouseEntry=entry=>{
    if(!window.confirm(`Â¿Eliminar la entrada ${entry.ref} de almacÃ©n?\n\nSe quitarÃ¡ del stock y del expediente vinculado, pero no tocarÃ¡ documentos subidos en otros pasos.`))return;
    ...
    setWarehouseEntries(nextWarehouse);setCases(nextCases);saveOperational(nextCases,transports,nextWarehouse,customs,calendarEvents,providers,vessels,deletedVesselKeys,{action:'warehouse.delete',details:{warehouseRef:entry.ref,caseRef:entry.expediente||'',vessel:entry.buque,packages:entry.bultos,weight:entry.peso}});notify('Entrada de almacÃ©n eliminada');
  };
  const updateCustom=updated=>{const next=customs.map(item=>item.id===updated.id?updated:item);setCustoms(next);saveOperational(cases,transports,warehouseEntries,next);notify('TrÃ¡mite aduanero actualizado')};
  const deleteCalendarService=event=>{
    if(!event?.id)return;
    const related=cases.find(item=>item.id===event.expediente);
    if(!window.confirm(`Â¿Eliminar este servicio del calendario${related?` de ${related.buque}`:''}?\n\nSe quitarÃ¡ solo esta tarjeta y su transporte relacionado.`))return;
    ...
    notify(hasOtherTransport?'Servicio eliminado del calendario':'Servicio eliminado; no se recrearÃ¡ automÃ¡ticamente hasta crear otro transporte');
  };
  const saveCalendarEvent=event=>{
    let colored={...event,tipoServicio:event.tipoServicio||(event.transporte?'Transporte':'RecepciÃ³n'),color:calendarTone(event,cases)};
    if(!isTransportCalendarEvent(colored)){notify('El calendario solo muestra transportes. Las recepciones se registran desde AlmacÃ©n/Expediente.');return}
    ...
    if(colored.tipoServicio==='Transporte'||String(colored.tipoServicio||'').toLowerCase().startsWith('survey')){
      ...
      const route=`${parts.origen} â†’ ${parts.destino}`;
  ...
  const deleteVessel=vessel=>{
    ...
    if(!name)return;
    if(!window.confirm(`Â¿Borrar la ficha del buque ${name}?\n\nNo se borrarÃ¡n expedientes, almacÃ©n ni transportes.`))return;
  ...
  const target=cases.find(item=>item.id===id);
    ...
    if(!expected||expected.key!==stepKey){notify('Completa primero el paso anterior');return false}
    ...
    if(stepKey==='cargo'&&!cargoEvidence.length&&!cargoHasWarehouse){notify('AÃ±ade una foto o selecciona una mercancÃ­a existente en almacÃ©n');return false}
    if(stepKey==='delivery'&&!deliveryPhotos.length&&!surveyService){notify('AÃ±ade al menos una foto de la mercancÃ­a entregada');return false}
    if(stepKey==='delivery'&&surveyService&&!deliveryPhotos.length&&!podFiles.length){notify('AÃ±ade una foto, documento o informe del survey realizado');return false}
    if(stepKey==='delivery'&&!storageOnly&&!surveyService&&!podFiles.length&&!podException){notify('Escanea o adjunta el POD firmado, o marca POD no sellado con observaciÃ³n');return false}
    if(stepKey==='delivery'&&!storageOnly&&!surveyService&&podException&&!podExceptionReason){notify('Indica por quÃ© no se sellÃ³ el POD antes de cerrar la entrega');return false}
    ...
    const warehouseReviewNote=ready&&!surveyService?` AlmacÃ©n revisado: ${deliveryWarehouseScope.length} partida(s) activa(s) para ${target.buque}.`:ready&&surveyService?' Servicio survey confirmado a bordo.':'';
    const linkedWarehouseNote=cargoHasWarehouse?` AlmacÃ©n vinculado: ${selectedWarehouseEntries.map(entry=>entry.ref).join(', ')}.`:'';
    const podExceptionNote=ready&&podException?` POD no sellado / no disponible: ${podExceptionReason}.`:'';
    const timelineEntry={id:`FLOW-${id}-${stepKey}-${Date.now()}`,fecha:now.toLocaleDateString('es-ES'),hora:now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),titulo:ready&&podException?'Entrega confirmada sin POD sellado':expected.title,detalle:`${note||'Paso confirmado sin incidencias'}${podExceptionNote}${linkedWarehouseNote}${warehouseReviewNote}`,actor:visibleUser.fullName,archivo:ready?podFiles[0]||null:null,archivos:stepKey==='cargo'?[...cargoEvidence,...selectedWarehouseEntries.flatMap(entry=>[...(entry.fotos||[]),...(entry.documentosRecepcion||[])])]:stepKey==='documents'?documentEvidence:[...deliveryPhotos,...podFiles.slice(1)],estado:'done'};
  ...
  const undoCaseStep=(id,stepKey)=>{
    ...
    if(!lastCompleted||lastCompleted.key!==stepKey){notify('Solo puedes deshacer el Ãºltimo paso completado');return}
    ...
    const now=new Date();
    const timelineEntry={id:`UNDO-${id}-${stepKey}-${Date.now()}`,fecha:now.toLocaleDateString('es-ES'),hora:now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),titulo:`Paso reabierto: ${reopened.title}`,detalle:'El conductor deshizo la confirmaciÃ³n para corregir o repetir este paso. Las fotos, PODs y documentos existentes se conservan.',actor:visibleUser.fullName,estado:'done'};
  ...
  const reopenCaseStep=(id,stepKey)=>{
    ...
    if(stepIndex<0||!operationFlow(target)[stepKey]){notify('Ese paso todavÃ­a no estÃ¡ completado');return}
    if(!window.confirm(`Â¿Reabrir el paso "${reopened.title}"?
No se borrarÃ¡n documentos ni fotos. El expediente volverÃ¡ a este punto para corregir o aÃ±adir informaciÃ³n.`))return;
    const currentFlow=operationFlow(target);
    ...
    const progress=Math.round(steps.filter(step=>flow[step.key]).length/steps.length*100);
    const now=new Date();
    const timelineEntry={id:`REOPEN-${id}-${stepKey}-${Date.now()}`,fecha:now.toLocaleDateString('es-ES'),hora:now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),titulo:`Paso reabierto: ${reopened.title}`,detalle:'Reabierto desde el expediente para corregir, eliminar o aÃ±adir informaciÃ³n sin perder evidencias existentes',actor:visibleUser.fullName,estado:'done'};
    ...
    const vesselName=relatedCase?.buque||form.identificacion?.trim()||'MercancÃ­a sin identificar';
  ...
  return <div className="shell">
    ...
    {menuOpen&&<button className="scrim" aria-label="Cerrar menÃº" onClick={()=>setMenuOpen(false)}/>}
    <main className="main">
      <header className="topbar">
        <div className="topbar-title">
          <button className="icon-button menu-button" aria-label="Abrir menÃº" onClick={()=>setMenuOpen(true)}><Menu/></button>
      ...
      <div className="content">
        {previewUser&&<div className="preview-banner"><Eye/><span>EstÃ¡s viendo la aplicaciÃ³n como <b>{previewUser.fullName}</b> ({roleLabel(previewUser)}). Tu cuenta sigue siendo administrador.</span><button onClick={()=>setPreviewUser(null)}>Salir de la vista previa</button></div>}
  ...
  return <>
    ...
    <aside className="notification-drawer" aria-label="Centro de notificaciones">
      ...
      <section className="notification-drawer-actions">
        ...
        <button className="button secondary" onClick={openBilling}><ReceiptText/> Abrir facturaciÃ³n</button>
      ...
      <section>
        <h3>Ahora requiere atenciÃ³n</h3>
        {alerts.length?alerts.map(alert=><article className={`notification-card active ${alert.type==='billing'?'billing':''}`} key={alert.key}>
          ...
          <div><b>{alert.type==='billing'?'FacturaciÃ³n':alert.case?.buque||alert.event?.titulo||'Entrega'}</b><p>{alert.message}</p><small>{alert.rule?.label||'Aviso operativo'}  -  repetirÃ¡ hasta gestionar</small><button className="button secondary notification-ack" onClick={()=>acknowledge?.(alert)}>Entendido</button></div>
      ...
      <section>
        ...
        </article>):<p className="notification-empty">TodavÃ­a no hay avisos guardados en este dispositivo.</p>}
      </section>
    </aside>
  </>;
}
function DeliveryPopup({alert,close,openHistory}){
  return <div className="delivery-popup" role="alertdialog" aria-label="Aviso de entrega">
    ...
    <div>
      <small>{alert.type==='billing'?'FacturaciÃ³n pendiente':alert.rule?.followUp?'Seguimiento operativo':'Aviso de entrega'}</small>
...
function Sidebar({tab,open,navigate,close,nav,user,onLogout}){
  return <aside className={'sidebar '+(open?'open':'')}>
    <div className="brand"><span className="brand-mark"><Anchor/></span><div><b>SWIFTPORT</b><small>OPERATING SYSTEM</small></div><button className="icon-button sidebar-close" aria-label="Cerrar menÃº" onClick={close}><X/></button></div>
    <nav aria-label="NavegaciÃ³n principal">{nav.map(([id,label,Icon])=><button key={id} className={tab===id?'active':''} onClick={()=>navigate(id)}><Icon/><span>{label}</span>{tab===id&&<ChevronRight className="nav-arrow"/>}</button>)}</nav>
    <div className="sidebar-card"><div className="live-dot"/> <div><b>Operativa conectada</b><small>Datos de demostraciÃ³n</small></div></div>
    <div className="profile"><div className="avatar light">{initials(user.fullName)}</div><div><b>{user.fullName}</b><small>{roleLabel(user)}</small></div><button className="profile-logout" aria-label="Cerrar sesiÃ³n" title="Cerrar sesiÃ³n" onClick={onLogout}><LogOut/></button></div>
  </aside>;
}
function MobileNav({tab,navigate,more,nav}){
  const visible=nav.slice(0,4);
  return <nav className={'mobile-nav '+(nav.length===1?'single':'')} aria-label="NavegaciÃ³n mÃ³vil">{visible.map(([id,label,Icon])=><button key={id} className={tab===id?'active':''} onClick={()=>navigate(id)}><Icon/><span>{label}</span></button>)}{nav.length>1&&<button className={!visible.some(item=>item[0]===tab)?'active':''} onClick={more}><Menu/><span>MÃ¡s</span></button>}</nav>;
}
...
function AisTrackingPanel({item,csrfToken,reloadOperational,notify}){
  ...
  const enableDeviceAlerts=async()=>{
    if(!('Notification' in window)){notify('Este navegador no admite avisos. En iPhone, aÃ±ade Swiftport a la pantalla de inicio.');return}
    const permission=await Notification.requestPermission();
    if(permission!=='granted'){notify('Debes permitir las notificaciones de Swiftport en el telÃ©fono.');return}
    ...
    notify('Avisos de aproximaciÃ³n activados en este telÃ©fono');
  };
  const refreshButton=<button className="button secondary ais-refresh" onClick={refresh} disabled={refreshing}><RefreshCw className={refreshing?'spinning':''}/>{refreshing?'Buscando seÃ±al AISâ€¦':'Actualizar posiciÃ³n ahora'}</button>;
  const alertButton=<button className={'button '+(deviceAlerts?'device-alert-enabled':'tertiary')} onClick={enableDeviceAlerts} disabled={deviceAlerts}><Bell/>{deviceAlerts?'Avisos activos':'Activar avisos en este mÃ³vil'}</button>;
  if(!hasIdentifier)return <section className="ais-panel ais-empty"><Navigation/><div><small>SEGUIMIENTO DEL BUQUE</small><b>AÃ±ade el IMO o MMSI para localizarlo</b><p>Edita el expediente e introduce el IMO de 7 dÃ­gitos o el MMSI de 9 dÃ­gitos.</p></div></section>;
  if(!tracking)return <section className="ais-panel"><VesselFinderMap item={item}/><div className="ais-info"><span className="overline"><Navigation/> MAPA OFICIAL VESSELFINDER</span><div className="ais-status"><i className="stale"/><span><small>DATOS DE SWIFTPORT</small><b>Esperando seÃ±al propia</b></span></div><p>El mapa muestra la Ãºltima posiciÃ³n disponible en VesselFinder. Swiftport seguirÃ¡ consultando AISStream para calcular mÃ©tricas y alertas.</p><div className="ais-actions">{item.mmsi?refreshButton:<p>AÃ±ade tambiÃ©n el MMSI para activar la actualizaciÃ³n automÃ¡tica de Swiftport.</p>}{alertButton}</div></div></section>;
  ...
  return <section className="ais-panel"><VesselFinderMap item={item}/><div className="ais-info"><span className="overline"><Navigation/> VESSELFINDER + AISSTREAM</span><div className="ais-status"><i className={stale?'stale':['Atracado','En fondeo','Atraque probable'].includes(tracking.status)?'moored':'live'}/><span><small>ESTADO ESTIMADO</small><b>{stale?'SeÃ±al sin actualizar':tracking.status}</b></span></div><div className="ais-metrics"><span><small>DISTANCIA AL PUERTO</small><b>{tracking.distanceToPortNm==null?'No calculada':tracking.distanceToPortNm+' mn'}</b></span><span><small>ETA ESTIMADA AIS</small><b>{etaEstimate&&!Number.isNaN(etaEstimate.getTime())?etaEstimate.toLocaleString('es-ES',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'Sin calcular'}</b></span><span><small>VELOCIDAD</small><b>{tracking.speed} kn</b></span><span><small>RUMBO</small><b>{tracking.course}Â°</b></span><span><small>ÃšLTIMA SEÃ‘AL</small><b>{last?new Date(last).toLocaleString('es-ES'):'â€”'}</b></span></div><div className="ais-actions">{refreshButton}{alertButton}<p>AutomÃ¡tico cada 30 minutos  -  ETA AIS orientativa; confirma el atraque con el consignatario.</p></div></div></section>;
}
...
const PORT_TONE_MAP={BARCELONA:'blue',TARRAGONA:'orange',SAGUNTO:'teal',VALENCIA:'purple',ALGECIRAS:'red',BILBAO:'green',VINAROS:'pink',CASTELLON:'teal',MARIN:'purple','A CORUNA':'green',HUELVA:'red',SEVILLA:'pink',CARTAGENA:'orange',ALICANTE:'blue'};
...
function formatSchedule(date,start,end){if(!date||!start)return 'Por programar';const label=new Date(date+'T12:00:00').toLocaleDateString('es-ES',{day:'2-digit',month:'short'}).replace('.','');return label+'  -  '+start+(end?'â€“'+end:'')}
...
const billingAlertsForCases=(cases=[],invoices=[],user,now=new Date())=>{
  ...
  return (cases||[]).flatMap(item=>{
    ...
    const message=`FacturaciÃ³n pendiente: ${vessel} (${item.id}) estÃ¡ listo para facturar en ${port}. Estado: ${status}${amount?` Â· Importe aprox. ${moneyExact(amount)}`:''}.`;
    return [{key:`${item.id}-billing-${bucket}`,type:'billing',case:item,invoice,rule:{key:'billing60',label:'facturaciÃ³n pendiente cada 60 min'},message,moment:now}];
  });
};
...
const deliveryAlertsForSchedule=(events=[],cases=[],user,now=new Date())=>(events||[])
  ...
  .flatMap(event=>{
    ...
    if(diff<=0&&calendarHasValidStart(event)){
      ...
      const message=`Seguimiento activo: ${vessel} tenÃ­a transporte programado para ${timeNote} en ${port}. Confirma con ${driver} que estÃ¡ coordinado o registra el avance.`;
...
const driverTransportDone=(event,cases=[],transports=[])=>{
  ...
  const isWarehousePickup=destination.includes('ALMACEN')||destination.includes('ALMACÃ‰N')||destination.includes('BLUESPACE');
...
const driverDueInfo=event=>{
  ...
  if(days<0)return {label:'Atrasado',detail:`Hace ${Math.abs(days)} dÃ­a${Math.abs(days)===1?'':'s'}  -  ${time}`,tone:'late'};
  if(days===0)return {label:'Hoy',detail:time,tone:'today'};
  if(days===1)return {label:'MaÃ±ana',detail:time,tone:'soon'};
  if(days<=6)return {label:`En ${days} dÃ­as`,detail:time,tone:'soon'};
...
function Calendario({events,team,cases,transports,providers,warehouseEntries,saveEvent,deleteEvent,completeCaseStep,undoCaseStep,openCase,currentUser,csrfToken,reloadOperational,notify}){
  ...
  return <>
    <section className="calendar-toolbar">
      <div className="calendar-nav"><button className="button tertiary" onClick={()=>movePeriod(-1)}>â€¹</button><button className="button tertiary" onClick={goToday}>Hoy</button><button className="button tertiary" onClick={()=>movePeriod(1)}>â€º</button><h2>{calendarTitle}</h2></div>
      <div className="calendar-actions"><div className="calendar-view-switch">{['day','week','month'].map(mode=><button key={mode} className={viewMode===mode?'active':''} onClick={()=>{setViewMode(mode);setWeekStart(current=>mode==='week'?startOfWeek(current):mode==='month'?startOfMonth(current):localDay(current))}}>{mode==='day'?'DÃ­a':mode==='week'?'Semana':'Mes'}</button>)}</div>{hasRole(currentUser,'operations')&&<button className={'button '+(mineOnly?'secondary':'tertiary')} onClick={()=>setMineOnly(!mineOnly)}><UserRound/> Mis servicios</button>}<button className="button primary" onClick={newEvent}><Plus/> Nuevo transporte</button></div>
    </section>
    {viewMode==='month'?<CalendarMonthView days={days} monthDate={periodStart} events={baseEvents} cases={cases} setEditing={setEditing} openCase={openCase}/>:<section className="calendar-shell panel" style={{'--calendar-days':days.length,'--calendar-min-width':`${70+(days.length*260)}px`}}>
      <div className="calendar-help"><span><CalendarDays/> Solo transportes a ETB/ETA</span><small>Las recepciones quedan en expediente/almacÃ©n. Si falta hora ETB/ETA, el transporte queda arriba del dÃ­a como â€œFalta horarioâ€.</small></div>
...
function DraggableCalendarEvent({event,cases,team,saveEvent,setEditing,openCase,canDeleteEvent,deleteEvent,startPointerDrag,draggingId,suppressClick}){
  ...
  return <article onPointerDown={pointer=>startPointerDrag(pointer,clean)} onClick={openRelated} className={`calendar-event ${event.color} ${event._columns>1?'is-overlap':''} ${draggingId===event.id?'dragging':''}`} style={calendarEventStyle(event)} title={`${event.inicio}â€“${event.fin}  -  ${event.titulo||event.id}`}>
...
function WarehouseTransportReview({entries,item,checked,setChecked}){
  ...
  return <section className="warehouse-transport-review">
    <div className="warehouse-review-head"><Box/><span><b>RevisiÃ³n obligatoria de almacÃ©n</b><small>{entries.length?`Hay ${entries.length} partida(s) activas para ${item.buque}. Revisa tambiÃ©n mercancÃ­a antigua o sin expediente.`:`No aparece stock activo para ${item.buque}, pero debes confirmarlo antes de cerrar.`}</small></span><strong>{totalUnits} bultos</strong></div>
    {entries.length?<div className="warehouse-review-list">{entries.map((entry,index)=>{
      ...
      return <article key={entry.ref||index}><span><b>{vessel}</b><small>{entry.ref||`Entrada ${index+1}`}  -  {linked}  -  {entry.entrada||formatReceptionDate(entry.fechaRecepcion||entry.fecha)||'Fecha pendiente'}</small></span><em>{pieces}</em><small>{entry.zona||'UbicaciÃ³n pendiente'}</small></article>;
    })}</div>:<p><CircleAlert/> Si sabes que hay una caja antigua para este buque, regÃ­strala en almacÃ©n antes de cerrar la entrega.</p>}
    <label className="warehouse-review-check"><input type="checkbox" checked={checked} onChange={event=>setChecked(event.target.checked)}/><span>He revisado todo lo que hay en almacÃ©n para este buque y se entregarÃ¡ todo lo pendiente.</span></label>
  </section>;
}
function WarehouseCargoReception({entries,selectedRefs,toggle}){
  ...
  return <section className="warehouse-cargo-reception">
    <div className="warehouse-review-head"><WarehouseIcon/><span><b>MercancÃ­a ya registrada en almacÃ©n</b><small>{entries.length?'Selecciona las entradas que corresponden a este expediente para recepcionarlas aquÃ­.':'No hay stock activo detectado para este buque/expediente. Puedes tomar fotos de recepciÃ³n para crear la entrada.'}</small></span><strong>{totalUnits} bultos</strong></div>
    ...
    })}</div>:<p><CircleAlert/> Si la mercancÃ­a ya estÃ¡ fÃ­sicamente en almacÃ©n pero no aparece aquÃ­, regÃ­strala primero desde AlmacÃ©n o toma fotos en este paso.</p>}
  </section>;
}
function PodDocuments({item,notify,onDelete}){
  const documentation=item?.documentacionMercancia||{};
  ...
  return <div className="document-box"><h3>POD de entrega <small>{pods.length?`${pods.length} PDF${pods.length===1?'':'S'}`:podException?'NO SELLADO':'PENDIENTE'}</small></h3>{podException&&<div className="pod-exception-note"><CircleAlert/><span><b>POD no sellado / no disponible</b><small>{documentation.podObservacion||'Sin observaciÃ³n registrada'}</small></span></div>}{pods.length?pods.map((file,index)=><div className="attachment-row compact" key={file.id||file.url||index}><a className="document-link" href={file.url} target="_blank" rel="noreferrer"><FileText/><span><b>POD {index+1}</b><small>{file.name}</small></span><ExternalLink/></a>{onDelete&&<button type="button" className="icon-button danger attachment-delete" aria-label="Eliminar POD" onClick={()=>onDelete(file,'pod')}><Trash2/></button>}</div>):!podException&&<button onClick={()=>notify('POD todavÃ­a pendiente')}><Camera/><span><b>POD / fotografÃ­as</b><small>Pendiente de entrega</small></span><ChevronRight/></button>}{deliveryPhotos.length>0&&<div className="delivery-photo-list"><b>Fotos de entrega</b>{deliveryPhotos.map((file,index)=><div className="attachment-row compact" key={file.id||file.url||index}><a className="document-link" href={file.url} target="_blank" rel="noreferrer"><Camera/><span><b>Foto entrega {index+1}</b><small>{file.name}</small></span><ExternalLink/></a>{onDelete&&<button type="button" className="icon-button danger attachment-delete" aria-label="Eliminar foto de entrega" onClick={()=>onDelete(file,'delivery-photo')}><Trash2/></button>}</div>)}</div>}</div>;
}
function DriverTaskModal({event,item,transport,warehouseEntries,currentUser,csrfToken,reloadOperational,notify,close,claim,submit,undo}){
  ...
  const instructions={
    review:'Lee el servicio completo y comprueba buque, fecha, puerto, ruta, mercancÃ­a y observaciones antes de empezar.',
    cargo:surveyService?'Confirma llegada al buque o acceso a bordo para realizar el muestreo. Puedes aÃ±adir foto si hace falta.':inWarehouse?'Comprueba cantidades, peso y estado de la mercancÃ­a antes de cargar.':'Recoge la mercancÃ­a en el lugar indicado y comprueba cantidades, peso y estado.',
    documents:surveyService?'Revisa instrucciones, formularios o documentos del muestreo antes de cerrar el servicio.':'Comprueba que estÃ¡n listos los documentos necesarios antes de salir a entregar.',
    assignment:'El transporte debe quedar asignado. Puedes asignÃ¡rtelo desde esta misma pantalla si estÃ¡ libre.',
    delivery:surveyService?'Confirma que el survey / Ballast Water Samples quedÃ³ realizado y adjunta foto, nota o informe.':'Antes de entregar, revisa todo lo que hay en almacÃ©n para este buque. Luego fotografÃ­a la entrega y escanea el POD firmado.'
  };
  if(storageOnly)instructions.delivery='Confirma la salida del almacÃ©n o la recogida por tercero con una foto/evidencia clara.';
  const uploadEvidence=async(files,evidenceType)=>{
    ...
    try{
      ...
      if(evidenceType==='pod'&&selected.some(file=>file.type.startsWith('image/')))notify?.('POD escaneado automÃ¡ticamente: recortado, corregido y guardado como PDF');
  ...
  const evidenceLabel=file=>file.evidenceType==='pod'?(surveyService?'Informe / documento':'POD escaneado'):file.evidenceType==='delivery-photo'?(surveyService?'Evidencia survey':'Foto de entrega'):'Foto de recepciÃ³n';
  return <div className="modal-backdrop driver-task-backdrop" onMouseDown={mouse=>{if(mouse.target===mouse.currentTarget)close()}}>
    <section className="modal driver-task-modal">
      <div className="modal-head"><div><span className="overline">{event.inicio}â€“{event.fin}  -  {event.tipoServicio}</span><h2>{item.buque}</h2><p>{caseLabel(item)}</p></div><button className="icon-button" onClick={close}><X/></button></div>
      <div className="driver-task-body">
        ...
        {false&&<div className="driver-owner-alert"><UserRound/><span><b>{event.asignado&&event.asignado!=='Sin asignar'?`Asignado a ${event.asignado}`:'Trabajo sin conductor'}</b><small>AsÃ­gnatelo antes de registrar avances.</small></span><button className="button secondary" onClick={claim}>Asignarme</button></div>}
        ...
        {step?<>
          ...
          {needsEvidence&&<div className="pod-scanner evidence-capture">
            <div><Camera/><span><b>{step.key==='cargo'?(surveyService?'Evidencia inicial del survey':'Fotos de la mercancÃ­a recibida'):surveyService?'Evidencia del survey realizado':storageOnly?'Evidencias de salida / recogida':'Evidencias de la entrega'}</b><small>{step.key==='cargo'?(surveyService?'Opcional: foto de acceso, equipo o llegada al buque.':'Se requiere al menos una foto; puedes aÃ±adir todas las necesarias.'):surveyService?'Se requiere foto, documento o informe del servicio realizado.':storageOnly?'Se requiere foto o evidencia clara de que la mercancÃ­a saliÃ³ del almacÃ©n.':'Se requiere foto de la entrega y POD firmado.'}</small></span></div>
            {false&&<div className="evidence-assignment-lock"><LockKeyhole/><span><b>Activa el registro de este trabajo</b><small>{event.asignado&&event.asignado!=='Sin asignar'?`Ahora figura asignado a ${event.asignado}. AsÃ­gnatelo para activar la cÃ¡mara y el POD.`:'El servicio todavÃ­a no tiene conductor. AsÃ­gnatelo para activar la cÃ¡mara y el POD.'}</small></span><button className="button secondary" onClick={claim}>Asignarme y activar</button></div>}
            <div className="pod-scanner-actions">
              <label className={`button primary${!mine?' disabled':''}`}><Camera/> {uploading?'Procesandoâ€¦':step.key==='cargo'?'AÃ±adir fotos de recepciÃ³n':'AÃ±adir fotos de entrega'}<input type="file" accept="image/*" capture="environment" multiple disabled={uploading||!mine} onChange={change=>{uploadEvidence(change.target.files,step.key==='cargo'?'cargo-photo':'delivery-photo');change.target.value=''}}/></label>
              {step.key==='delivery'&&!storageOnly&&!surveyService&&<><label className={`button secondary${!mine?' disabled':''}`}><ScanLine/> Escanear POD automÃ¡tico<input type="file" accept="image/*" capture="environment" multiple disabled={uploading||!mine} onChange={change=>{uploadEvidence(change.target.files,'pod');change.target.value=''}}/></label><label className={`button tertiary${!mine?' disabled':''}`}><FileText/> AÃ±adir PDFs de POD<input type="file" accept="application/pdf" multiple disabled={uploading||!mine} onChange={change=>{uploadEvidence(change.target.files,'pod');change.target.value=''}}/></label></>}
              {step.key==='delivery'&&surveyService&&<label className={`button tertiary${!mine?' disabled':''}`}><FileText/> AÃ±adir informe / PDF<input type="file" accept="application/pdf,image/*" multiple disabled={uploading||!mine} onChange={change=>{uploadEvidence(change.target.files,'pod');change.target.value=''}}/></label>}
            </div>
            {step.key==='delivery'&&!storageOnly&&!surveyService&&<label className={'document-switch pod-exception '+(podException?'checked':'')}><input type="checkbox" checked={podException} onChange={change=>setPodException(change.target.checked)}/><CircleAlert/><span><b>POD no sellado / no disponible</b><small>Permite cerrar la entrega dejando una observaciÃ³n obligatoria para facturaciÃ³n.</small></span></label>}
            {step.key==='delivery'&&<div className="evidence-requirements"><span className={deliveryPhotos.length?'done':''}><CheckCircle2/> {surveyService?'Foto / evidencia':storageOnly?'Evidencia de salida':'Foto de entrega'} {deliveryPhotos.length?`(${deliveryPhotos.length})`:'pendiente'}</span>{!storageOnly&&!surveyService&&<span className={podFiles.length||podException?'done':''}><CheckCircle2/> {podException?'POD no sellado justificado':'POD firmado'} {podFiles.length?`(${podFiles.length})`:podException?'con observaciÃ³n':'pendiente'}</span>}{surveyService&&<span className={podFiles.length?'done':''}><CheckCircle2/> Informe / documento {podFiles.length?`(${podFiles.length})`:'opcional'}</span>}</div>}
          ...
          <label className="field"><span>{noteRequiredForPodException?'ObservaciÃ³n obligatoria':'ObservaciÃ³n del trabajo (opcional)'}</span><input value={note} onChange={change=>setNote(change.target.value)} placeholder={noteRequiredForPodException?'Ej. El buque/consignatario no sellÃ³ el POD, entrega realizada sin selloâ€¦':'Persona que recibe, incidencia, referenciaâ€¦'}/></label>
          <button className="button primary full driver-confirm" disabled={uploading||!evidenceReady} onClick={()=>submit(step.key,note,{files:evidenceFiles,podException,podExceptionReason:note})}><CheckCircle2/> {!evidenceReady?(step.key==='cargo'?'AÃ±ade una foto para confirmar':surveyService?'AÃ±ade evidencia o informe del survey':warehouseReviewMissing?'Marca que revisaste el almacÃ©n':storageOnly?'AÃ±ade evidencia de salida':podException?'Escribe la observaciÃ³n del POD no sellado':'Revisa almacÃ©n, foto de entrega y POD'):`Confirmar: ${step.title}`}</button>
        </>:<div className="driver-finished"><CheckCircle2/><span><b>Trabajo terminado</b><small>{surveyService?'Survey confirmado y expediente listo para facturaciÃ³n.':storageOnly?'Salida confirmada y expediente listo para facturaciÃ³n.':'POD recibido y expediente listo para facturaciÃ³n.'}</small></span></div>}
...
function DriverTaskModalLegacy({event,item,transport,warehouseEntries,currentUser,csrfToken,reloadOperational,notify,close,claim,submit,undo}){
  ...
  const instructions={
    review:'Lee el servicio completo y comprueba buque, fecha, puerto, ruta, mercancÃ­a y observaciones antes de empezar.',
    cargo:inWarehouse?'Comprueba cantidades, peso y estado de la mercancÃ­a antes de cargar.':'Recoge la mercancÃ­a en el lugar indicado y comprueba cantidades, peso y estado.',
    documents:'Comprueba que estÃ¡n listos los documentos necesarios antes de salir a entregar.',
    delivery:'Entrega toda la mercancÃ­a, confirma quiÃ©n la recibe y fotografÃ­a o escanea el POD firmado. Al confirmar quedarÃ¡ lista para facturar.'
  };
  ...
  const needsEvidence=['cargo','delivery'].includes(step?.key);
  const evidenceTitle=step?.key==='cargo'?'Fotografiar mercancÃ­a recibida':storageOnly?'Confirmar salida de almacÃ©n':'Scanner automÃ¡tico de POD';
  return <div className="modal-backdrop driver-task-backdrop" onMouseDown={mouse=>{if(mouse.target===mouse.currentTarget)close()}}><section className="modal driver-task-modal"><div className="modal-head"><div><span className="overline">{event.inicio}â€“{event.fin}  -  {event.tipoServicio}</span><h2>{item.buque}</h2><p>{caseLabel(item)}</p></div><button className="icon-button" onClick={close}><X/></button></div><div className="driver-task-body"><div className="driver-route"><MapPin/><span><small>PUERTO / RUTA</small><b>{transport?.ruta||item.puerto}</b></span></div>{!mine&&<div className="driver-owner-alert"><UserRound/><span><b>{event.asignado&&event.asignado!=='Sin asignar'?`Asignado a ${event.asignado}`:'Trabajo sin conductor'}</b><small>AsÃ­gnatelo antes de registrar avances.</small></span><button className="button secondary" onClick={claim}>Asignarme</button></div>}<OperationChecklist item={item} csrfToken={csrfToken} reloadOperational={reloadOperational} notify={notify}/><CargoManifest item={item} transport={transport||event}/>{step?<><div className="driver-next-action"><span>{OPERATION_STEPS.findIndex(entry=>entry.key===step.key)+1}</span><div><small>AHORA TOCA</small><b>{step.title}</b><p>{instructions[step.key]}</p></div></div>{needsEvidence&&<div className="pod-scanner"><div><Camera/><span><b>{evidenceTitle}</b><small>{step.key==='cargo'?'Haz al menos una foto clara. Puedes aÃ±adir varias.':'Haz una foto del POD; Swiftport lo recorta, limpia y guarda como PDF.'}</small></span></div><div className="pod-scanner-actions"><label className="button primary"><Camera/> {uploading?'Escaneandoâ€¦':step.key==='cargo'?'Hacer foto':'Escanear con cÃ¡mara'}<input type="file" accept="image/*" capture="environment" disabled={uploading||!mine} onChange={change=>uploadEvidence(change.target.files?.[0])}/></label>{step.key==='delivery'&&<label className="button tertiary"><FileText/> Adjuntar PDF<input type="file" accept="application/pdf" disabled={uploading||!mine} onChange={change=>uploadEvidence(change.target.files?.[0])}/></label>}</div>{evidenceFiles.length>0&&<div className="evidence-file-list">{evidenceFiles.map((file,index)=><a className="pod-uploaded" href={file.url} target="_blank" rel="noreferrer" key={file.id}><CheckCircle2/><span><b>{step.key==='cargo'?`Foto ${index+1}`:'POD escaneado en PDF'}</b><small>{file.name}</small></span><ExternalLink/></a>)}</div>}{error&&<p className="form-error"><CircleAlert/>{error}</p>}</div>}<label className="field"><span>ObservaciÃ³n del trabajo (opcional)</span><input value={note} disabled={!mine} onChange={change=>setNote(change.target.value)} placeholder="Persona que recibe, incidencia, referenciaâ€¦"/></label><button className="button primary full driver-confirm" disabled={!mine||uploading||(needsEvidence&&!evidenceFiles.length)} onClick={()=>submit(step.key,note,step.key==='cargo'?evidenceFiles:evidenceFiles[0]||null)}><CheckCircle2/> {needsEvidence&&!evidenceFiles.length?(step.key==='cargo'?'Haz una foto para confirmar':'Escanea el POD para confirmar'):`Confirmar: ${step.title}`}</button></>:<div className="driver-finished"><CheckCircle2/><span><b>Trabajo terminado</b><small>POD recibido y expediente listo para facturaciÃ³n.</small></span></div>}{mine&&lastCompleted&&<button className="button tertiary full driver-undo-step" onClick={()=>undo(lastCompleted.key)}><Undo2/> Deshacer: {lastCompleted.title}</button>}<button className="button tertiary full" onClick={close}>{flow.billingReady?'Cerrar':'Volver al calendario'}</button></div></section></div>;
}
function Dashboard({cases,warehouseEntries,calendarEvents,openCase,navigate,showFinance,user}){
  ...
  const deliveryWatch=upcomingEvents
    ...
    .map(({event,related})=>{
      const next=nextOperationStep(related);
      const missing=[calendarNeedsTime(event)&&'falta hora',(!event.asignado||event.asignado==='Sin asignar')&&'sin conductor'].filter(Boolean).join(' Â· ');
      const step=next?.title||'Listo para facturar';
      return {tone:missing?'warning':related.prioridad==='Urgente'?'danger':'info',title:String(related.buque||event.titulo||'BUQUE').toUpperCase(),meta:`${event.fecha} ${calendarNeedsTime(event)?'Falta hora':event.inicio} Â· Paso: ${step}${missing?` Â· ${missing}`:''}`,action:()=>openCase(related.id)};
    });
  const attention=[
    ...deliveryWatch,
    ...missingDriver.slice(0,2).map(event=>({tone:'warning',title:'Transporte sin conductor',meta:`${String(event.titulo||'BUQUE').toUpperCase()} Â· ${formatSchedule(event.fecha,event.inicio,event.fin)}`,action:()=>navigate('calendario')})),
    ...missingTime.slice(0,2).map(event=>({tone:'warning',title:'Falta horario de servicio',meta:`${String(event.titulo||'BUQUE').toUpperCase()} Â· ${event.fecha||'Sin fecha'}`,action:()=>navigate('calendario')})),
    ...urgentCases.slice(0,2).map(item=>({tone:'danger',title:'Expediente urgente',meta:`${caseLabel(item)} Â· ${item.puerto}`,action:()=>openCase(item.id)})),
    ...readyToBill.filter(item=>item.estado!=='Completado').slice(0,3).map(item=>({tone:'info',title:'Listo para facturar',meta:`${caseLabel(item)} Â· revisar borrador`,action:()=>showFinance?navigate('facturacion'):openCase(item.id)}))
  ].slice(0,5);
  ...
  const agendaSubtitle=nextFive.length?'PrÃ³ximos 5 trabajos por fecha y hora':'Sin trabajos prÃ³ximos';
  const agendaMeta=event=>{
    ...
    const route=[event.origen,event.destino].filter(Boolean).join(' â†’ ')||event.titulo||event.puerto||related?.puerto||'Ruta por confirmar';
    return `${dateLabel} Â· ${event.tipoServicio||'Transporte'} Â· ${route} Â· ${event.asignado||'Sin asignar'}`;
  };
  ...
  return <>
    <section className="welcome"><div><span className="overline"><Sparkles/> Resumen del turno</span><h2>Buenos dÃ­as, {user.fullName.split(' ')[0]}</h2><p>Hay <b>{alerts} operaciones que necesitan atenciÃ³n</b>. El resto avanza segÃºn lo previsto.</p></div><button className="button ghost-light" onClick={()=>navigate('expedientes')}>Ver operativa <ChevronRight/></button></section>
    <section className={'kpi-grid '+(!showFinance?'kpi-grid-three':'')}>
      ...
      <Kpi icon={PackageCheck} label="Bultos en almacÃ©n" value={String(stock)} note={`${stockEntries.length} entradas activas`} tone="teal"/>
      <Kpi icon={CircleAlert} label="Requieren acciÃ³n" value={alerts} note={`${urgentCases.length} de prioridad urgente`} tone="orange"/>
    ...
    <div className="dashboard-grid">
      <section className="panel attention-panel"><SectionHeader title="Requieren acciÃ³n" subtitle="Ordenado por prioridad" action={<button className="text-button" onClick={()=>navigate('expedientes')}>Ver todos</button>}/><div className="attention-list">
      ...
      <section className="panel today-panel"><SectionHeader title="Agenda operativa" subtitle={agendaSubtitle}/><div className="schedule">
        {nextFive.length?nextFive.map(event=><Schedule key={event.id} time={calendarNeedsTime(event)?'Falta hora':event.inicio} title={agendaTitle(event)} meta={agendaMeta(event)} active={!calendarNeedsTime(event)} alert={calendarNeedsTime(event)}/>):<Empty text="No hay transportes prÃ³ximos programados."/>}
      </div></section>
    </div>
    <section className="panel operations"><SectionHeader title="Operaciones recientes" subtitle="Ãšltimos expedientes creados o modificados" action={<button className="filter-button" onClick={()=>navigate('expedientes')}><Filter/> Filtrar</button>}/><div className="responsive-table"><div className="table-head"><span>Expediente</span><span>Destino</span><span>ETA</span><span>Progreso</span><span>Estado</span><span/></div>{recent.map(item=><button className="table-row" key={item.id} onClick={()=>openCase(item.id)}><span className="primary-cell"><span className="ship-icon"><Ship/></span><span><b>{caseLabel(item)}</b><small>{item.cliente}</small></span></span><span data-label="Destino"><MapPin/>{item.puerto}</span><span data-label="ETA">{item.eta}</span><span data-label="Progreso"><span className="mini-progress"><i style={{width:item.progreso+'%'}}/></span>{item.progreso}%</span><span data-label="Estado"><Badge>{item.estado}</Badge></span><ChevronRight/></button>)}</div></section>
  </>;
}
...
function Expedientes({cases,selected,select,search,setSearch,completeCaseStep,notify,showFinance,updateCase,deleteCase,deleteAttachment,reopenCaseStep,clientOptions,warehouseEntries,transports,calendarEvents,team,providers,vessels,saveEvent,csrfToken,reloadOperational,currentUser}){
  ...
  return <div className={'case-layout '+(mobileDetail?'mobile-detail-open':'')}>
    ...
    {flowOpen&&<OperationStepModal item={selected} warehouseEntries={warehouseEntries} transports={transports} csrfToken={csrfToken} currentUser={currentUser} close={()=>setFlowOpen(false)} submit={(key,note,evidence)=>{if(completeCaseStep(selected.id,key,note,evidence))setFlowOpen(false)}}/>}
  </div>;
}
function CaseServicesPanel({item,events,cases,transports,team,providers,warehouseEntries=[],saveEvent}){
  ...
  return <><SectionHeader title="Transporte programado" subtitle={`${scheduled.length} transporte(s) sincronizados con Calendario`}/><div className="case-service-actions"><button className="button primary" onClick={()=>addService('Transporte')}><Truck/> AÃ±adir transporte</button></div>{scheduled.length?<div className="case-service-list">{scheduled.map(event=><button key={event.id} onClick={()=>setEditing(event)}><span className="case-service-icon transport"><Truck/></span><span><b>{event.tipoServicio||'Transporte'}  -  {calendarNeedsTime(event)?'Falta horario':`${event.inicio}â€“${event.fin}`}</b><small>{event.fecha}  -  {event.titulo||item.puerto}</small></span><span><b>{event.asignado||'Sin asignar'}</b><small>TRANSPORTE Y CALENDARIO</small></span><PencilLine/></button>)}</div>:<div className="case-services-empty"><CalendarDays/><span><b>Este expediente todavÃ­a no tiene transporte</b><small>AÃ±ade el transporte al buque para que aparezca en el calendario de los conductores.</small></span></div>}{editing&&<CalendarEventModal item={editing} team={team} cases={cases} transports={transports} providers={providers} warehouseEntries={warehouseEntries} close={()=>setEditing(null)} submit={event=>{saveEvent(event);setEditing(null)}} openCase={()=>setEditing(null)}/>}</>;
}
...
function ActualTimeline({item}){
  ...
  if(!events.length)return <div className="timeline-empty"><Clock3/><b>Sin actividad registrada</b><small>La cronologÃ­a aparecerÃ¡ cuando el equipo confirme el primer paso.</small></div>;
  return <div className="timeline actual-timeline">{events.map((event,index)=><div className="timeline-event done" key={event.id||event.titulo+index}><span className="timeline-marker"><CheckCircle2/></span><time>{event.hora||'â€”'}<small>{event.fecha||''}</small></time><span><b>{event.titulo}</b><small>{event.detalle}</small>{event.actor&&<em>Registrado por {event.actor}</em>}{event.archivo&&<a href={event.archivo.url} target="_blank" rel="noreferrer"><FileText/> POD principal: {event.archivo.name}</a>}{(event.archivos||[]).map((file,fileIndex)=>{const document=['pod','shipment-document'].includes(file.evidenceType)||String(file.name||'').toLowerCase().endsWith('.pdf');return <a href={file.url} target="_blank" rel="noreferrer" key={file.id||fileIndex}>{document?<FileText/>:<Camera/>} {file.evidenceType==='pod'?'POD adicional':document?'Documento':`Foto ${fileIndex+1}`}: {file.name}</a>})}</span></div>)}</div>;
}
function CaseStepReopenPanel({item,reopen}){
  const flow=operationFlow(item);
  const completed=operationStepsFor(item).filter(step=>flow[step.key]);
  if(!completed.length)return null;
  return <section className="case-reopen-panel"><div><Undo2/><span><b>Corregir pasos completados</b><small>Reabre un paso para aÃ±adir o modificar documentos, fotos, mercancÃ­a o POD sin borrar lo ya subido.</small></span></div><div>{completed.map(step=><button type="button" key={step.key} onClick={()=>reopen(step.key)}><Undo2/><span>Reabrir {step.title}</span></button>)}</div></section>;
}
function ClientCostPanel({item,warehouseEntries,updateCase,notify}){
  ...
  const save=()=>{updateCase({...item,clientCostEstimate:{...draft,updatedAt:new Date().toISOString()}});notify?.('PrevisiÃ³n de costes guardada en el expediente')};
  const copy=async()=>{
    const text=[
      `*${String(item.buque||item.id).toUpperCase()}*`,
      `${item.id} Â· ${String(item.puerto||'PUERTO PENDIENTE').toUpperCase()}`,
      '',
      ...(draft.lines||[]).map(line=>`â€¢ ${String(line.item||'CONCEPTO').toUpperCase()} - ${line.detail||cargo}: ${moneyExact(clientCostLineTotal(line))}`),
      '',
      `*TOTAL: ${moneyExact(total)}*`
    ].join('\n');
    try{await navigator.clipboard.writeText(text);notify?.('Costes copiados para enviar al cliente')}
    catch{notify?.('No se pudo copiar automÃ¡ticamente. Selecciona el texto manualmente.')}
  };
  return <><SectionHeader title="Costes para cliente" subtitle={isAlsCase(item)?'Tarifa ALS Barcelona 2026: UNLOAD, STORAGE y LOAD separados':'PrevisiÃ³n editable para adelantos o costes a informar'} action={<div className="client-cost-actions"><button className="button tertiary" onClick={recalc}><RefreshCw/> Recalcular</button><button className="button secondary" onClick={copy}><ClipboardList/> Copiar para cliente</button></div>}/><div className="client-cost-summary"><article><small>Cliente</small><b>{item.cliente}</b></article><article><small>MercancÃ­a</small><b>{cargo}</b></article><article className="client-cost-weight"><small>KG totales</small><b>{weight.toLocaleString('es-ES',{maximumFractionDigits:2})} kg</b></article><article><small>Storage real</small><b>{storageDays} dÃ­as</b></article></div><div className="client-cost-note">{isAlsCase(item)?<><b>Regla ALS:</b> LOAD y UNLOAD se cobran por separado a 0,12 â‚¬/kg. Storage tiene 3 dÃ­as gratis y luego se cobra por dÃ­a segÃºn peso.</>:<><b>Sin tarifa automÃ¡tica:</b> puedes aÃ±adir los conceptos manualmente y guardar la previsiÃ³n.</>}</div><div className="client-cost-lines"><div className="client-cost-lines-head"><b>Conceptos</b><button type="button" className="button secondary compact" onClick={addLine}><Plus/> AÃ±adir concepto</button></div>{(draft.lines||[]).map((line,index)=><article key={line.id||index}><label className="field"><span>Concepto</span><input value={line.item||''} onChange={event=>updateLine(index,'item',event.target.value)}/></label><label className="field line-detail"><span>Detalle</span><input value={line.detail||''} onChange={event=>updateLine(index,'detail',event.target.value)} placeholder={cargo}/></label><label className="field"><span>Precio</span><input type="number" min="0" step="0.01" value={line.price} onChange={event=>updateLine(index,'price',event.target.value)}/></label><label className="field"><span>Uds.</span><input type="number" min="0" step="0.01" value={line.units} onChange={event=>updateLine(index,'units',event.target.value)}/></label><strong>{moneyExact(clientCostLineTotal(line))}</strong>{(draft.lines||[]).length>1&&<button type="button" className="icon-button danger" onClick={()=>removeLine(index)} aria-label="Eliminar concepto"><Trash2/></button>}</article>)}</div><div className="client-cost-footer"><span><small>Total previsto para informar</small><b>{moneyExact(total)}</b></span><button className="button primary" onClick={save}><Save/> Guardar costes</button></div></>;
}
function CaseExpensesPanel({item,updateCase,notify}){
  ...
  const addExpense=()=>{
    ...
    if(!draft.concepto.trim()||amount<=0){notify?.('Indica concepto e importe del gasto.');return}
    ...
    notify?.('Gasto aÃ±adido al expediente.');
  };
...
function OperationStepModal({item,warehouseEntries,transports,csrfToken,currentUser,close,submit}){
  ...
  const guidance={
    review:'Comprueba buque, ETA, puerto, ruta, mercancÃ­a solicitada y observaciones del correo antes de iniciar el servicio.',
    cargo:surveyService?'Confirma que el responsable estÃ¡ en ruta o a bordo y registra evidencia inicial del servicio.':inWarehouse?'Confirma que la mercancÃ­a recibida coincide con fotos, cantidades y peso.':'Confirma la recogida en el punto indicado y comprueba cantidades y estado.',
    documents:surveyService?'Adjunta orden de servicio, instrucciones de muestreo, formularios o cualquier documento recibido del cliente.':'Comprueba packing list, CMR, delivery note y documento aduanero. Puedes adjuntar aquÃ­ los archivos recibidos por correo.',
    assignment:'Selecciona el conductor en el transporte o calendario. Este paso se completarÃ¡ automÃ¡ticamente.',
    delivery:surveyService?'Confirma que el Ballast Water Samples / survey quedÃ³ realizado. Adjunta fotos, observaciones o informe; no requiere POD.':storageOnly?'Confirma la salida de almacÃ©n o recogida por tercero con foto/evidencia clara. No hace falta POD.':'Revisa todo el almacÃ©n de este buque, adjunta fotografÃ­as de la mercancÃ­a entregada y el POD firmado.'
  };
  const uploadEvidence=async(files,evidenceType)=>{
    const selected=[...files].filter(Boolean);
    if(!selected.length)return;
  ...
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}>
    <section className="modal operation-modal">
      ...
      <div className="operation-modal-body">
        ...
        <div className="operation-guidance"><ClipboardCheck/><div><b>QuÃ© debes comprobar</b><p>{guidance[step.key]}</p>{step.key==='delivery'&&transport&&<small>{transport.id}  -  {transport.ruta}</small>}</div></div>
        ...
        {step.key==='documents'&&<div className="pod-scanner shipment-document-upload">
          <div><FileCheck2/><span><b>Adjuntar documentaciÃ³n del envÃ­o</b><small>Packing list, delivery note, CMR, T1, levante u otros documentos.</small></span></div>
          <div className="pod-scanner-actions"><label className="button primary"><UploadCloud/> {uploading?'Subiendoâ€¦':'AÃ±adir varios PDFs'}<input type="file" accept="application/pdf,image/*" multiple disabled={uploading} onChange={event=>{uploadEvidence(event.target.files,'shipment-document');event.target.value=''}}/></label></div>
        ...
        {needsEvidence&&<div className="pod-scanner evidence-capture">
          <div><Camera/><span><b>{step.key==='cargo'?(surveyService?'Evidencia inicial del survey':'Fotos de la mercancÃ­a recibida'):surveyService?'Evidencia del survey realizado':storageOnly?'Evidencias de salida / recogida':'Evidencias de la entrega'}</b><small>{step.key==='cargo'?(surveyService?'Opcional: foto de acceso, equipo o nota inicial.':'Puedes tomar varias fotografÃ­as.'):surveyService?'Foto, informe o documento del servicio realizado.':storageOnly?'Se exige al menos una foto o evidencia de salida.':'Se exige al menos una foto de entrega y un POD.'}</small></span></div>
          <div className="pod-scanner-actions">
            <label className="button primary"><Camera/> {uploading?'Procesandoâ€¦':step.key==='cargo'?'AÃ±adir fotos de recepciÃ³n':'AÃ±adir fotos de entrega'}<input type="file" accept="image/*" capture="environment" multiple disabled={uploading} onChange={event=>{uploadEvidence(event.target.files,step.key==='cargo'?'cargo-photo':'delivery-photo');event.target.value=''}}/></label>
            {step.key==='delivery'&&!storageOnly&&!surveyService&&<><label className="button secondary"><ScanLine/> Escanear POD automÃ¡tico<input type="file" accept="image/*" capture="environment" multiple disabled={uploading} onChange={event=>{uploadEvidence(event.target.files,'pod');event.target.value=''}}/></label><label className="button tertiary"><FileText/> AÃ±adir PDFs de POD<input type="file" accept="application/pdf" multiple disabled={uploading} onChange={event=>{uploadEvidence(event.target.files,'pod');event.target.value=''}}/></label></>}
            {step.key==='delivery'&&surveyService&&<label className="button tertiary"><FileText/> AÃ±adir informe / PDF<input type="file" accept="application/pdf,image/*" multiple disabled={uploading} onChange={event=>{uploadEvidence(event.target.files,'pod');event.target.value=''}}/></label>}
          </div>
          {step.key==='delivery'&&!storageOnly&&!surveyService&&<label className={'document-switch pod-exception '+(podException?'checked':'')}><input type="checkbox" checked={podException} onChange={event=>setPodException(event.target.checked)}/><CircleAlert/><span><b>POD no sellado / no disponible</b><small>Permite cerrar la entrega dejando una observaciÃ³n obligatoria para facturaciÃ³n.</small></span></label>}
          {step.key==='delivery'&&<div className="evidence-requirements"><span className={deliveryPhotos.length?'done':''}><CheckCircle2/> {surveyService?'Foto / evidencia':storageOnly?'Evidencia de salida':'Foto de entrega'} {deliveryPhotos.length?`(${deliveryPhotos.length})`:'pendiente'}</span>{!storageOnly&&!surveyService&&<span className={podFiles.length||podException?'done':''}><CheckCircle2/> {podException?'POD no sellado justificado':'POD firmado'} {podFiles.length?`(${podFiles.length})`:podException?'con observaciÃ³n':'pendiente'}</span>}{surveyService&&<span className={podFiles.length?'done':''}><CheckCircle2/> Informe / documento {podFiles.length?`(${podFiles.length})`:'opcional'}</span>}</div>}
          {evidenceFiles.length>0&&<div className="evidence-file-list">{evidenceFiles.map((file,index)=><a className="pod-uploaded" href={file.url} target="_blank" rel="noreferrer" key={`${file.id}-${index}`}><CheckCircle2/><span><b>{file.evidenceType==='pod'?(surveyService?'Informe / documento':'POD escaneado'):file.evidenceType==='delivery-photo'?(surveyService?'Evidencia survey':'Foto de entrega'):'Foto de recepciÃ³n'}</b><small>{file.name}</small></span><ExternalLink/></a>)}</div>}
          {error&&<p className="form-error"><CircleAlert/>{error}</p>}
        </div>}
        <label className="field"><span>{noteRequiredForPodException?'ObservaciÃ³n obligatoria':'ObservaciÃ³n (opcional)'}</span><input value={note} onChange={event=>setNote(event.target.value)} placeholder={noteRequiredForPodException?'Ej. El buque/consignatario no sellÃ³ el POD, entrega realizada sin selloâ€¦':'Incidencias, persona que recibe, referenciaâ€¦'}/></label>
        <div className="modal-actions"><button className="button tertiary" onClick={close}>Cancelar</button><button className="button primary" disabled={uploading||!evidenceReady} onClick={()=>submit(step.key,note,step.key==='cargo'?{files:evidenceFiles,warehouseRefs:surveyService?[]:selectedWarehouseRefs}:{files:evidenceFiles,podException,podExceptionReason:note})}><CheckCircle2/> {!evidenceReady?(step.key==='cargo'?'AÃ±ade una foto o selecciona almacÃ©n':surveyService?'AÃ±ade evidencia o informe del survey':warehouseReviewMissing?'Marca que revisaste el almacÃ©n':storageOnly?'AÃ±ade evidencia de salida':podException?'Escribe la observaciÃ³n del POD no sellado':'Revisa almacÃ©n, foto de entrega y POD'):'Confirmar paso'}</button></div>
      </div>
    </section>
  </div>;
}
function OperationStepModalLegacy({item,warehouseEntries,transports,csrfToken,close,submit}){
  ...
  const guidance={
    review:'Comprueba buque, ETA, puerto, ruta, mercancÃ­a solicitada y observaciones del correo antes de iniciar el servicio.',
    cargo:inWarehouse?'Confirma que la mercancÃ­a recibida coincide con fotos, cantidades y peso.':'Confirma la recogida en el punto indicado y comprueba cantidades y estado.',
    documents:'Comprueba packing list, CMR, delivery note y documento aduanero cuando corresponda.',
    delivery:'Entrega toda la mercancÃ­a y adjunta el POD firmado. La salida quedarÃ¡ archivada y el expediente pasarÃ¡ a FacturaciÃ³n.'
  };
  const uploadEvidence=async file=>{if(!file)return;setUploading(true);setError('');try{const prepared=step.key==='delivery'&&file.type.startsWith('image/')?await scannedPodPdf(file,item.id):file;const uploaded=await uploadAttachment(prepared,prepared.type==='application/pdf'?'document':'photo',csrfToken);setEvidenceFiles(current=>step.key==='cargo'?[...current,uploaded]:[uploaded])}catch(reason){setError(reason.message)}finally{setUploading(false)}};
  const needsEvidence=['cargo','delivery'].includes(step.key);
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal operation-modal"><div className="modal-head"><div><span className="overline">Paso operativo</span><h2>{step.title}</h2><p>{item.id}  -  {item.buque}</p></div><button className="icon-button" onClick={close}><X/></button></div><div className="operation-modal-body"><OperationChecklist item={item}/><div className="operation-guidance"><ClipboardCheck/><div><b>QuÃ© debes comprobar</b><p>{guidance[step.key]}</p>{step.key==='delivery'&&transport&&<small>{transport.id}  -  {transport.ruta}</small>}</div></div>{needsEvidence&&<div className="pod-scanner"><div><Camera/><span><b>{step.key==='cargo'?'Fotografiar mercancÃ­a recibida':'Scanner automÃ¡tico de POD'}</b><small>{step.key==='cargo'?'La foto es obligatoria y quedarÃ¡ en el expediente.':'Haz una foto del POD; Swiftport lo recorta, limpia y guarda como PDF.'}</small></span></div><div className="pod-scanner-actions"><label className="button primary"><Camera/> {uploading?'Escaneandoâ€¦':step.key==='cargo'?'Hacer foto':'Escanear con cÃ¡mara'}<input type="file" accept="image/*" capture="environment" disabled={uploading} onChange={event=>uploadEvidence(event.target.files?.[0])}/></label>{step.key==='delivery'&&<label className="button tertiary"><FileText/> Adjuntar PDF<input type="file" accept="application/pdf" disabled={uploading} onChange={event=>uploadEvidence(event.target.files?.[0])}/></label>}</div>{evidenceFiles.length>0&&<div className="evidence-file-list">{evidenceFiles.map((file,index)=><a className="pod-uploaded" href={file.url} target="_blank" rel="noreferrer" key={file.id}><CheckCircle2/><span><b>{step.key==='cargo'?`Foto ${index+1}`:'POD escaneado en PDF'}</b><small>{file.name}</small></span><ExternalLink/></a>)}</div>}{error&&<p className="form-error"><CircleAlert/>{error}</p>}</div>}<label className="field"><span>ObservaciÃ³n (opcional)</span><input value={note} onChange={event=>setNote(event.target.value)} placeholder="Incidencias, persona que recibe, referenciaâ€¦"/></label><div className="modal-actions"><button className="button tertiary" onClick={close}>Cancelar</button><button className="button primary" disabled={uploading||(needsEvidence&&!evidenceFiles.length)} onClick={()=>submit(step.key,note,step.key==='cargo'?evidenceFiles:evidenceFiles[0]||null)}><CheckCircle2/> {needsEvidence&&!evidenceFiles.length?(step.key==='cargo'?'Haz una foto':'Escanea el POD'):'Confirmar paso'}</button></div></div></section></div>;
}
function MerchandisePanel({item,updateCase,deleteAttachment}){
  ...
  return <><SectionHeader title="MercancÃ­a y documentaciÃ³n" subtitle={`${total} unidades  -  POD ${podLabel}`}/><div className="global-documents"><label className="field"><span>Documento aduanero</span><select value={documentation.alcance} onChange={event=>updateDocumentation({alcance:event.target.value})}><option value="individual">INDIVIDUAL POR MERCANCÃA</option><option value="global">UNO PARA TODO EL EXPEDIENTE</option></select></label>{documentation.alcance==='global'&&<><label className="field"><span>Tipo</span><select value={documentation.tipoAduanero} onChange={event=>updateDocumentation({tipoAduanero:event.target.value})}><option value="">SIN ASIGNAR</option><option>T1</option><option>LEVANTE ADUANERO</option></select></label><label className={'document-switch '+(documentation.aduaneroDisponible?'checked':'')}><input type="checkbox" checked={documentation.aduaneroDisponible} onChange={event=>updateDocumentation({aduaneroDisponible:event.target.checked})}/><FileCheck2/><span><b>DOCUMENTO ADUANERO</b><small>{documentation.aduaneroDisponible?'DISPONIBLE':'PENDIENTE'}</small></span></label></>}<label className={'document-switch pod locked '+(documentation.podDisponible?'checked':'')}><input type="checkbox" checked={documentation.podDisponible} disabled readOnly/><ClipboardCheck/><span><b>POD CONJUNTO</b><small>{documentation.podDisponible?(documentation.podNoSellado?'ENTREGA CERRADA CON OBSERVACIÃ“N':'RECIBIDO  -  LISTO PARA FACTURAR'):'SE REGISTRA EN EL FLUJO OPERATIVO'}</small></span></label></div><div className="merchandise-list">{merchandise.map((piece,index)=><details className="merchandise-item" key={piece.id}><summary><span className="box-icon"><Box/></span><span><b>{piece.buque||item.buque||'BUQUE SIN INDICAR'}</b><small>{piece.cantidad} {piece.tipo}{piece.cantidad===1?'':'S'}  -  {piece.peso||'PESO PENDIENTE'}{piece.sourceEntry?`  -  ${piece.sourceEntry}`:''}{piece.seguimiento?`  -  TRACKING: ${piece.seguimiento}`:''}</small></span><span className="document-count">{documentation.alcance==='global'?'DOC GLOBAL':`${(piece.documentos||[]).length}/2 DOCS`}</span><ChevronRight/></summary><div className="merchandise-editor"><label className="field"><span>Tipo</span><select value={piece.tipo} onChange={event=>updatePiece(piece.id,{tipo:event.target.value})}><option>CAJA</option><option>PALLET</option><option>SOBRE</option><option>PAQUETE</option><option>BULTO</option></select></label><label className="field"><span>Cantidad</span><input type="number" min="1" value={piece.cantidad} onChange={event=>updatePiece(piece.id,{cantidad:Number(event.target.value)||1})}/></label><label className="field"><span>Peso del grupo (kg)</span><input type="number" min="0.1" step="0.1" value={String(piece.peso||'').replace(/[^\d,.]/g,'').replace(',','.')} onChange={event=>updatePiece(piece.id,{peso:event.target.value?`${event.target.value} KG`:''})}/></label><label className="field"><span>N.Âº seguimiento (opcional)</span><input value={piece.seguimiento||''} onChange={event=>updatePiece(piece.id,{seguimiento:event.target.value.toUpperCase()})}/></label>{documentation.alcance==='individual'&&<div className="piece-documents"><span>Documento aduanero individual</span>{DOC_TYPES.map(document=><label key={document} className={(piece.documentos||[]).includes(document)?'checked':''}><input type="checkbox" checked={(piece.documentos||[]).includes(document)} onChange={()=>toggleDocument(piece,document)}/><FileCheck2/><b>{document}</b><small>{(piece.documentos||[]).includes(document)?'DISPONIBLE':'PENDIENTE'}</small></label>)}</div>}</div></details>)}</div><ReceptionRecords records={item.recepciones||[]} item={item} deleteAttachment={deleteAttachment}/></>;
}
function ReceptionRecords({records,item,deleteAttachment}){
  if(!records.length)return <div className="reception-empty"><Camera/><span><b>Sin recepciones documentadas</b><small>Las fotos y documentos aparecerÃ¡n aquÃ­ al registrar la entrada.</small></span></div>;
  return <section className="reception-records"><div className="reception-title"><Camera/><div><h3>Recepciones de mercancÃ­a</h3><p>Evidencias fotogrÃ¡ficas identificadas y documentos de llegada.</p></div></div>{records.map(record=><article className="reception-record" key={record.ref}><header><div><b>{formatReceptionDate(record.fecha)}</b><small>{record.ref}  -  ZONA {record.zona}</small></div><Badge>{(record.fotos||[]).length} FOTOS  -  {(record.documentos||[]).length} DOCS</Badge></header>{Boolean((record.fotos||[]).length)&&<div className="reception-photos">{record.fotos.map((file,index)=><figure key={file.id||file.url||index}><div className="reception-photo-frame"><a href={file.url} target="_blank" rel="noreferrer" title={file.name}><img src={file.url} alt={`${file.tipo||'Vista general'}  -  ${file.mercancia||'RecepciÃ³n completa'}`}/><span>FOTO {String(index+1).padStart(2,'0')}</span></a>{deleteAttachment&&<button type="button" className="icon-button danger attachment-delete floating" aria-label="Eliminar foto" onClick={()=>deleteAttachment(item.id,'reception-photo',file,record.ref)}><Trash2/></button>}</div><figcaption><b>{file.tipo||'VISTA GENERAL'}</b><strong>{file.mercancia||'RECEPCIÃ“N COMPLETA'}</strong>{file.nota&&<small>{file.nota}</small>}</figcaption></figure>)}</div>}{Boolean((record.documentos||[]).length)&&<div className="reception-documents">{record.documentos.map((file,index)=><div className="attachment-row" key={file.id||file.url||index}><a href={file.url} target="_blank" rel="noreferrer"><FileText/><span><b>{documentLabel(file.name)}</b><small>{file.name}</small></span><ExternalLink/></a>{deleteAttachment&&<button type="button" className="icon-button danger attachment-delete" aria-label="Eliminar documento de recepciÃ³n" onClick={()=>deleteAttachment(item.id,'reception-document',file,record.ref)}><Trash2/></button>}</div>)}</div>}</article>)}</section>;
}
...
function VesselModal({item,close,submit}){
  ...
  const update=event=>setForm({...form,[event.target.name]:event.target.value});
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal vessel-modal" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="overline">Ficha de buque</span><h2>{item.name?'Editar buque':'Nuevo buque'}</h2><p>Esta informaciÃ³n se reutiliza al crear expedientes y para seguimiento AIS.</p></div><button className="icon-button" onClick={close}><X/></button></div><form onSubmit={event=>{event.preventDefault();submit(form)}}><label className="field wide"><span>Nombre del buque *</span><input name="name" value={form.name} onChange={update} required autoFocus/></label><label className="field"><span>IMO</span><input name="imo" inputMode="numeric" maxLength="7" value={form.imo} onChange={update} placeholder="7 dÃ­gitos"/></label><label className="field"><span>MMSI</span><input name="mmsi" inputMode="numeric" maxLength="9" value={form.mmsi} onChange={update} placeholder="9 dÃ­gitos"/></label><label className="field wide"><span>Puerto habitual / Ãºltimo puerto</span><input name="lastPort" value={form.lastPort} onChange={update} placeholder="Ej. SAGUNTO"/></label><label className="field wide"><span>Foto del buque (URL opcional)</span><input name="photoUrl" value={form.photoUrl} onChange={update} placeholder="Pega aquÃ­ una URL de imagen del buque"/></label><div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Guardar ficha</button></div></form></section></div>;
}
function Transportes({items,update,openCase,team,providers,saveProvider}){
  ...
  return <><section className="provider-strip panel"><SectionHeader title="Proveedores de transporte" subtitle="Empresas disponibles para asignar servicios" action={<button className="button secondary" onClick={()=>setProviderOpen(true)}><Plus/> AÃ±adir proveedor</button>}/><div>{providers.filter(item=>item.activo!==false).map(provider=><span key={provider.id}><Truck/><b>{provider.nombre}</b><small>{provider.contacto||'Sin contacto'}</small></span>)}</div></section><section className="module-toolbar"><div className="filter-chips">{['Todos','En ruta','Asignado','Sin asignar','Entregado'].map(value=><button key={value} className={filter===value?'active':''} onClick={()=>setFilter(value)}>{value}</button>)}</div></section><section className="transport-grid">{visible.map(item=>{const provider=providers.find(entry=>entry.id===item.proveedorId);const route=routeParts(item);return <article className="transport-card" key={item.id}><div className="transport-head"><span className={'transport-icon '+statusTone(item.estado)}><Truck/></span><div><small>{item.id}  -  {item.expediente}</small><Badge>{item.estado}</Badge></div><button className="icon-button compact" aria-label={'Editar '+item.id} onClick={()=>setEditing(item)}><PencilLine/></button></div><div className="transport-route-detail"><span><MapPin/><small>LUGAR DE RECOGIDA</small><b>{route.origen}</b></span><i><ChevronRight/></i><span><Navigation/><small>LUGAR DE ENTREGA</small><b>{route.destino}</b></span></div><div className="transport-provider">{provider?.nombre||'Proveedor sin asignar'}</div><div className="transport-info"><span><Clock3/><small>Horario</small><b>{item.hora}</b></span><span><UserRound/><small>Conductor</small><b>{item.conductor}</b></span><span><Navigation/><small>VehÃ­culo</small><b>{item.vehiculo}</b></span></div><div className="card-actions"><button className="button tertiary" onClick={()=>openCase(item.expediente)}>Ver expediente</button><button className="button primary" onClick={()=>setEditing(item)}>{item.estado==='Sin asignar'?'Asignar servicio':'Editar transporte'}</button></div></article>})}</section>{editing&&<TransportEditModal item={editing} team={team} providers={providers} close={()=>setEditing(null)} submit={item=>{update(item);setEditing(null)}}/>}{providerOpen&&<ProviderModal close={()=>setProviderOpen(false)} submit={item=>{saveProvider(item);setProviderOpen(false)}}/>}</>;
}
function Aduanas({items,update,openCase,notify}){
  const [editing,setEditing]=useState(null);
  return <><section className="alert-banner"><CircleAlert/><div><b>{items.filter(item=>item.estado==='Pendiente').length} trÃ¡mite requiere atenciÃ³n</b><p>Revisa los documentos pendientes y sus fechas lÃ­mite.</p></div></section><section className="panel"><SectionHeader title="TrÃ¡mites aduaneros" subtitle="DUA, T1, T2L y levantes vinculados a expedientes"/><div className="customs-grid">{items.map(item=><article className="custom-card" key={item.id}><div className="custom-card-top"><span className="doc-icon"><FileCheck2/></span><div><small>{item.id}  -  {item.expediente}</small><h3>{item.tipo}</h3></div><Badge>{item.estado}</Badge></div><dl><div><dt>Referencia</dt><dd>{item.referencia}</dd></div><div><dt>Fecha lÃ­mite</dt><dd>{item.limite}</dd></div></dl><p>{item.nota}</p><div className="card-actions"><button className="button tertiary" onClick={()=>openCase(item.expediente)}>Ver expediente</button><button className="button secondary" onClick={()=>setEditing(item)}><PencilLine/> Editar</button></div></article>)}</div></section>{editing&&<CustomEditModal item={editing} close={()=>setEditing(null)} submit={item=>{update(item);setEditing(null)}}/>}</>;
}
function Clientes({notify,clients,updateClient}){
  ...
  return <><section className="panel clients-directory-panel"><SectionHeader title="Fichas de clientes" subtitle="Datos fiscales, contactos, condiciones de pago y tarifas por cliente" action={<button className="button secondary" onClick={()=>setEditing(normalizeClientProfile({nombre:'',codigo:'CLI-'+Date.now()}))}><Plus/> Nuevo cliente</button>}/><label className="search-box standalone"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar cliente, NIF, contacto o tarifaâ€¦"/></label><div className="client-file-grid">{visible.map(item=><article className="client-file-card" key={item.codigo}><header><span className="client-avatar">{item.nombre.split(' ').map(word=>word[0]).slice(0,2).join('')}</span><div><small>{item.codigo}</small><h3>{item.nombre||'Cliente sin nombre'}</h3><p>{item.fiscalName}</p></div><button className="icon-button" aria-label={'Editar '+item.nombre} onClick={()=>setEditing(item)}><PencilLine/></button></header><div className="client-file-meta"><span><small>NIF / VAT</small><b>{item.taxId||'Pendiente'}</b></span><span><small>Condiciones</small><b>{item.condicionesPago}</b></span><span><small>Moneda</small><b>{item.moneda}</b></span><span><small>Expedientes</small><b>{item.expedientes}</b></span></div><div className="client-contact-line"><Mail/><a href={item.contacto?`mailto:${item.contacto}`:'#'}>{item.contacto||'Email pendiente'}</a>{item.telefono&&<small>{item.telefono}</small>}</div><div className="client-address"><MapPin/><span>{item.direccion||'DirecciÃ³n fiscal pendiente'}</span></div><div className="client-rate-box"><b>{item.tarifaActiva}</b><span><small>RecepciÃ³n</small>{item.recepcion}</span><span><small>Storage</small>{item.storage}</span><span><small>Transporte</small>{item.transporte}</span><span><small>Recargo</small>{item.recargo}</span></div><button className="button tertiary full" onClick={()=>setEditing(item)}>Editar ficha completa <PencilLine/></button></article>)}</div>{!visible.length&&<Empty text="No hay clientes con esa bÃºsqueda."/>}</section>{editing&&<ClientEditModal item={editing} close={()=>setEditing(null)} submit={item=>{updateClient(item);setEditing(null)}}/>}</>;
}
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],transports=[],calendarEvents=[],clients=[],updateInvoice,syncInvoices,csrfToken,currentUser}){
  ...
  const invoiceCaseReady=item=>{
    ...
    return Boolean(flow.billingReady||related.estado==='Completado'||related.progreso>=100);
  };
  ...
  const invoiceMarginOf=item=>invoiceRevenue(item)-invoiceCostOf(item);
  ...
  const archiveInvoice=item=>{
    if(!canAdmin){notify('Solo un administrador puede archivar facturaciÃ³n.');return}
    updateInvoice({...item,estado:'Archivado',archivedAt:new Date().toISOString()});
    notify(`${item.expediente} archivado de facturaciÃ³n`);
  };
  const archiveReadyCase=item=>{
    if(!canAdmin){notify('Solo un administrador puede archivar facturaciÃ³n.');return}
    const draft=draftFromCase(item);
    updateInvoice({...draft,estado:'Archivado',archivedAt:new Date().toISOString()});
    notify(`${item.id} archivado de facturaciÃ³n`);
  };
  ...
  const sendHolded=item=>{
    if(!csrfToken){notify('Inicia sesiÃ³n en la web publicada para enviar a Holded.');return}
...
const MAIL_ACTION_LABELS={new:'NUEVO SERVICIO',update:'ACTUALIZACIÃ“N',cancel:'CANCELACIÃ“N',information:'INFORMATIVO',not_service:'NO OPERATIVO'};
const MAIL_SERVICE_LABELS={reception:'RECEPCIÃ“N',pickup:'RECOGIDA',delivery:'ENTREGA',reception_and_delivery:'RECEPCIÃ“N + ENTREGA',customs:'ADUANAS',other:'OTRO SERVICIO',none:'SIN SERVICIO'};
function Correos({csrfToken,notify,openCase,reloadOperational,canRebuild}){
  ...
  const process=async()=>{
    ...
    try{
      ...
      const synced=0;
      notify(`${summary.scanned} correos nuevos  -  ${summary.processed} trabajos creados  -  ${summary.review} para revisar${synced?`  -  ${synced} trabajos al calendario`:''}${removedInvalid?`  -  ${removedInvalid} invÃ¡lidos retirados`:''}${removedOld?`  -  ${removedOld} antiguos retirados`:''}${repaired?`  -  ${repaired} duplicados corregidos`:''}`);
      await Promise.all([load(filter),reloadOperational()]);
    }catch(reason){setError(reason.message)}
    finally{setProcessing(false)}
  };
  const rebuild=async()=>{
    setRebuilding(true);setError('');setRebuildProgress('Preparando reconstrucciÃ³nâ€¦');
    try{
      ...
      if(!window.confirm(`Se borrarÃ¡n ${preview.caseCount} expedientes, incluidos los completados, y toda su operativa. Se guardarÃ¡ una copia de seguridad y se reinterpretarÃ¡n ${preview.mailCount} correos de junio. Â¿Continuar?`)){setRebuildProgress('');return}
  ...
  useEffect(()=>{
    ...
    button.textContent=rebuilding?(rebuildProgress||'Creando prueba automÃ¡ticaâ€¦'):'Crear todo automÃ¡ticamente';
  ...
  const reprocess=async item=>{
    try{const result=await api('/api/mail/review.php',{method:'PUT',headers:{'X-CSRF-Token':csrfToken},body:JSON.stringify({id:item.id,action:'reprocess'})});notify(result.status==='processed'?`Trabajo creado automÃ¡ticamente: ${result.caseRef}`:result.status==='review'?'Servicio detectado, pero faltan datos en el correo':'El correo sigue sin datos operativos suficientes');if(result.status==='processed')await reloadOperational();setFilter(result.status);load(result.status)}
    catch(reason){setError(reason.message)}
  };
  ...
  return <><section className="mail-automation-hero"><div><Mail/><span><b>Entrada automÃ¡tica con IA</b><small>Los servicios nuevos crean el trabajo; cambios y dudas esperan revisiÃ³n.</small></span></div><div><small>Ãšltima comprobaciÃ³n</small><b>{lastRun?.finished_at&&formatReceptionDate(lastRun.finished_at)||'TodavÃ­a no ejecutada'}</b></div><div className="mail-hero-actions"><button className="button primary" disabled={processing||rebuilding} onClick={process}><RefreshCw className={processing?'spinning':''}/>{processing?'Leyendo buzonesâ€¦':'Comprobar correos ahora'}</button></div></section>{error&&<div className="form-error"><CircleAlert/>{error}</div>}<section className="panel"><SectionHeader title="Bandeja de servicios" subtitle="info@swiftportlogistic.com y operations@swiftportlogistic.com"/><div className="mail-filters">{[['all','Todos',Object.values(counts).reduce((a,b)=>a+b,0)],['review','Revisar',counts.review],['processed','Creados',counts.processed],['ignored','Descartados',counts.ignored],['error','Errores',counts.error]].map(([value,label,total])=><button key={value} className={filter===value?'active':''} onClick={()=>setFilter(value)}>{label}<span>{total}</span></button>)}</div>{loading?<div className="users-loading">Cargando correosâ€¦</div>:items.length?<div className="mail-list">{items.map(item=><article key={item.id} className={'mail-item '+item.status}><header><div><b>{item.subject||'Sin asunto'}</b><small>{item.sender_name||item.sender_email}  -  {formatReceptionDate(item.received_at)}</small></div><Badge>{MAIL_STATUS[item.status]||item.status}</Badge></header>{item.extracted&&<><div className="mail-extracted"><span><small>BUQUE</small><b>{item.extracted.vessel||'â€”'}</b></span><span><small>ETB / ETA</small><b>{[item.extracted.etb||item.extracted.eta,item.extracted.etb_time||item.extracted.eta_time].filter(Boolean).join('  -  ')||'POR CONFIRMAR'}</b></span><span><small>PUERTO</small><b>{item.extracted.port||'POR CONFIRMAR'}</b></span><span><small>SERVICIO</small><b>{MAIL_SERVICE_LABELS[item.extracted.service_kind]||[item.extracted.reception?.required&&'RECEPCIÃ“N',item.extracted.transport?.required&&'TRANSPORTE'].filter(Boolean).join(' + ')||'â€”'}</b></span></div><div className="mail-ai-summary"><span>{MAIL_ACTION_LABELS[item.extracted.request_action]||'CLASIFICACIÃ“N ANTERIOR'}</span><b>{item.extracted.cargo_summary||item.extracted.operational_notes||'Sin resumen operativo'}</b>{item.extracted.operational_notes&&item.extracted.cargo_summary&&<small>{item.extracted.operational_notes}</small>}<em>Confianza {Math.round(Number(item.extracted.confidence||item.confidence||0)*100)}%</em></div>{item.extracted.tasks?.length>0&&<MailTaskProposal tasks={item.extracted.tasks}/>}</>}{item.review_reason&&<p className="mail-reason"><CircleAlert/>{item.review_reason}</p>}{item.error_message&&<p className="mail-reason error"><CircleAlert/>{item.error_message}</p>}<details className="mail-original"><summary>Ver correo original</summary><pre>{item.body}</pre></details><footer>{item.case_ref&&<button className="button tertiary" onClick={()=>openCase(item.case_ref)}>Abrir {item.case_ref}</button>}{['ignored','error'].includes(item.status)&&<button className="button secondary" onClick={()=>reprocess(item)}><RefreshCw/> Reinterpretar</button>}{item.status==='review'&&<><button className="button tertiary" onClick={()=>ignore(item)}>Descartar</button><button className="button secondary" onClick={()=>reprocess(item)}><RefreshCw/> Reinterpretar con IA</button><button className="button primary" onClick={()=>setEditing(item)}><PencilLine/> Revisar y crear</button></>}</footer></article>)}</div>:<Empty text="No hay correos en este estado."/>}</section>{editing&&<MailReviewModal item={editing} close={()=>setEditing(null)} submit={data=>approve(editing,data)}/>}</>;
}
function MailTaskProposal({tasks}){
  const labels={reception:'RecepciÃ³n',pickup:'Recogida',delivery:'Entrega',samples:'Muestras',crew_transport:'TripulaciÃ³n',other:'Otro'};
  return <div className="mail-task-proposal"><strong>PROPUESTA OPERATIVA</strong>{tasks.map((task,index)=><span key={index}><i>{index+1}</i><span><b>{labels[task.kind]||task.kind}  -  {[task.date,task.time].filter(Boolean).join(' ')||'Fecha pendiente'}</b><small>{task.pickup||'Origen pendiente'} â†’ {task.delivery||'Destino pendiente'}</small>{task.cargo&&<em>{task.cargo}</em>}</span><small>{Math.round(Number(task.confidence||0)*100)}%</small></span>)}</div>;
}
...
const AUDIT_LABELS={
  ...
  'finance.update':'FacturaciÃ³n actualizada',
  ...
  'warehouse.create':'Entrada de almacÃ©n creada',
  'warehouse.update':'Entrada de almacÃ©n editada',
  'warehouse.delete':'Entrada de almacÃ©n eliminada',
...
const rawAuditDetailsText=details=>{
  ...
  return entries.map(([key,value])=>`${key}: ${Array.isArray(value)?value.join(', '):typeof value==='object'?JSON.stringify(value):value}`).join(' Â· ');
};
...
const auditDetailsText=(details,action='')=>{
  ...
  if(action==='case.create')return auditJoin([`Creo ${caseText||'expediente'}`,vesselText,details.client&&`Cliente ${details.client}`,details.port&&`Puerto ${details.port}`,details.serviceType&&`Tipo ${details.serviceType}`,details.transportCreated!==undefined&&`Transporte ${details.transportCreated?'creado':'no creado'}`]);
  if(action==='case.update')return auditJoin([`Edito ${caseText||'expediente'}`,vesselText,details.client&&`Cliente ${details.client}`,details.port&&`Puerto ${details.port}`,details.eta&&`ETA ${details.eta}`,details.etb&&`ETB ${details.etb}`,details.etd&&`ETD ${details.etd}`]);
...
function Auditoria({notify}){
  ...
  return <section className="panel audit-panel">
    <SectionHeader title="Registro de actividad" subtitle="Solo administradores: entradas, borrados, cambios, archivos, facturaciÃ³n y accesos" action={<button className="button secondary" onClick={load} disabled={loading}>{loading?<RefreshCw className="spin"/>:<RefreshCw/>} Actualizar</button>}/>
    <div className="audit-toolbar"><label className="search-box standalone"><Search/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar usuario, acciÃ³n, expediente, IP o detalleâ€¦"/></label><div className="audit-kpis"><span><b>{visible.length}</b><small>movimientos</small></span><span><b>{important}</b><small>relevantes</small></span></div></div>
    {error&&<div className="form-error"><CircleAlert/>{error}</div>}
    {loading?<div className="users-loading">Cargando auditorÃ­aâ€¦</div>:<div className="audit-table">
      <div className="audit-head"><span>Fecha</span><span>Usuario</span><span>AcciÃ³n</span><span>Detalle</span><span>IP</span></div>
      {visible.map(item=><article className="audit-row" key={item.id}>
        <time>{item.createdAt?new Date(item.createdAt.replace(' ','T')).toLocaleString('es-ES'):'â€”'}</time>
...
function Usuarios({csrfToken,notify,onPreview,onUsersChanged}){
  ...
  const RoleChecks=({roles,toggle})=><div className="multi-role-selector">{Object.entries(ROLE_LABELS).map(([value,label])=><label className={roles.includes(value)?'checked':''} key={value}><input type="checkbox" checked={roles.includes(value)} onChange={()=>toggle(value)}/><CheckCircle2/><span><b>{label}</b><small>{value==='driver'?'Calendario, almacÃ©n y entregas':value==='operations'?'Expedientes, correos y planificaciÃ³n':value==='finance'?'Importes, tarifas y facturaciÃ³n':'Control total y usuarios'}</small></span></label>)}</div>;
  return <div className="users-layout">
    <section className="panel"><SectionHeader title="Equipo con acceso" subtitle="Una persona puede combinar varios roles y permisos"/>{error&&<div className="form-error users-error"><CircleAlert/>{error}</div>}{loading?<div className="users-loading">Cargando usuariosâ€¦</div>:<div className="user-list">{users.map(item=><article key={item.id}><div className="avatar">{initials(item.fullName)}</div><div className="user-identity"><b>{item.fullName}</b><small>{item.email}</small><em>{roleLabel(item)}</em></div><RoleChecks roles={rolesOf(item)} toggle={role=>toggleUserRole(item,role)}/><button className="button tertiary preview-user" onClick={()=>onPreview(item)}><Eye/> Ver como</button></article>)}</div>}</section>
    <section className="panel create-user"><SectionHeader title="AÃ±adir usuario" subtitle="Selecciona uno o varios roles"/><form onSubmit={submit}><label className="field"><span>Nombre completo</span><input name="fullName" value={form.fullName} onChange={update} required/></label><label className="field"><span>Email</span><input name="email" type="email" value={form.email} onChange={update} required/></label><label className="field"><span>ContraseÃ±a temporal</span><input name="password" type="password" minLength="4" value={form.password} onChange={update} required/></label><div className="field"><span>Roles y permisos</span><RoleChecks roles={form.roles} toggle={toggleFormRole}/></div><button className="button primary full" disabled={busy}><UserPlus/>{busy?'Creandoâ€¦':'Crear usuario'}</button></form></section>
  </div>;
}
function CaseEditModal({item,close,submit,vessels=[],clientOptions=[]}){
  const call=item.portCall||{};
  const legacyEta=String(item.eta||'').match(/^20\d{2}-\d{2}-\d{2}/)?.[0]||'';
  const [form,setForm]=useState({...item,serviceType:serviceTypeOf(item),imo:item.imo||'',mmsi:item.mmsi||'',servicios:(item.servicios||[]).join(', '),etaDate:call.etaDate||legacyEta,etaTime:call.etaTime||'',etbDate:call.etbDate||'',etbTime:call.etbTime||'',etdDate:call.etdDate||'',etdTime:call.etdTime||''});
...
function LegacyCaseEditModal({item,close,submit}){
  ...
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="overline">Expediente {item.id}</span><h2>Editar informaciÃ³n</h2><p>Los cambios se compartirÃ¡n con todos los usuarios.</p></div><button className="icon-button" onClick={close}><X/></button></div><form onSubmit={save}><label className="field"><span>Buque</span><input name="buque" value={form.buque} onChange={update} required/></label><label className="field"><span>Cliente</span><select name="cliente" value={form.cliente} onChange={update}>{selectableClients.map(name=><option key={name}>{name}</option>)}</select></label><label className="field"><span>Puerto</span><input name="puerto" value={form.puerto} onChange={update} required/></label><label className="field"><span>ETA  -  fecha</span><input name="etaDate" type="date" value={form.etaDate} onChange={update}/></label><label className="field"><span>ETA  -  hora</span><input name="etaTime" type="time" value={form.etaTime} onChange={update}/></label><label className="field"><span>ETB  -  fecha</span><input name="etbDate" type="date" value={form.etbDate} onChange={update}/></label><label className="field"><span>ETB  -  hora</span><input name="etbTime" type="time" value={form.etbTime} onChange={update}/></label><label className="field"><span>ETD  -  fecha</span><input name="etdDate" type="date" value={form.etdDate} onChange={update}/></label><label className="field"><span>ETD  -  hora</span><input name="etdTime" type="time" value={form.etdTime} onChange={update}/></label><label className="field"><span>Estado</span><select name="estado" value={form.estado} onChange={update}>{['Nuevo','En curso','Completado','Cancelado'].map(value=><option key={value}>{value}</option>)}</select></label><label className="field"><span>Prioridad</span><select name="prioridad" value={form.prioridad} onChange={update}>{['Baja','Media','Alta','Urgente'].map(value=><option key={value}>{value}</option>)}</select></label><label className="field"><span>Bultos</span><input name="bultos" type="number" min="0" value={form.bultos} onChange={update}/></label><label className="field"><span>Peso</span><input name="peso" value={form.peso} onChange={update}/></label><label className="field"><span>Progreso (%)</span><input name="progreso" type="number" min="0" max="100" value={form.progreso} onChange={update}/></label><label className="field"><span>Siguiente acciÃ³n</span><input name="siguiente" value={form.siguiente} onChange={update}/></label><label className="field wide"><span>Servicios (separados por comas)</span><input name="servicios" value={form.servicios} onChange={update}/></label><div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Guardar cambios</button></div></form></section></div>;
}
function TransportEditModal({item,team,providers,close,submit}){
  ...
  const save=event=>{event.preventDefault();const estado=form.conductor==='Sin asignar'?'Sin asignar':form.estado==='Sin asignar'?'Asignado':form.estado;submit({...form,ruta:`${form.origen.trim()} â†’ ${form.destino.trim()}`,estado})};
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal transport-edit-modal" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="overline">{item.id}</span><h2>Editar recorrido</h2><p>Indica libremente dÃ³nde se recoge y dÃ³nde se entrega.</p></div><button className="icon-button" onClick={close}><X/></button></div><form onSubmit={save}><label className="field wide"><span>Lugar de recogida</span><input name="origen" value={form.origen} onChange={update} placeholder="Ej. ALMACÃ‰N SWIFTPORT, TECHNYMON, AEROPUERTOâ€¦" required/></label><label className="field wide"><span>Lugar de entrega</span><input name="destino" value={form.destino} onChange={update} placeholder="Ej. BUQUE, ALMACÃ‰N, EMPRESA X (BILBAO)â€¦" required/></label><div className="route-preview wide"><MapPin/><span><small>RECORRIDO</small><b>{form.origen||'ORIGEN'} â†’ {form.destino||'DESTINO'}</b></span></div><label className="field"><span>Fecha</span><input name="fecha" type="date" value={form.fecha} onChange={update} required/></label><label className="field"><span>Hora inicio</span><input name="inicio" type="time" value={form.inicio} onChange={update} required/></label><label className="field"><span>Hora fin</span><input name="fin" type="time" value={form.fin} onChange={update} required/></label><label className="field"><span>Estado</span><select name="estado" value={form.estado} onChange={update}>{['Sin asignar','Asignado','En ruta','Entregado'].map(value=><option key={value}>{value}</option>)}</select></label><label className="field"><span>Conductor</span><select name="conductor" value={form.conductor} onChange={update}><option>Sin asignar</option>{team.filter(member=>hasRole(member,'operations')||hasRole(member,'driver')).map(member=><option key={member.id} value={member.fullName}>{member.fullName}</option>)}</select></label><label className="field"><span>Proveedor</span><select name="proveedorId" value={form.proveedorId||''} onChange={update}><option value="">Sin proveedor</option>{providers.filter(provider=>provider.activo!==false).map(provider=><option key={provider.id} value={provider.id}>{provider.nombre}</option>)}</select></label><label className="field"><span>VehÃ­culo / matrÃ­cula</span><input name="vehiculo" value={form.vehiculo} onChange={update}/></label><div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Guardar recorrido</button></div></form></section></div>;
}
function ProviderModal({close,submit}){
  const [form,setForm]=useState({id:'',nombre:'',contacto:'',telefono:'',activo:true});
  const update=event=>setForm({...form,[event.target.name]:event.target.value});
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="overline">Proveedores</span><h2>AÃ±adir empresa de transporte</h2><p>QuedarÃ¡ disponible en Calendario y Transportes.</p></div><button className="icon-button" onClick={close}><X/></button></div><form onSubmit={event=>{event.preventDefault();submit(form)}}><label className="field wide"><span>Empresa</span><input name="nombre" value={form.nombre} onChange={update} required autoFocus/></label><label className="field"><span>Persona / departamento</span><input name="contacto" value={form.contacto} onChange={update}/></label><label className="field"><span>TelÃ©fono</span><input name="telefono" value={form.telefono} onChange={update}/></label><div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Guardar proveedor</button></div></form></section></div>;
}
function ClientEditModal({item,close,submit}){
  ...
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal client-profile-modal" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="overline">{form.codigo}</span><h2>Ficha de cliente</h2><p>Datos que se reutilizan en facturaciÃ³n y tarifas automÃ¡ticas.</p></div><button className="icon-button" onClick={close}><X/></button></div><form onSubmit={event=>{event.preventDefault();submit({...form,expedientes:Number(form.expedientes)||0})}}><div className="client-form-section wide"><b>Datos fiscales</b></div><label className="field"><span>Nombre comercial *</span><input name="nombre" value={form.nombre} onChange={update} required autoFocus/></label><label className="field"><span>RazÃ³n social</span><input name="fiscalName" value={form.fiscalName} onChange={update}/></label><label className="field"><span>NIF / VAT</span><input name="taxId" value={form.taxId} onChange={update}/></label><label className="field"><span>Expedientes activos</span><input name="expedientes" type="number" min="0" value={form.expedientes} onChange={update}/></label><label className="field wide"><span>DirecciÃ³n fiscal</span><input name="direccion" value={form.direccion} onChange={update}/></label><div className="client-form-section wide"><b>Contacto y pago</b></div><label className="field"><span>Email de contacto</span><input name="contacto" type="email" value={form.contacto} onChange={update}/></label><label className="field"><span>TelÃ©fono</span><input name="telefono" value={form.telefono} onChange={update}/></label><label className="field"><span>Condiciones de pago</span><input name="condicionesPago" value={form.condicionesPago} onChange={update}/></label><label className="field"><span>Moneda</span><input name="moneda" value={form.moneda} onChange={update}/></label><div className="client-form-section wide"><b>Tarifa de facturaciÃ³n</b><small>Por ahora la tarifa automÃ¡tica real estÃ¡ activa para LIMANI.</small></div><label className="field wide"><span>Tarifa activa</span><input name="tarifaActiva" value={form.tarifaActiva} onChange={update}/></label><label className="field wide"><span>RecepciÃ³n</span><input name="recepcion" value={form.recepcion} onChange={update}/></label><label className="field wide"><span>Storage</span><input name="storage" value={form.storage} onChange={update}/></label><label className="field wide"><span>Transporte</span><input name="transporte" value={form.transporte} onChange={update}/></label><label className="field wide"><span>Recargo fuera de horario / festivos</span><input name="recargo" value={form.recargo} onChange={update}/></label><label className="field wide"><span>Notas internas</span><input name="notas" value={form.notas} onChange={update} placeholder="Preferencias, emails habituales, excepcionesâ€¦"/></label><div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Guardar ficha</button></div></form></section></div>;
}
function InvoiceEditModal({item,cases=[],warehouseEntries=[],transports=[],calendarEvents=[],clients=[],close,submit,simulateHolded}){
  ...
  const applySupplierLinesFromText=(text,source='texto')=>{
    ...
    if(!lines.length){setSupplierNote(source==='PDF'?'PDF guardado, pero no pude leer conceptos claros. Si es escaneado/foto, pega el texto o usa OCR.':'No encontrÃ© conceptos claros. Pega lÃ­neas como â€œLoad / Unload 18.00â€ o â€œDelivery vessel Algeciras Port 65.00â€.');return false}
    ...
    setSupplierNote(`${lines.length} concepto(s) importados automÃ¡ticamente desde ${source} con tarifa Swiftport. Revisa los importes antes de enviar a Holded.`);
    return true;
  };
  const addSupplierFiles=async event=>{
    ...
    if(extractedText){
      ...
      setSupplierNote(`${files.length} PDF(s) guardados como evidencia. No pude extraer texto automÃ¡tico; pega los conceptos de la factura proveedor para aplicar tu tarifa.`);
    }
  ...
  const rescanSupplierFiles=()=>{
    const extracted=(form.supplierInvoices||[]).map(file=>file.text).filter(Boolean).join('\n\n');
    if(!extracted){setSupplierNote('Estos PDFs estaban guardados sin texto escaneado. Vuelve a subirlos con "Subir y escanear PDFs" para leerlos.');return}
    setSupplierText(extracted);
    applySupplierLinesFromText(extracted,'PDF guardado');
  };
...
function CustomEditModal({item,close,submit}){
  ...
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="overline">{item.id}</span><h2>Editar trÃ¡mite aduanero</h2></div><button className="icon-button" onClick={close}><X/></button></div><form onSubmit={event=>{event.preventDefault();submit(form)}}><label className="field"><span>Expediente</span><input name="expediente" value={form.expediente} onChange={update}/></label><label className="field"><span>Tipo</span><input name="tipo" value={form.tipo} onChange={update}/></label><label className="field"><span>Referencia</span><input name="referencia" value={form.referencia} onChange={update}/></label><label className="field"><span>Fecha lÃ­mite</span><input name="limite" value={form.limite} onChange={update}/></label><label className="field"><span>Estado</span><select name="estado" value={form.estado} onChange={update}>{['Pendiente','DocumentaciÃ³n','Liberado'].map(value=><option key={value}>{value}</option>)}</select></label><label className="field"><span>Nota</span><input name="nota" value={form.nota} onChange={update}/></label><div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Guardar trÃ¡mite</button></div></form></section></div>;
}
function transportCargoOptions(item,warehouseEntries=[]){
  ...
  const fromWarehouse=warehouseEntriesForVessel(warehouseEntries,item).flatMap((entry,entryIndex)=>{
    ...
    return goods.map((piece,pieceIndex)=>({
      ...
      entryRef:entry.ref||entry.id||'AlmacÃ©n',
...
function TransportCargoSelector({options,selectedIds,onChange}){
  ...
  if(!options.length)return <div className="cargo-selection-panel wide"><div className="cargo-selection-head"><span><b>MercancÃ­a de este viaje</b><small>No hay bultos/pallets vinculados al expediente todavÃ­a. Cuando se registren en almacÃ©n podrÃ¡s elegirlos aquÃ­.</small></span></div></div>;
  return <div className="cargo-selection-panel wide"><div className="cargo-selection-head"><span><b>MercancÃ­a de este viaje</b><small>Marca exactamente quÃ© cajas, pallets o bultos lleva este transporte.</small></span><div className="cargo-selection-actions"><button type="button" className="button mini secondary" onClick={()=>onChange(allSelected?[]:options.map(option=>option.id))}>{allSelected?'Limpiar selecciÃ³n':'Seleccionar todo'}</button></div></div><div className="cargo-option-list">{options.map(option=><label key={option.id} className={'cargo-option-card '+(selectedIds.includes(option.id)?'selected':'')}><input type="checkbox" checked={selectedIds.includes(option.id)} onChange={()=>toggle(option.id)}/><span><b>{option.summary}</b><small>{option.entryRef}  -  {option.detail}</small></span></label>)}</div></div>;
}
function CargoManifest({item,transport}){
  ...
  return <div className="driver-manifest wide"><div><Box/><span><b>CARGA PARA EL CONDUCTOR</b><small>{item.buque}  -  {item.puerto}</small></span></div>{selectedCargo.length>0&&<div className="manifest-selected-cargo"><PackageCheck/><span><small>CARGA SELECCIONADA PARA ESTE VIAJE</small>{selectedCargo.map(piece=><b key={piece.id||piece.ref}>{piece.summary}<em>{piece.entryRef}  -  {piece.detail}</em></b>)}</span></div>}{item.resumenMercancia&&<div className="manifest-email-brief"><PackageCheck/><span><small>MERCANCÃA INDICADA EN EL CORREO</small><b>{item.resumenMercancia}</b></span></div>}{item.notasOperativas&&<div className="manifest-email-brief notes"><FileText/><span><small>INSTRUCCIONES OPERATIVAS</small><b>{item.notasOperativas}</b></span></div>}{transport?.observacion&&<div className="manifest-email-brief notes driver-observation"><ClipboardCheck/><span><small>OBSERVACIÃ“N DEL TRANSPORTE</small><b>{transport.observacion}</b></span></div>}{item.referenciaCliente&&<div className="manifest-reference">Referencia: <b>{item.referenciaCliente}</b></div>}{(item.mercancias||[]).map(piece=><p key={piece.id}><b>{piece.buque||item.buque||'BUQUE SIN INDICAR'}  -  {piece.cantidad} {piece.tipo}{piece.cantidad===1?'':'S'}  -  {piece.peso||'PESO PENDIENTE'}</b><span>{piece.sourceEntry?`${piece.sourceEntry}  -  `:''}{piece.seguimiento?`Tracking: ${piece.seguimiento}`:'Sin seguimiento'}</span></p>)}{Boolean(photos.length||receptionDocuments.length)&&<div className="manifest-arrival-files"><Camera/><span><b>{photos.length} fotos  -  {receptionDocuments.length} documentos de llegada</b><small>Disponibles en el expediente</small></span></div>}<div className="manifest-next-step"><ClipboardCheck/><span><small>SIGUIENTE PASO</small><b>{nextStep?.title||'Operativa completada'}</b></span></div><footer><span>Aduanas: {documentation.alcance==='global'?(documentation.aduaneroDisponible?`${documentation.tipoAduanero} DISPONIBLE`:'PENDIENTE'):'DOCUMENTOS INDIVIDUALES'}</span><span>POD: {documentation.podDisponible?'DISPONIBLE':'PENDIENTE'}</span></footer></div>;
}
function CalendarEventModal({item,team,cases,transports,providers,warehouseEntries=[],close,submit:rawSubmit,openCase}){
  ...
  const [form,setForm]=useState({...item,...initialRoute,observacion:item.observacion||initialTransport?.observacion||'',tipoServicio:item.tipoServicio||(item.transporte?'Transporte':'RecepciÃ³n')});
  const update=event=>{
    if(event.target.name==='tipoServicio'){const type=event.target.value;const related=cases.find(entry=>entry.id===form.expediente);setForm({...form,tipoServicio:type,transporte:type==='RecepciÃ³n'?'':form.transporte,origen:type==='Transporte'?(form.origen||SWIFTPORT_WAREHOUSE):form.origen,destino:type==='Transporte'?(form.destino||`BUQUE ${related?.buque||''}  -  ${related?.puerto||''}`):form.destino});return}
  ...
  const saveWithCargo=event=>{event.preventDefault();const route=form.tipoServicio==='Transporte'?`${form.origen.trim()} â†’ ${form.destino.trim()}`:form.titulo;const selectedCargo=cargoOptions.filter(option=>selectedCargoIds.includes(option.id));submit({...form,titulo:route,selectedCargoIds,selectedCargo})};
  const save=event=>{event.preventDefault();const route=form.tipoServicio==='Transporte'?`${form.origen.trim()} â†’ ${form.destino.trim()}`:form.titulo;submit({...form,titulo:route})};
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="modal calendar-event-modal" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="overline">PlanificaciÃ³n</span><h2>{item.titulo?'Editar transporte':'Nuevo transporte'}</h2><p>Solo se agenda el transporte al buque/gabarra. Si falta hora ETB, puedes dejar la hora vacÃ­a.</p></div><button className="icon-button" onClick={close}><X/></button></div><form onSubmit={save}><label className="field"><span>Tipo de servicio</span><select name="tipoServicio" value={form.tipoServicio} onChange={update}><option>Transporte</option></select></label><label className="field"><span>Expediente / buque</span><select name="expediente" value={form.expediente} onChange={update} required><option value="">Seleccionar expediente</option>{cases.map(entry=><option key={entry.id} value={entry.id}>{caseLabel(entry)}</option>)}</select></label><label className="field"><span>Fecha</span><input name="fecha" type="date" value={form.fecha} onChange={update} required/></label><label className="field"><span>Conductor</span><select name="asignado" value={form.asignado} onChange={update} autoFocus><option>Sin asignar</option>{validTeam.map(member=><option key={member.id} value={member.fullName}>{member.fullName}</option>)}</select></label><label className="field"><span>Hora de inicio</span><input name="inicio" type="time" value={form.inicio} onChange={update}/></label><label className="field"><span>Hora de fin</span><input name="fin" type="time" value={form.fin} onChange={update}/></label>{form.tipoServicio==='Transporte'?<><label className="field wide"><span>Lugar de recogida</span><input name="origen" value={form.origen} onChange={update} placeholder="ALMACÃ‰N SWIFTPORT" required/></label><label className="field wide"><span>Lugar de entrega</span><input name="destino" value={form.destino} onChange={update} placeholder="BUQUE / EMPRESA / ALMACÃ‰Nâ€¦" required/></label><div className="route-preview wide"><MapPin/><span><small>RECORRIDO DEL CONDUCTOR</small><b>{form.origen||'ORIGEN'} â†’ {form.destino||'DESTINO'}</b></span></div><label className="field wide"><span>ObservaciÃ³n para el conductor</span><textarea name="observacion" value={form.observacion||''} onChange={update} rows="3" placeholder="Ej. llamar antes de llegar, preguntar por Juan, llevar transpaleta, referencia de cargaâ€¦"/></label><label className="field"><span>Empresa de transporte</span><select name="proveedorId" value={form.proveedorId||''} onChange={update}><option value="">Sin proveedor</option>{providers.filter(provider=>provider.activo!==false).map(provider=><option key={provider.id} value={provider.id}>{provider.nombre}</option>)}</select></label><label className="field"><span>Transporte relacionado</span><select name="transporte" value={form.transporte} onChange={update}><option value="">Crear transporte nuevo</option>{transports.map(entry=><option key={entry.id} value={entry.id}>{entry.id}  -  {transportRoute(entry)}</option>)}</select></label></>:<label className="field wide"><span>Lugar / notas</span><input name="titulo" value={form.titulo} onChange={update} placeholder="AlmacÃ©n, terminal, proveedorâ€¦"/></label>}<TransportCargoSelector options={cargoOptions} selectedIds={selectedCargoIds} onChange={setSelectedCargoIds}/><CargoManifest item={relatedCase} transport={form}/>{form.expediente&&<button type="button" className="button tertiary wide calendar-case-link" onClick={()=>{close();openCase(form.expediente)}}>Abrir expediente relacionado <ExternalLink/></button>}<div className="modal-actions wide"><button type="button" className="button tertiary" onClick={close}>Cancelar</button><button className="button primary"><Save/> Guardar transporte</button></div></form></section></div>;
}
function NewCaseModal({clientOptions=[],vessels=[],team=[],close,submit}){
  const warehouse=SWIFTPORT_WAREHOUSE;
  const [form,setForm]=useState({serviceType:'vessel_delivery',buque:'',imo:'',mmsi:'',cliente:clientOptions[0]||'UME Shipping',puerto:'Barcelona',etaDate:'',etaTime:'',etbDate:'',etbTime:'',etdDate:'',etdTime:'',prioridad:'Media',bultos:'',createReception:false,receptionDate:'',receptionStart:'09:00',receptionEnd:'10:00',receptionLocation:warehouse,createTransport:false,transportDate:'',transportStart:'',transportEnd:'',transportDateManual:false,transportStartManual:false,transportPickup:warehouse,transportDelivery:'BUQUE POR CONFIRMAR  -  Barcelona',transportConductor:'Sin asignar'});
  const update=event=>{
  ...
  useEffect(()=>{
    ...
    add.textContent='+ AÃ±adir cliente nuevo';
...
function WarehouseEntryModal({cases,close,submit,csrfToken}){
  ...
  const save=async event=>{
    ...
    try{
      if(!photos.length)throw new Error('AÃ±ade al menos una foto de la mercancÃ­a.');
      ...
      const uploadedPhotos=await Promise.all(photos.map(async photo=>{
        ...
        const merchandise=!line?'MERCANCÃA SIN IDENTIFICAR':`${line.cantidad} ${line.tipo}${Number(line.cantidad)===1?'':'S'}  -  ${Number(line.peso).toLocaleString('es-ES',{maximumFractionDigits:2})} KG${line.seguimiento?`  -  ${line.seguimiento.toUpperCase()}`:''}`;
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)close()}}>
    <section className="modal warehouse-entry-modal" role="dialog" aria-modal="true" aria-labelledby="warehouse-entry-title">
      <div className="modal-head"><div><span className="overline">AlmacÃ©n</span><h2 id="warehouse-entry-title">Registrar mercancÃ­a</h2><p>Indica cantidad y peso de cada grupo recibido.</p></div><button className="icon-button" aria-label="Cerrar" disabled={busy} onClick={close}><X/></button></div>
      <form onSubmit={save}>
        <label className="field wide"><span>Expediente (opcional)</span><select name="expediente" value={form.expediente} onChange={update}><option value="">SIN EXPEDIENTE  -  VINCULAR DESPUÃ‰S</option>{cases.map(item=><option value={item.id} key={item.id}>{caseLabel(item)}</option>)}</select></label>
        {!form.expediente&&<label className="field wide"><span>Buque / referencia de mercancÃ­a *</span><input name="identificacion" value={form.identificacion} onChange={update} placeholder="Ej. DENSA PUMA, BOS CHABLIS, tracking o proveedor" required/></label>}
        ...
        <label className="field"><span>UbicaciÃ³n *</span><input name="zona" value={form.zona} onChange={update} placeholder="Ej. A-01" required/></label>
        <div className="cargo-lines wide">
          <div className="cargo-lines-title"><b>MercancÃ­as</b><button type="button" className="button secondary" onClick={addLine}><Plus/> AÃ±adir tipo</button></div>
          ...
          {form.mercancias.map((line,index)=><div className="cargo-line" key={index}>
            ...
            <label className="field tracking-field"><span>N.Âº seguimiento (opcional)</span><input value={line.seguimiento} onChange={event=>updateLine(index,'seguimiento',event.target.value)} placeholder="Tracking / AWB"/></label>
        ...
        <div className="arrival-files wide">
          <label className="file-picker"><Camera/><span><b>AÃ±adir fotos de la mercancÃ­a *</b><small>Puedes tomar varias fotos; las nuevas se suman a las anteriores</small></span><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" multiple required={!photos.length} onChange={selectPhotos}/></label>
          <label className="file-picker"><FileText/><span><b>Escanear documentos</b><small>Packing list, CMR, delivery note o albarÃ¡n</small></span><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf,.pdf" multiple onChange={event=>setDocuments([...event.target.files])}/></label>
          {Boolean(photos.length)&&<div className="photo-identification"><div className="photo-identification-title"><b>IdentificaciÃ³n fotogrÃ¡fica</b><small>{photos.length} foto(s). Asocia cada evidencia con su mercancÃ­a.</small></div>{photos.map((photo,index)=><article key={photo.id||`${photo.file.name}-${photo.file.lastModified}`}><img src={photo.preview} alt={`Vista previa ${index+1}`}/><div><span className="photo-number">FOTO {String(index+1).padStart(2,'0')}<button type="button" onClick={()=>removePhoto(index)}>Quitar</button></span><label className="field"><span>QuÃ© muestra</span><select value={photo.tipo} onChange={event=>updatePhoto(index,{tipo:event.target.value})}>{PHOTO_TYPES.map(type=><option key={type}>{type}</option>)}</select></label><label className="field"><span>MercancÃ­a asociada *</span><select value={photo.mercanciaIndex} onChange={event=>updatePhoto(index,{mercanciaIndex:event.target.value})} required>{form.mercancias.map((line,lineIndex)=><option key={lineIndex} value={lineIndex}>{line.cantidad} {line.tipo}{Number(line.cantidad)===1?'':'S'}  -  {line.peso||'â€”'} KG{line.seguimiento?`  -  ${line.seguimiento.toUpperCase()}`:''}</option>)}</select></label><label className="field photo-note"><span>ObservaciÃ³n (opcional)</span><input value={photo.nota} onChange={event=>updatePhoto(index,{nota:event.target.value})} placeholder="Ej. esquina golpeada, precinto intactoâ€¦"/></label></div></article>)}</div>}
        ...
        <div className="modal-actions wide"><button type="button" className="button tertiary" disabled={busy} onClick={close}>Cancelar</button><button className="button primary" disabled={busy}><UploadCloud/> {busy?'Subiendo archivosâ€¦':'Registrar entrada'}</button></div>
      </form>
    </section>
  </div>;
}
