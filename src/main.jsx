...
function App({auth,finance,onFinanceChange,onLogout}){
  ...
  const createCase=form=>{
    ...
    const transport=form.createTransport?{id:transportId,expediente:id,ruta:route,fecha:form.transportDate,inicio:form.transportStart,fin:form.transportEnd,hora:formatSchedule(form.transportDate,form.transportStart,form.transportEnd),conductor:'Sin asignar',proveedorId:'',vehiculo:'Por asignar',estado:'Sin asignar'}:null;
    const transportEvent=transport?{id:`EV-${stamp}-T`,titulo:route,tipoServicio:'Transporte',fecha:transport.fecha,inicio:transport.inicio,fin:transport.fin,asignado:'Sin asignar',expediente:id,transporte:transportId,proveedorId:'',color:'gray'}:null;
  ...
  const saveCalendarEvent=event=>{let colored={...event,tipoServicio:event.tipoServicio||(event.transporte?'Transporte':'Recepción'),color:driverTone(event.asignado,operationalTeam)};let nextTransports=transports;const relatedCase=cases.find(item=>item.id===event.expediente);if(colored.tipoServicio==='Transporte'&&!colored.transporte){const transportId=`TR-${Date.now()}`;const route=colored.titulo||`ALMACÉN → ${relatedCase?.puerto||'PUERTO'}`;colored={...colored,transporte:transportId,titulo:route};nextTransports=[{id:transportId,expediente:colored.expediente,ruta:route,fecha:colored.fecha,inicio:colored.inicio,fin:colored.fin,hora:formatSchedule(colored.fecha,colored.inicio,colored.fin),conductor:colored.asignado,proveedorId:colored.proveedorId||'',vehiculo:'Por asignar',estado:colored.asignado==='Sin asignar'?'Sin asignar':'Asignado'},...transports]}else{nextTransports=transports.map(item=>item.id===colored.transporte?{...item,expediente:colored.expediente||item.expediente,ruta:colored.titulo||item.ruta,conductor:colored.asignado,proveedorId:colored.proveedorId||item.proveedorId||'',fecha:colored.fecha,inicio:colored.inicio,fin:colored.fin,hora:formatSchedule(colored.fecha,colored.inicio,colored.fin),estado:colored.asignado==='Sin asignar'?'Sin asignar':item.estado==='Sin asignar'?'Asignado':item.estado}:item)}const exists=calendarEvents.some(item=>item.id===colored.id);const nextCalendar=exists?calendarEvents.map(item=>item.id===colored.id?colored:item):[...calendarEvents,colored];const nextCases=cases.map(item=>item.id===colored.expediente?{...item,conductor:colored.asignado}:item);setCalendarEvents(nextCalendar);setTransports(nextTransports);setCases(nextCases);saveOperational(nextCases,nextTransports,warehouseEntries,customs,nextCalendar);notify(exists?'Tarea, transporte y expediente actualizados':'Trabajo añadido y sincronizado con el calendario')};
...
function CaseServicesPanel({item,events,cases,transports,team,providers,saveEvent}){
  ...
  const addService=type=>{
    ...
    const route=type==='Recepción'?'Recepción en almacén':`ALMACÉN → ${item.puerto||'PUERTO'}`;
    setEditing({id:`EV-${Date.now()}`,titulo:route,tipoServicio:type,fecha:date,inicio:start,fin:plusHourClient(start),asignado:'Sin asignar',expediente:item.id,transporte:'',proveedorId:'',color:'gray'});
  };
...
function NewCaseModal({clientOptions=[],close,submit}){
  ...
  const update=event=>{
    ...
    if(name==='puerto'){setForm(current=>({...current,puerto:value,transportDelivery:`Puerto de ${value}`}));return}
