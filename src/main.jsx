...
function Facturacion({openCase,notify,invoices,cases,warehouseEntries=[],transports=[],calendarEvents=[],clients=[],updateInvoice,syncInvoices,csrfToken,currentUser}){
  ...
  const controlSent=billingControlRows.filter(row=>row.stage==='sent'||row.stage==='overdue'||row.stage==='verify');
  ...
  const billingControlBuckets=[
    ...
    ['all','Todos los expedientes',billingControlRows.length]
  ];
  const billingControlVisible=billingControlView==='attention'?controlAttention:billingControlView==='sent'?controlSent:billingControlView==='confirmed'?controlConfirmed:billingControlView==='excluded'?controlExcluded:billingControlRows;  return <>
